...
The whole process presented until now is repeated for \textit{n\_runs} times and finally the base and contrastive models' mean evaluation results are computed, aggregating the results of the single runs.
...
The malware family predictions and rankings, in this case, were obtained in a slightly different way with respect to how it was done when evaluating the \textbf{MTJE} model on the same tasks, as described in section \ref{sec:baseModelEvaluationWithFreshDataset}. In particular, the rankings were created by considering the samples from the test subset as anchors and the samples from the training subset as query samples, generating one ranking per anchor by sorting the query samples based on their \textit{Euclidean} distance to the current anchor and then truncating the resulting rankings to the top '\textit{q}' ($= 100$ in all experiments) samples. On the other hand, the family classification predictions were obtained by applying the \textbf{Distance Weighted k-NN} approach on the newly created rankings using odd values of '\textit{k}' from '\textit{k\_nn\_min}' ($= 1$) to '\textit{k\_nn\_max}' ($= 11$). In particular, the \textbf{Distance Weighted k-NN} approach is a refinement of the k-NN classification algorithm which consists of weighing the contribution of each of the k nearest neighbours according to their distance to the current anchor sample $x_q$, ultimately assigning greater weights $w_i$ to closer neighbours. The \textbf{k-NN} decision function was therefore the following (eq. \ref{eq:knnDecisionFunction}):
\begin{equation}\label{eq:knnDecisionFunction}
	y_a = F(x_a) = \argmax_{v} \sum_{(x_i, y_i) \in D_k} w_i \times I(v = y_i)
\end{equation}
where $(x_a, y_a)$ is an anchor sample, $(x_i, y_i) \in D_k$ is one of the $k$ nearest neighbours, $D_k$ is the set of all the $k$ nearest training data points to the anchor sample, $I(\textit{condition})$ is the identity function which outputs a $1$ if the condition is true, $0$ otherwise and $w_i$ is the inverse of the square distance between the current nearest neighbour and the anchor sample, as depicted in equation \ref{eq:knnWeight}:
...
\begin{algorithm}[h!]
	...
	\begin{algorithmic}[1]
		\algrestore{bkbreak}
			\State \Comment Continued from previous page..
			\ForAll{\textit{anchor\_shas}, \textit{anchor\_features}, \textit{anchor\_labels} $\in$ \textit{test\_gen}}
				\State \textbf{torch.no\_grad}()
				...
				\ForAll{\textit{query\_shas}, \textit{query\_features}, \textit{query\_labels} $\in$ \textit{train\_generator}}
					\State \textbf{torch.no\_grad}()
					\State \textit{query\_pe\_embeddings} $\gets$ \textit{model}(\textit{query\_features})
					\State
					\State \textit{distances} $\gets$ \textbf{torch.cdist}(\textit{anchor\_pe\_embeddings}, \textit{query\_pe\_embeddings}, p=2.0)
					\State
					\If{\textit{top\_distances} \textbf{is} None}
						...
						\State
						\State \textit{top\_shas} $\gets$ \textit{query\_shas}[\textit{indices}[:\textit{rank\_size}]]
						\State \textit{top\_labels} $\gets$ \textit{query\_labels}[\textit{indices}[:\textit{rank\_size}]]
						\State \textit{top\_distances} $\gets$ \textit{top\_distances}[\textit{indices}[:\textit{rank\_size}]]
					\Else
						\State \textit{top\_shas} $\gets$ \textbf{np.concatenate}(\textit{top\_shas}, \textit{query\_shas})
						\State \textit{top\_labels} $\gets$ \textbf{torch.cat}(\textit{top\_labels}, \textit{query\_labels})
	...
	\begin{algorithmic}[1]
		\algrestore{bkbreak}
				...
				\For{\textit{k}, \textit{s} $\in$ \textbf{enumerate}(\textit{shas})}
					...
					\State \hskip1.9em 'query\_sha': \textit{s},
					\State \hskip1.9em 'ground\_truth\_label': \textit{anchor\_labels}[\textit{k}],
					\State \hskip1.9em 'ground\_truth\_family': \textbf{label\_to\_sig}(\textit{anchor\_labels}[\textit{k}]),
