...
\input{results/contrastive_learning_results.tex}
...
\sommario
\vspace{-0.5cm}
In most recent years the proliferation of malicious software, namely Malware, has had a massive increase: according to AV Atlas Dashboard \cite{AvAtlas} the new malware samples (and PUA - Potentially Unwanted Application) currently detected every day are about 440.000 (at the time of writing), and this number is predicted to only keep growing. The total number of known Microsoft Windows malicious software (and PUA) passed from about 55 million in 2011, to about 400 million in 2016, and finally to nearly 830 million now. The huge number of malware samples out there in the wild renders the detection through manually generated signatures (patterns that identify malicious code) infeasible and consequently imposes the urgent need for tools able to automatically detect malware and possibly describe it in a human-interpretable way. Several methodologies have been proposed through the years, ranging from signature-based detection (especially with \textit{Yara Rules}) to various Machine Learning approaches like Decision Trees, Naive Bayes models and Neural Networks.

This thesis presents a novel model, called \textbf{Multi Task Joint Embedding} (\textbf{MTJE}), built upon previous works in \textbf{ML-based} (Machine Learning) automatic PE (Microsoft Windows Portable Executable) malware detection and description and introduces a new evaluation procedure on the learned implicit representation/signature of malware samples that may prove the applicability of its usage in the Malicious family prediction and ranking tasks. The model is trained on an open source large scale dataset of malware and benignware samples with the aim of creating high quality implicit signatures capable of correctly detecting (and describing) unseen malware samples as well as obfuscated malware and new variants, with high True Positive Rate (TPR) and high Recall at low False Positive Rates (FPRs).

The results show that the proposed \textbf{MTJE} model generates implicit signatures (samples embeddings) which provide higher TPRs, Accuracies, Recalls, Precisions and F1 Scores at low false positive rates with respect to the ones produced by the reference previous methods on the corresponding tasks. When testing the learned representations of the \textbf{MTJE} model on the Malware Family prediction and ranking tasks, however, the results were less promising.

Therefore, a new Malware \textbf{Family Classifier} model, built on top of the \textbf{MTJE} model base topology, was created. This new model was specifically trained and evaluated on the malware family classification task using a specially crafted dataset of $10.000$ PE files, exploiting the parameters from a previous \textbf{MTJE} model training run, with the aid of \textbf{transfer learning}. The introduction of this new Family Classifier provided more meaningful results, although not exceptional, in the family classification task while also demonstrating the potential of using \textit{Transfer learning} in this context. This new model, however, could not be used to produce family rankings nor to query samples based on their similarity and was limited to working only with a fixed number of predefined families.

Therefore, a new \textbf{Contrastive (Siamese) Model} was created based on the \textbf{MTJE} model which overcame such limitations by refining the \textbf{MTJE} model implicit representations of PE executables using the \textbf{Online Triplet Loss} in a \textit{contrastive learning} setting. In this way, the final learned implicit representations of PE files could address both the family prediction/classification and ranking tasks with meaningful results with an indefinite number of families at the cost of having a slightly worse performance in the family classification task than the \textbf{Family Classifier}.

Future (and more performant) works capable of overcoming some of the final models limitations may be very useful to the IT-Security field in the current scenario and could even enable the generation of explicit (and thus more interpretable) signatures derived from the learned implicit ones.
...
Malware first made its appearance in the 1960s. Then, hackers used to design computer viruses mainly for fun, as an exciting prank/experiment; their creations would generally display harmless messages and then spread to other computers \cite{ReganWIM}. There are numerous examples of malware created at that time within a laboratory setting: for example the \textit{Darwin game} in 1962 and \textit{Creeper} in 1971.

In the early 1980s, the concept of malware caught on in the technology industry, and numerous examples of viruses and worms appeared both on Apple and IBM personal computers. With the introduction of the World Wide Web and the commercial internet in the 1990s it eventually became widely popularized, so much that Yisreal Radai coined the term \textit{malware} in 1990.
...
The previously mentioned 1960s and 1970s malware were all kept within a laboratory environment and never managed to escape to the wild. \textit{Elk Cloner} (1981) was the first known virus to have been able to escape its creation environment. It spread through infected floppy disks and simply displayed the 'poem' shown in figure \ref{fig:elkClonerExample} on the user's screen without causing any deliberate harm. Following the success of that prank gone wild, the first Microsoft PC virus, called \textit{Brain}, was then created in 1986. Again, like \textit{Elk Cloner}, Brain was mostly annoying rather than harmful, but it was also the first known virus capable of concealing its presence on disk thus evading detection. In 1988 an experimental, self-propagating, self-replicating program called \textit{Morris}, was released on the internet \cite{NamanyaTWM} and became the first known worm. In 1988 made its appearance also the first example of intentionally harmful virus, the \textit{Vienna} virus, which encrypted data and destroyed files. This led to the creation of the first antivirus tool ever \cite{ReganWIM}.

In the following decades malware has evolved both regarding its complexity and malware sample numbers. In particular, according to AV Atlas \cite{AvAtlas}, in 2019 and 2020 (and until mid 2021 - that is until the time of writing) the number of newly generated malware blew with respect to previous years, as shown in graph \ref{fig:avAtlasReportNew}, to the point that approximately 5.1 new Microsoft Windows malware (and PUA) are currently generated per second, $\sim18,500$ per hour. Consequently, as reported by AV Atlas dashboard and as shown in graph \ref{fig:avAtlasReport}, in recent years the total amount of unique malware (and PUA) variants reached impressive numbers, to the point that they are now more than $830$ millions. Moreover, nowadays malware commonly use obfuscation and other sophisticated techniques, such as Polymorphism and Metamorphism, to evolve their structure thus evading detection.
...
This thesis focuses on the first three phases previously mentioned. In particular, it concentrates on defining, training and evaluating the best model architecture possible for the tasks at hand. However, some code optimization challenges resulting from the slowness of the code in the instance used for the experiments meant that it could be possible to train the model only with the first half of the samples of the Sorel20M dataset in a reasonable time, with some approximations on the samples dispersion when random sampling them from the dataset. This resulted in slightly worse performance than might be expected using the current architecture with the entire dataset. Nevertheless, the deployment of the proposed \textbf{MTJE} model on a real-world scenario is theoretically possible with the current final architecture, although it would be better to train the model on the whole Sorel20M dataset on a better instance first in order to see its true potential.

At a later moment, the proposed framework was extended with the addition of a \textbf{Malware Family Classifier} model head defined on top of the proposed \textbf{MTJE} model base topology in order to improve its relatively poor results in the \textbf{Malware family prediction/classification} task. This new model was then specifically trained (and tested) for such purpose using the training and test subsets of the relatively small \textbf{fresh dataset}, which contain the information about the malware family each sample belongs to. However, instead of training the newly defined architecture from scratch on such small dataset at the risk of overfitting, the technique called \textbf{Transfer Learning} was used by transferring the knowledge (the learned model parameters) from a previous \textbf{MTJE} model training run on the large Sorel20M dataset onto the new model base topology (the one shared with the \textbf{MTJE} model architecture), before training. Then, during the training procedure, some of the imported parameters were '\textit{fine-tuned}' while the ones corresponding to the newly added Family Classifier head were learned from scratch.

However, this new \textbf{Family Classifier} model could not be used to produce family rankings nor to query samples based on their similarity to some anchor, which are very useful tasks in the field of Information Security since they allow to quickly obtain samples similar to the currently analysed one, facilitating its study. Moreover this model was also limited to working only with a fixed number of predefined families. Therefore, in order to overcome such limitations a new model - referred to as \textbf{Contrastive model} in this document - was introduced consisting of a \textit{Siamese Network} which refined, in a contrastive learning setting, the implicit representation of PE files (\textit{PE Embeddings}) learned by a previous \textbf{MTJE} model training run on the Sorel20M dataset (with the aid of \textit{Transfer Learning}) using samples from the training subset of the \textit{fresh dataset} with the \textbf{Online Triplet Loss} function. The final PE Embeddings can, in fact, be used to address both the family prediction/classification - applying the distance weighted k-NN (k Nearest Neighbours) algorithm in the resulting embedding space - and ranking tasks and to query samples based on their similarity in the Embedding space.

\chapter{Background}

\section{Malware}
\textbf{Malware}, short for \textit{malicious software}, is a general term for all types of programs designed to perform harmful or undesirable actions on a system. In fact in the context of IT security the term \textit{malicious software} commonly means \cite{SharpAIM}:
...
The computer system whose security policy is attempted to be breached is usually known as the \textit{target} for the malware. The cybercriminal who originally launched the malware with the purpose of attacking one or more targets, on the other hand, is generally referred to as the \textit{initiator of the malware}. Furthermore, depending on the malware type, the initiator may or may not exactly know what the set of targets is \cite{SharpAIM}.

According to the above definitions software is defined as malicious in relation to an attempted breach of the target's \textbf{\textit{security policy}}. In other words, software is often identified as malware based on its \textit{intended use}, rather than on the particular technique or technology used to build it.
...
More generally, malware serves a variety of purposes. For example, some of its most common uses are: \cite{CraneWIM}

\begin{itemize}
	\item \textbf{To profit financially (either directly or through the sale of products or services)}. For example, attackers may use malware to infect targets' devices with the purpose of stealing their credit account information or cryptocurrency. Alternatively, they may sell their malware to other cybercriminals or as a service offering (\textit{malware-as-a-service}).
	
	\item \textbf{As a means of revenge or to carry out a personal agenda}. For example, Brian Krebs of Krebs on Security was struck by a big DDoS attack in 2016 after having talked about a DDoS attacker on his blog.
	
	\item \textbf{To carry out a political or social agenda}. For example, there exist many Nation-state actors (such as state-run hacker groups in China and North Korea) and hacker groups such as Anonymous.
	
	\item \textbf{As a way to entertain themselves}. Some cybercriminals perpetrate attacks on victims just for fun.
\end{itemize}
...
There are different ways of categorizing malware; one way is by \textbf{how} the malicious software spreads. Another one is by \textbf{what} it \textbf{does} once it has successfully infected its victim's computers (i.e. what is its payload, how it exploits the system or makes it vulnerable).
...
\begin{itemize}
	\item \textbf{\textit{Trojan horse}}. Generally speaking, a \textit{Trojan Horse}, commonly referred to as a \textit{Trojan}, is any program that disguises itself as legitimate and invites the user to download and run it, concealing a malicious payload. When executed, the payload - malicious routines - may immediately take effect and cause many undesirable effects, such as deleting the user files or installing additional malware or PUAs (Potentially Unwanted Apps), depending on the payload attached to them \cite{NamanyaTWM}.
...
There is a wide range of potential attack techniques used by malware, some of which are briefly described below:
...
\begin{itemize}
	...
	Adware, by itself, is annoying but somewhat harmless, since it is solely designed to deliver ads; however, it often comes bundled with spyware (such as keyloggers), and/or other privacy-invasive software that is capable of tracking user activity and stealing information \cite{DuPaulCMT}.
	...
	On the other hand, it is not uncommon to see many (non-malicious) device or software manufacturers ship their products with intentionally hidden backdoors to allow company personnel or law enforcement to access to the system when needed \cite{IngallsTOM}. Alternatively, backdoors are sometimes hidden in programs also by intelligence services.
	
	\item \textbf{\textit{Browser Hijacker}}. A \textit{Browser Hijacker}, also called \textit{hijackware}, is a type of malicious program which considerably modifies the behaviour of the victim's web browser. It can be used to make money off unwanted ads, to steal information from users, or to infect the systems with other malware by redirecting users to malicious websites \cite{IngallsTOM}.	
	
	\item \textbf{\textit{Bots}/\textit{Botnet}}. In general, \textit{bots} (short for \textit{robots}) are software programs designed to automatically perform specific operations. Some bots are used for legitimate and harmless purposes such as video programming, video gaming, internet auctions and online contest, among other functions. It is however becoming increasingly common to see bots being used maliciously. Malicious bots can be (and usually are) used to form \textit{botnets}. A botnet is defined as a network of host computers (zombies/bots) that is controlled by an attacker - the \textit{bot-master} \cite{NamanyaTWM}. Botnets are frequently used for DDoS (Distributed Denial of Service) attacks, but there are other ways that botnets can be useful to cybercriminals: for example they can be employed to carry out brute force attacks on websites or to distribute malware \cite{CraneWIM}.
	
	\item \textbf{\textit{Crypto-miner}}. Crypto-miners are a relatively new malware type. Cybercriminals employ this type of malicious tools to mine Bitcoin and/or other similar digital currencies on the target machine by exploiting the computing power of the victim's system, without the owner realising it. Obviously, the mined coins end up in the attackers' digital crypto wallets.
	...
	\item \textbf{\textit{File-less malware}}. As the term suggests, File-less malware is a type of malware that operates from a victim's computer memory, not from files on the hard drive, taking advantage of legitimate code and tools (known as \textit{LOLBins} \cite{CraneWIM}) already existing within the system. File-less malware leaves no malware files to scan and no malicious processes to detect, and is therefore harder to detect and remove than traditional malware: it is, in fact, up to ten times more successful \cite{BakerMCTM}. Furthermore, it also renders forensic analysis more difficult because it disappears from the victim's computer upon rebooting.
	
	\item \textbf{\textit{Keylogger}}. Keyloggers, often considered as being a sub-category of spyware, are malicious programs which secretly track keystrokes on a keyboard, without the system owner consent - action that is usually referred to as \textit{keylogging} or \textit{Keystroke logging}. The collected data is stored and sent to the attacker who uses it to figure out highly sensitive information such as passwords, usernames and payment details. Keylogging can be performed in various ways, ranging from hardware and software-based approaches to the more sophisticated electromagnetic and acoustic analysis \cite{MullinsMT}.
...
Initially, malware analysts/researchers had to manually analyse each malware sample. This process was however rather complex, it required high expertise, and was quite time-consuming. Moreover, the number of malware samples that nowadays need to be analysed on a daily basis is of the order of thousands. This implies that the analysis of malware samples can no longer be exclusively done manually. As such, several analysis tools have been developed in recent years to facilitate analysts in analysing malware samples.

Traditionally, there are two main types of analysis: \textbf{static} and \textbf{dynamic}. Moreover, these two types can be, and frequently are, combined together (\textbf{hybrid} analysis) in various stages of malware analysis to optimize results \cite{NamanyaTWM}.

\subsubsection{Static analysis}
\textit{Static analysis} consists of examining the code of an executable file without actually executing it. Static analysis techniques usually extract peculiar features from malicious samples in order to be able to recognise them and distinguish them from benign ones. The features usually extracted are, for example, string signatures, byte-sequence n-grams, library or API calls, opcode frequency distributions, peculiar attributes found in the executable header etc. However, this approach, since it is based on signatures/features extracted from already analysed samples, is not much effective on zero-day and evolutionary malware.

A malware analyst performing manual static analysis usually disassembles the binary first, meaning that he \textit{translates} the program's machine code instructions back into assembly language, generating a more human-interpretable code listing. The disassembled binary can then be subject of more advanced static analysis techniques such as control flow analysis, data flow analysis and many more. This is done in order to try understanding the program functionality and inner workings, among other useful information \cite{BayerDAMC}.
...
\textit{Hybrid Analysis} is the combination of static and dynamic analysis. It is a technique that integrates run-time information extracted through dynamic analysis with information extracted  through static analysis in order to have a complete view of the malware's behaviour while avoiding the problems posed by anti-analysis techniques as much as possible.

\subsection{Anti-reversing}
\textit{Anti-reversing} techniques were originally meant to complicate the reverse engineering process, making it as difficult as possible for hackers or any malicious user. Attackers employing reversing methodologies can, in fact, get an insight of the logic of the code as well as hidden information by disassembling the binaries of unprotected files.
...
In this case, the conditional jump \textit{jz loc} is immediately followed by a jump to the same target but with opposite condition: \textit{jnz loc}. This implies that the location \textit{loc} will always be jumped to. Consequently, the combination of \textit{jz} with \textit{jnz} acts, in this case, like an unconditional \textit{jmp} instruction. A disassembler, however, won't recognize this given that it disassembles just one instruction at a time. During the disassembly process, in fact, when the \textit{jnz} instruction is encountered, the disassembler takes the false branch of the instruction first and therefore continues disassembling, even though this branch will never be executed in practice.
...
Example:
\begin{lstlisting}[caption={Jump Instructions with a Constant Condition}, label=code:JumpConstant, language={[x86masm]Assembler}, style=mystyle]
	33 C0		xor	eax, eax
	74 01		jz 	loc
...
The first instruction in the example code, \textit{xor eax, eax}, sets the \textit{EAX} register to zero and, consequently, it sets the zero flag. The next instruction, \textit{jz} (jump if zero flag is set), appears to be a conditional jump but in reality is not conditional at all. In fact, at this point in the program execution, the zero flag will always be set. The disassembler, however, will process the false branch first, even if in reality it would never trigger.

\paragraph{Impossible Disassembly}
The simple anti-disassembly techniques mentioned above are frequently coupled with the use of a, so called, \textit{rogue byte}. A \textit{rogue byte} is a data byte, which is not part of the program logic flow, strategically placed after a conditional \textit{jump} instruction in order to trick the disassembler. The byte inserted usually is the opcode for a multi-byte instruction, therefore disassembling it prevents the real following instructions from being properly disassembled \cite{SikorskiPMA}.
...
Example:
\begin{lstlisting}[caption={Impossible Disassembly}, label=code:ImpossibleDisassembly, language={[x86masm]Assembler}, style=mystyle]
	EB 
				JMP -1
	FF
				INC EAX
...
In this simple example the first instruction is a 2-byte \textit{jmp -1} instruction (\textit{EB FF}). Its target is its own second byte. At run time this causes no errors because the \textit{FF} byte is the first byte of the next instruction \textit{inc eax} (\textit{FF C0}). However, when disassembling, if the disassembler interprets the \textit{FF} byte as part of the \textit{jmp} instruction, it won't be able to interpret it also as the beginning of the \textit{inc eax} instruction.

This 4-byte example code increments the \textit{EAX} register, and then decrements it, therefore it is essentially a complex \textit{NOP} sequence. Being a simple and small sequence it could easily be inserted at any location in a program code in order to fool disassemblers. However this sequence is also easily recognisable by reverse engineers and substituted with \textit{NOP} instructions using IDA Pro or other instruments and/or scripts or forced to be interpreted as data bytes and therefore skipped by the disassembler.
...
\textit{Control-flow analysis} (CFA) is a static-code-analysis technique for determining the control flow of a program. Modern disassemblers like \textit{IDA Pro} are able to correlate function calls and extract high-level information about the program knowing how functions are related to each other \cite{SikorskiPMA}.
...
\textit{Function pointers} are a common programming idiom present in widely adopted programming languages such as \textbf{C}, while being extensively used in the background in object oriented languages like \textbf{C++} and \textbf{Java} \cite{SikorskiPMA}.
...
\begin{itemize}
	...
	This API function is similar to the previously described one (\textit{IsDebuggerPresent}) but it checks for a \textit{remote} debugger on the specified process. The term \textit{remote} in the name CheckRemoteDebuggerPresent does not imply that the debugger necessarily resides on a different machine; instead, it indicates that the debugger resides in a separate and parallel process. This function takes a process handle as argument, and checks if that process has a debugger attached. It can however be used also to check the current process by passing its handle.
	
	\item \textbf{NtQueryInformationProcess}
	This function can retrieve different kinds of information from a process. The first argument for this function is the process handle, the second one is the ProcessInformationClass parameter which specifies the information to retrieve. When using the value \textit{ProcessDebugPort} for this parameter, for example, the function returns a zero if the process is not being debugged; otherwise it returns a non-zero value representing the debugger port number.
...
\begin{itemize}
	...
	The Windows PEB structure contains all user-mode parameters associated with a process, including the process environment data such as environment variables, addresses in memory and debugger status, among other things.
...
Debugging tools typically leave some traces of their presence on the system. Malicious programs can therefore be designed to search for these traces in order to determine when they are being analysed. For example, malware can search for references to debuggers in the registry keys \cite{SikorskiPMA}. Moreover, malware can also be designed to search through the system for files and directories commonly related to debuggers, such as debugger program executables. Finally, malwares can also detect debugger residues in live memory, by viewing the current process listing or, more commonly, by performing a \textit{FindWindow} in search for a debugger.
...
A common anti-debugging technique exploited by malware authors consists in making the process scan its own code in search for an \textbf{INT 3} (opcode \textit{0xCC}) instruction. \textit{INT 3} is, in fact, a software interrupt used by debuggers: when setting a breakpoint the debugger replaces the target instruction in the running program with the opcode \textit{0xCC} (\textit{INT 3}) which causes the process to call the debug exception handler \cite{SikorskiPMA}.
...
Another anti-debugging technique consists in calculating the checksum (cyclic redundancy check (CRC) or MD5 checksum) of a section of the process code. This has the same net effects as scanning the code for software interrupts \cite{SikorskiPMA}.

Again this technique can be countered by using hardware breakpoints instead of software ones, or by modifying the program control flow at runtime with a debugger.
...
Using timing checks it is possible to detect a debugger in the following ways:
\begin{enumerate}
...
\begin{itemize}
	...
	\item \textbf{Checking for Files Indicating a VM}.
...
Packing programs, commonly known as \textit{packers}, are software programs that take an \textit{executable file} or \textit{dynamic link library (DLL)}, compress and/or encrypt its contents and then pack it into a new executable file \cite{SikorskiPMA}.
...
\begin{lstlisting}[caption={Constant folding, before}, label=code:ConstantFoldingBefore, language=C, style=mystyle]
	x = 4 * 5;
\end{lstlisting}

\begin{lstlisting}[caption={Constant folding, after}, label=code:ConstantFoldingAfter, language=C, style=mystyle]
	x = 20;
\end{lstlisting}
...
\begin{lstlisting}[caption={Constant unfolding, before}, label=code:ConstantUnfoldingBefore, language={[x86masm]Assembler}, style=mystyle]
	push	0h
\end{lstlisting}

\begin{lstlisting}[caption={Constant unfolding, after}, label=code:ConstantUnfoldingAfter, language={[x86masm]Assembler}, style=mystyle]
	push	0F9CBE47Ah
...
\textit{Dead code elimination} is another common compiler optimization technique. Its objective is to remove program statements/expressions that have no real effects on the program operation and final results \cite{DangPRE}.
...
\begin{lstlisting}[caption={Dead code elimination, before}, label=code:DeadCodeEliminationBefore, language=C, style=mystyle]
	int f(){
		int x, y;
...
\begin{lstlisting}[caption={Dead code elimination, after}, label=code:DeadCodeEliminationAfter, language=C, style=mystyle]
	int f(){
		return 3;
	}
\end{lstlisting}
...
Reverse engineers frequently rely on control-flow and call graphs to better understand a program's high-level logic. In particular a call graph represents calling relationships between subroutines (functions) in a computer program. Each node of a call graph represents a procedure and each edge (\textbf{f} $\rightarrow$ \textbf{g}) indicates that function \textbf{f} eventually calls procedure \textbf{g}. By making the call graph harder to interpret, obfuscators can hinder the reverse engineers' capability of understanding the program behaviour \cite{DangPRE}. In order to do so, the following two techniques can be used:
...
Using these two operations in combination on a program code results in a degenerated call graph with no clear logic. Moreover, also the function prototypes can be modified by adding extra fake arguments, reordering them and so on, to further hide the high-level logic.
...
\paragraph{Opaque Predicates}
An \textit{opaque predicate} is a non-trivial boolean expression with a constant result (always true or always false) known only at compilation/obfuscation time. By combining it with a conditional \textit{jmp} instruction it is possible to introduce an additional branch in the control flow graph (\textit{CFG}). This specific combination corresponds to the previously mentioned \textit{Jump instruction with a constant condition} anti-disassembly technique. The added branch should look as real as possible in order to elude detection, and it can be used to insert junk code or to form cycles in the control-flow graph to better hide the original program logic \cite{DangPRE}.
...
\textit{Control-flow graph flattening} consists in replacing all control structures within a sub-part of the control flow graph with a single switch statement commonly called \textit{dispatcher}. This is done with the purpose of hiding the true basic block relationships within the dispatcher. When using this technique, first a subpart of the program control flow graph is selected to be substituted by the dispatcher. Some transformations may then be applied to the basic blocks inside the chosen sub-graph (they are split or merged) to further complicate analysis and finally each basic block updates the context of the dispatcher to reflect the relative basic block relationships. The final resulting graph offers no clues about the structure of the algorithm, but has the same logic \cite{DangPRE}.
...
An obfuscator using the technique called \textit{code transposition} effectively reorders the sequence of code instructions of a program without changing its behaviour \cite{YouMalwareOT}. To achieve this two approaches are commonly followed:
\begin{itemize}
	\item Randomly shuffling the instructions and then recovering the original execution order by inserting unconditional branches. This is easily defeated, restoring the original program, by removing (and following) the unconditional branches.
	
	\item Choosing and reordering independent instructions that have no impact on the others. This approach is harder to implement given the complexity of finding independent instructions, but is more effective.
\end{itemize}

\paragraph{Code Virtualization}
\textit{Code virtualization} consists in transforming a program binary code (compiled for a specific machine) into a different binary representation that is understood by an interpreter embedded in the code. More specifically, the instruction set from the source machine is converted into a new, randomly chosen, instruction set. This means that a specific block of Intel x86 instructions, for example, can be converted into a different instruction set, preventing an analyst/attacker from recognizing the generated virtual opcodes after the transformation from x86 instructions \cite{DangPRE}. 

Usually, only some specific blocks of the program code are virtualized (and not the whole program) and inserted back into the executable file alongside the associated interpreter. At run time, the interpreter assumes execution control and translates the virtualized code back to the original byte code before executing it.
...
The first second-generation malwares ever existed exploited encryption in order to evade detection by signature-based antivirus scanners. An encrypted malware typically consists of two parts: the encrypted main body and a decryption code (also called \textit{decryptor}). The objective of the \textit{decryptor} is to recover the original malware code from the encrypted body whenever the infected file is run \cite{YouMOT}.

Moreover, to hide from signature-based scanners, encrypted malware can encrypt its code using a different key at each infection, thus creating a unique encrypted body. The decryption routine (\textit{decryptor}), however, remains the same from one generation to another. This means that encrypted malwares can be detected with signature-based scanners by searching for the decryptor specific code pattern \cite{Sharma_2014}.
...
However, this type of malware is able to generate at most a few hundred different decryptors. For example the virus called \textit{Win95/Memorial} was capable of constructing up to 96 different decryptor patterns. This means that signature-based detection techniques are still able to detect \textit{oligomorphic malwares} by generating the signature of all the decryptors utilised by the specific malware strain \cite{YouMOT}. Still, signature based techniques are not an effective approach to detect \textit{oligomorphic malware} \cite{NamanyaTWM}.

\subsubsection{Polymorphic Malwares}
The \textit{oligomorphic} malware limitations led malware authors to develop a more advanced type of malware called \textit{polymorphic}, which, similarly to \textit{oligomorphic} malware, consists of two parts: the malware encrypted main body and the decryptor. The decryptor is again run when the malware is executed and it enables the execution of the original malware code decrypting the encrypted body. When replication occurs, the malware encrypts its code with a different key, generates the new associated decryptor and encloses it in the new malware variant code. However, in this type of malware countless numbers (millions) of distinct decryptors can be generated by using a powerful toolkit called \textit{the Mutation Engine (MtE)}. In particular the mutation engine is responsible of preventing signature based detection by rearranging the decryptor code using different obfuscation methods including, for example, \textit{dead-code insertion}, \textit{register reassignment}, \textit{subroutine reordering}, \textit{instruction substitution}, \textit{code transposition/integration}, etc. \cite{YouMOT}. The malware appearance is thus changed at each infection \cite{Sharma_2014}.
...
However, a \textit{metamorphic} malware, in order to efficiently evolve its code, needs to be able to recognise, parse and mutate its own body during propagation. This is far from being easy. Moreover, creating a true \textit{metamorphic} malware without arbitrarily increasing its code size is also challenging \cite{NamanyaTWM}.
...
\textit{Integrity checking} can be considered as an important tool for detecting any system modification, but it is more an incident recovery method rather than a malware intrusion/infection prevention method.
...
Some commonly used meta tags are, for example, \textit{author} and \textit{description}, which convey information about the author and purpose of the rule. Moreover, malware analysts sometimes also leave tags with the hashes of the malicious files used for the creation of the rule, or references to blog posts with similar information \cite{SimonWIY}.
...
The core functionality of \textit{YARA} can be extended through the use of modules. Some modules like the \textit{PE module} and the \textit{Cuckoo module} are officially distributed with YARA, however additional ones can also be created. Here are mentioned some useful (in this context) Yara modules:

\begin{itemize}
	\item \textbf{YARA with PE}
	Starting with version 3.0, YARA can parse Portable Executable (PE) files \cite{NinjaYSEWDM}. For example, the rule in listing \ref{code:YaraPE} will check for the string "abc", will parse the PE file and look for "\textit{CreateProcess}" and "\textit{httpsendrequest}" function names in the import sections named '\textit{Kernel32.dll}' and '\textit{wininet.dll}', respectively.
...
\begin{enumerate}
	...
	\textit{Yara rules}, however, also have some limitations. Here are some of the most notable ones \cite{NaikEAGYRETE}:
	\begin{itemize}
...
The \textit{python-based} tool called \textit{YarGen} exploits some smart techniques, namely fuzzy regular expressions, Naive Bayes classifier and Gibberish Detector, to generate \textit{Yara rules}. 
...
\end{enumerate}
\textit{Bi-clustering} provides a simple approach to do this jointly over the features, rather than considering the features one at a time. In particular, the features within a bi-cluster are combined into an '\textit{and}' statement since they co-occur; moreover the '\textit{and}' statements from multiple bi-clusters are placed into an '\textit{or}' statement resulting in a \textit{disjunction of conjunctions} rule formulation.
...
\textit{Semantic-based} malware detection aims at identifying malware by deducing the analysed code logic and comparing it to a database of already known malicious logic patterns. This technique, differently from signature-based detection which looks at the code syntactic properties, tracks the semantics of the program code instructions. This implies that \textit{semantic-based} detection approaches are capable of overcoming some obfuscation attempts and may even be able to detect unknown malware variants \cite{NamanyaTWM}.
...
\textit{Behavioural-based} detection is for the most part immune to obfuscation attempts. However, its applicability is limited since it is based on the time consuming dynamic analysis and on the challenging task of determining the unsafe activities/behaviours to consider within the target environment.

\section{Heuristics-based Detection}
As opposed to traditional \textit{signature-base} detection methods which identify malware by examining code for specific bytes/strings, \textit{heuristic-based} detection uses rules and/or algorithms to search for commands or instructions not commonly found in harmless applications, thus indicating possible malicious intents \cite{MiaoUHSS}.
...
The output for the main malicious/benign label prediction task - $f_{mal}(\textbf{x})$ - is always present (it constitutes the baseline model) and consists of a single dense layer followed by a sigmoid activation function on top of the base shared network. On top of that are added one or more auxiliary outputs with a similar structure to the one described above: one fully connected layer (two for the \textit{tag} prediction task) with a task-specific activation function. Finally, the multi-task loss is produced by computing the sum, across all tasks, of the per-task losses multiplied by the task-specific weights (1.0 for the malicious/benign label prediction task and 0.1 for all other tasks) \cite{ruddALOHA}.
...
The task of predicting if a given binary file, represented by its features $\textbf{x}^{(i)}$, is malicious or benign is optimized by minimizing the binary cross-entropy loss between the malicious/benign output label of the network $\hat{y}^{(i)} = f_{mal}(\textbf{x}^{(i)})$ and the ground truth label $y^{(i)}$. This results in the following loss for a dataset with \textit{M} samples:
...
The first additional target used by the authors of \cite{ruddALOHA} is the count of malicious reports for a given sample from the vendor aggregation service. This is based on the assumption that the more a sample gets reported by different vendors, the more likely it is to be malicious. To model count data, the authors used the Poisson noise model parametrized by the parameter $\mu$, where $\mu$ is the mean and variance of the Poisson distribution. The probability of an observation of $y$ counts is therefore:
...
\begin{equation} \label{eq:alohaMalwareTagLoss}
	...
	\end{split}
\end{equation}
where $y_{t_j}^{(i)}$ indicates whether sample $i$ is annotated with tag $j$, and $\hat{y}_{t_j}^{(i)} = f_{tag_j}(\textbf{x}^{(i)})$ is the prediction of the network for that value.

\paragraph{Final Loss}\label{par:alohaFinalLoss}
Finally, in \cite{ruddALOHA} each model used a loss weight of 1.0 on the aggregate malicious/benign loss and 0.1 on each auxiliary loss. Therefore, when adding the $K$ targets to the main loss, the final loss that gets back-propagated through the model was:
...
These tags were then used to train a Joint Embedding neural network to learn a low dimensional Euclidean representation (embedding) space in which malware samples with similar characteristics are close to each other, having access only to the static binary representation of files; this in turn is used at test/deployment time for automatically predicting tags for new (unseen) files in real time. The representation of a malware sample in the embedding space can therefore be taught also as an implicit \textit{signature} describing its capabilities.
...
In particular, the \textit{Multi-Head} net (fig. \ref{fig:Multi_Head_model}) consisted of a base topology common to the prediction of all tags, and one output head per tag. The shared base topology, which can be thought of a feature extraction (or embedding) network that transforms the input features $\textbf{x}$ into a low dimensional hidden vector $\textbf{h}$, consisted of an input feed-forward layer of output size 2048, followed by a batch normalization layer, an ELU non-linearity and three blocks, each composed by dropout, a linear layer, batch normalization and ELU of output sizes 512, 128, and 32 respectively. Each head, on the other hand, is a binary classifier that predicts the presence or absence of each tag. This is implemented as a single linear layer composed of the same type of basic blocks as in the main base architecture, but with output size 11 (the number of tags being predicted) and a sigmoid non-linearity instead of the ELU activation function to compute the predicted probability for each label. The binary cross-entropy losses between each head output and the corresponding ground truth tags are computed and then added together to form the final loss.
...
\end{equation}
In practice, the embedding vector for tag $t_n$ is simply the $n$-th row of the tag embedding matrix, i.e. $\phi_E(t_n) = \textbf{E}_n$.

Finally, the prediction layer compares both tag and sample embeddings ($\textbf{e}$ and $\textbf{h}$ respectively) and produces a similarity score. This is later run through a sigmoid non-linearity to estimate the probability that sample $\textbf{x}$ is associated with tag $t$ for each $t \in \mathcal{T}$. In the final model implementation presented in \cite{DucauAMDATSE}, the similarity score used was the dot product between the embedding vectors. The output of the network $f_n(\textbf{x} | \theta, \textbf{E})$ therefore was,
\begin{equation}
...
\begin{itemize}
	...
	\item Another baseline file-path-only FP model, which takes only the file paths as input and outputs a malware confidence score.
...
The PE-only model can be easily derived from the PE+FP model by removing the FP arm, taking only $\textbf{x}_{PE}$ as input and fitting parameters $\theta_{PE}$ and $\theta_{O}$. Similarly, the FP-only model can be constructed by using the PE+FP model but without the PE sub-network, taking only $\textbf{x}_{FP}$ as input and fitting parameters $\theta_{FP}$ and $\theta_{O}$. Obviously the first layer of the output sub-network needs to be modified to match the output from the previous layer.
...
\end{equation}
where $y_i$ and $\textbf{x}^{(i)}$ are the label and feature vector of the \textit{i}th training sample, respectively, and $M$ is the number of samples in the dataset \cite{KyadigeLFCEIFPIBMD}.

\section{Malware Normalization}
In order to improve the detection rate of existing anti-malware techniques against malware produced by advanced packers and toolkits, code \textit{normalization} techniques can be exploited. These techniques generally consist of a \textit{normalizer} which accepts obfuscated code as input and tries to eliminate obfuscation producing as output the normalized executable. After \textit{normalization}, the usual signature-based techniques can be applied on the normalized sample \cite{Sharma_2014}.
...
The whole process presented until now is repeated for \textit{n\_runs} times and finally the mean evaluation results of the base and contrastive models are computed, aggregating the results of the single runs.
...
The \textit{\textbf{Sorel-20m}} large-scale dataset, which was released in 2020 and is used in the workflow steps shown in image \ref{fig:workflow_sorel}, consists of nearly 20 million files, each represented by the corresponding pre-extracted features, metadata (including the SHA256 hash digest), malicious/benign label derived from multiple sources, vendor detection counts and descriptive SMART tags which serve as additional targets \cite{HarangSOREL20M}.

The Sorel-20m dataset was built upon the EMBER dataset \cite{AndersonEMBER} - the first standard and open dataset specifically designed to be used for static malware detection - addressing some of its drawbacks that limited its utility as a malware benchmark set. In particular, EMBER contained $900,000$ training samples (300K malicious, 300K benign and 300K unlabelled) and $200,000$ test samples (100K malicious, 100K benign) and is therefore of limited size compared to the private/proprietary datasets commercial malware models are usually trained on, which contain from tens to hundreds of millions of samples. Additionally, the small number of validation samples makes it difficult to evaluate the model performance at low false positive rates due to high variance. Finally, the EMBER dataset only provides a single binary malware/benignware label with no additional information.

Sorel-20m, on the other hand, provides an order of magnitude more samples for analysis: when using the recommended time splits to establish training, validation and test sets, there are $12,699,013$ training samples, $2,495,822$ validation samples and $4,195,042$ test samples, respectively. Furthermore, Sorel20M \cite{HarangSOREL20M} provides $9,919,251$ samples of malware ($7,596,407$ training samples, $962,222$ validation samples and $1,360,622$ test samples), which have been disarmed by setting both the \textit{optional\_headers.subsystem} and \textit{file\_header.machine} to 0 in order to prevent execution. Additionally, complete PE metadata, obtained via the Python \textit{pefile} module using the \textit{dump\_dict} method, is provided for each sample. Finally, the dataset provides for each sample a number of additional target labels (SMART tags) that describe behaviour inferred from vendor family labels, together with the vendor detection count and the malicious/benign label.
...
\begin{itemize}
	...
	\begin{itemize}
		...
		\item The "\textit{ember\_features}" LMDB database contains the EMBER features (EMBER features version 2)
		
		\item the "\textit{pe\_metadata}" LMDB database contains the PE metadata extracted through the \textit{pefile} module, as described above
	\end{itemize}

	\item Moreover, some Pre-trained baseline models (a Pytorch feed-forward neural network (FNN) model and a LightGBM gradient-boosted decision tree model) and their results are also provided, but will not be used in this document.
\end{itemize}

All samples are identified by their sha256 hash which serves as the primary key for the SQLite3 database, and as the key to be used to access the two LMDB databases. LMDB entries are stored as arrays or dictionaries (for Ember feature vectors and PE metadata, respectively) that are then serialized with \textit{msgpack} and compressed with \textit{zlib}.
...
As previously mentioned, Sorel20M dataset consists of a bunch of databases (SQLite3 and LMDB), indexed by the sha256 hash of the files, which contain the feature vectors representing the samples together with the corresponding labels/metadata. In particular the feature vectors were derived from the sample files code through the use of an open source feature extraction function released by Anderson et al. \cite{AndersonEMBER} in 2018 as an attachment to the Endgame Malware BEnchmark for Research (EMBER) dataset. In particular, Sorel20M uses version 2 of this feature extraction code which adds information on data directories to the feature representation with respect to version 1.
...
The PE format structure consists of a number of standard headers (as it can be seen in figure \ref{fig:PEFileStructure}) followed by one or more sections and it encapsulates the information necessary for the Windows OS loader to manage the wrapped executable code: for example, among other things, it tells the dynamic linker how to map the file into memory. In particular, the PE structure usually consist of the following 3 headers:
\begin{itemize}
...
\begin{itemize}
	...
	\item \textbf{Header information} - From the \textit{COFF} header, the timestamp, the target machine (string) and a list of image characteristics (list of strings) are extracted. From the optional header, instead, the EMBER feature extracting code extracts the target subsystem (string), DLL characteristics (a list of strings), the file magic value as a string (e.g. "PE32"), major and minor image, linker, system and subsystem versions, and the code, headers and commit sizes. Then, in order to create the model features, string descriptors such as DLL characteristics, target machine, subsystem, etc. are summarized using the feature hashing trick, with 10 bins allotted for each indicator vector.
...
In the first implementation of the pre-processed data loading code the original \textit{Pytorch} Dataloader was used by passing it a specially crafted class called \textit{Dataset} (derived from the \textit{Pytorch} \textit{torch.utils.data.Dataset} class) which is responsible for loading the pre-processed version of the dataset and for retrieving the data (features, labels and sha256) for each sample '\textit{i}'.

\begin{algorithm}[h!]
	\caption{Dataset class, Init function}\label{alg:DatasetInit}
	\begin{algorithmic}[1]
		\Class{Dataset}
...
\begin{algorithm}[h!]
	\caption{Dataset class, GetItem function}\label{alg:DatasetGetItem}
	\begin{algorithmic}[1]
		\algrestore{bkbreak}
...
\begin{algorithm}[h!]
	\caption{Dataset class, GetAsTensors function}\label{alg:DatasetGetAsTensors}
	\begin{algorithmic}[1]
		\algrestore{bkbreak}
...
\begin{algorithm}[h!]
	\caption{Alt3 FastTensorDataLoader class, Iter function}\label{alg:Alt3FastDataLoaderIter}
	\begin{algorithmic}[1]
		\algrestore{bkbreak}
		\Function{Iter}{\textit{self}}
		\If{\textit{self.shuffle} $=$ \textit{true}}
		\State \textit{self.chunk\_indices} $\gets$ \textbf{randperm}(\textit{self.n\_chunks})
		\Else
...
\begin{algorithm}[h!]
	\caption{Alt3 FastTensorDataLoader class, Next function}\label{alg:Alt3FastDataLoaderNext}
	\begin{algorithmic}[1]
		\algrestore{bkbreak}
			\Function{Next}{\textit{self}}
...
\begin{figure}[h!]
	\vspace*{-0.3cm}
	\centering
	\includegraphics[width=0.99\textwidth]{./images/times_heatmap.png}
	\vspace*{-0.5cm}
...
\end{itemize}
More specifically, the function downloads, if possible, '\textit{\textbf{x}}' samples per malware family from Malware Bazaar ordering them by the time they were first seen (from most to least recent), otherwise it skips the malware family and considers the next in order of importance between the ones selected. It then extracts the features from each sample and creates the new dataset containing \textit{\textbf{n}} $=$ \textit{\textbf{x}} $\times$ \textit{\textbf{m}} samples which are then stored on disk as (\textit{numpy}) memory mapped arrays (as it was done with the pre-processed Sorel20M dataset). Therefore, between the \textit{\textbf{f}} $\ge$ \textit{\textbf{m}} malware families provided to the function, only the first \textit{\textbf{m}} for which \textit{\textbf{x}} PE samples can be retrieved from Malware Bazaar are considered.
...
\begin{algorithm}[h!]
	\caption{Fresh Dataset class, Init function}\label{alg:FreshDatasetInit}
	\begin{algorithmic}[1]
		\Class{Dataset}
...
\begin{algorithm}[h!]
	\caption{Fresh Dataset class, FromFile function}\label{alg:FreshDatasetFromFile}
	\begin{algorithmic}[1]
		\algrestore{bkbreak}
			...
			\State \textit{sig\_to\_label\_path} $\gets$ \textit{os}.\textbf{path.join}(\textit{ds\_root}, 'sig\_to\_label.json')
			\State
			\State \textit{S} $\gets$ \textbf{load\_as\_memmap}(\textit{S\_path}, dtype=\textbf{np.dtype}('U64'), mode='r+')
			...
			\State \textit{sig\_to\_label\_dict} $\gets$ \textbf{json.load}(sig\_to\_label\_path)
			\State
...
\begin{algorithm}[h!]
	\caption{Fresh Dataset class, GetItem function}\label{alg:FreshDatasetGetItem}
	\begin{algorithmic}[1]
		\algrestore{bkbreak}
			\Function{GetItem}{\textit{self}, \textit{index}}
...
\begin{algorithm}[h!]
	\caption{FreshDataset class, GetAsTensors function}\label{alg:FreshDatasetGetAsTensors}
	\begin{algorithmic}[1]
		\algrestore{bkbreak}
			\Function{GetAsTensors}{\textit{self}}
			\If{\textit{self}.\textit{return\_shas}}
...
The dataset class used for opening the fresh dataset - shown in alg. \ref{alg:FreshDatasetInit}, \ref{alg:FreshDatasetFromFile}, \ref{alg:FreshDatasetGetItem} and \ref{alg:FreshDatasetGetAsTensors} - is similar to the one used for the Sorel-20m dataset. This version, however, is also instantiatable with an already opened dataset/subset providing the corresponding tensors. The \textit{Init} function, presented in alg. \ref{alg:FreshDatasetInit}, in fact, instantiates the dataset given a set of already loaded dataset tensors (containing the shas, features and labels) as input, while the \textit{FromFile} function (alg. \ref{alg:FreshDatasetFromFile}) opens the dataset, loads it as as set of memory mapped arrays and finally calls the class Init function.
...
\begin{algorithm}[h!]
	...
	\State \textit{generator} $\gets$ \textbf{Pytorch\_DataLoader}(ds, ...)
	\EndIf
	\end{algorithmic}
\end{algorithm}
...
The actual base model evaluation on the \textit{\textbf{fresh dataset}} is computed by another function called \textit{\textbf{EvaluateFresh}}. This function evaluates the model learned representation in both the f-way malware family classification and malware family ranking tasks.

\paragraph{f-way Malware Family Classification task evaluation}\label{par:fWayFamilyClassificationEvaluation}
The base model learned representation first gets evaluated on the task of \textit{f-way} family classification via nearest neighbour search, using the fresh dataset. In particular a function called \textit{EvaluateFreshScores} randomly samples $k$ files per family as anchor samples, and $q$ files per family as query samples. Each of the $(f \times q)$ query samples is then predicted to belong to the same class as its closest anchor sample in feature space (Joint Embedding space in case of the Joint Embedding and \textbf{MTJE} Models). The number $q$ of query samples - per family - considered during the actual evaluation is set to $23$ and the number of anchor samples $k$ is varied from $1$ to $10$ (included). The sampling (for both anchors and query samples), classification and evaluation steps are repeated 15 times per value of $k$ to obtain uncertainty estimates for the results. More specifically, the \textit{accuracies} and the values of a number of other binary cross-entropy \textit{macro} (per-class average) and \textit{micro} (global average) statistics such as \textit{precision}, \textit{recall} and \textit{f1-score} are collected for each number of anchors $k$ and the resulting trends are plotted (with uncertainty estimates). Moreover, the function saves to file also the confusion matrixes corresponding to the best and worst classification accuracy results for the number $k$ of anchors which produced the overall best accuracy. Finally, also the per-family, micro (global average) and macro (per-family average) ROC-AUC scores are computed for each number $k$ of anchors and the resulting trends are plotted (with uncertainty).

\paragraph{Malware Family Ranking task evaluation}\label{par:FamilyRankingEvaluation}
On the other hand, to evaluate the base model learned representation on the family ranking task, the function first randomly selects '\textit{\textbf{q}}' query samples among the ones in the $fresh\_dataset$. Then, it computes the similarity (using the base model \textit{GetSimilarity} function) between each query sample with the other $n-1$ samples present in the $fresh\_dataset$, ordering the latter by similarity. At this point, for each query sample there is a ranking of all the other $n-1$ samples sorted from most to least similar. Using those rankings the function finally calculates the \textbf{MRR} (\textit{Mean Reciprocal Rank}) and \textbf{MAP} (\textit{Mean Average Precision}) scores for the base model learned representation.

The Mean Reciprocal Rank (\textbf{MRR}, eq. \ref{eq:MRR}) is the average of the Reciprocal Ranks (\textbf{RR}, eq. \ref{eq:RR}) of a series of queries. In particular, the Reciprocal Rank (\textbf{RR}) of a query response is the multiplicative inverse of the rank (position in the ranking) of the single sample belonging to the family of interest which classified with the highest rank (the closest to the query sample).
...
For all experiments presented in this document the number of query samples $q$ was set to 100.
...
\begin{figure}[H]
	...
	\caption[Family Classifier training and evaluation workflow steps]{Family Classifier Training and Evaluation Workflow steps}
	\label{fig:workflow_family_classifier}
\end{figure}
...
At a later moment the training, validation and test subsets of the Fresh Dataset - obtained as described in the previous section (\ref{sec:familyClassifierTrainingAndEvaluationDataset}) by following a 7-1-2 ratio - were also used both to fine-tune (and evaluate) the parameters from a previous \textbf{MTJE} model training run using a \textit{Siamese} network (with \textit{transfer learning}) and to train (and evaluate) an identical \textit{Siamese} network entirely from scratch (with no \textit{transfer learning} applied). In particular, the training and validation subsets were used for training the \textit{Siamese} network in a contrastive learning setting by optimizing the \textit{Triplet Loss} function, while the test subset was utilized to evaluate the resulting refined/learned implicit representation of PE files in the malware Family Classification (using \textbf{k-NN} - k Nearest Neighbours - approach) and ranking tasks. The resulting rankings, MAP and MRR scores along with the multi-class classification statistics - including \textit{accuracy}, \textit{Jaccard Similarity score}, \textit{recall}, \textit{precision} and \textit{f1-score} (\textbf{micro}, \textbf{macro} and \textbf{weighted} averaged) - were then computed and plotted together with the resulting confusion matrix.

The malware family predictions and rankings, in this case, were obtained in a slightly different way with respect to how it was done when evaluating the \textbf{MTJE} model on the same tasks, as described in section \ref{sec:baseModelEvaluationWithFreshDataset}. In particular, the rankings were created by considering the samples from the test subset as query samples and the samples from the training subset as anchors, generating one ranking per query by sorting the anchor samples based on their \textit{Euclidean} distance to the current query and then truncating the resulting rankings to the top '\textit{a}' ($= 100$ in all experiments) samples. On the other hand, the family classification predictions were obtained by applying the \textbf{Distance Weighted k-NN} approach on the newly created rankings using odd values of '\textit{k}' from '\textit{k\_nn\_min}' ($= 1$) to '\textit{k\_nn\_max}' ($= 11$). In particular, the \textit{Distance Weighted k-NN} approach is a refinement of the k-NN classification algorithm which consists of weighing the contribution of each of the k nearest neighbours according to their distance to the current query sample $x_q$, ultimately assigning greater weights $w_i$ to closer neighbours. The \textbf{k-NN} decision function was therefore the following (eq. \ref{eq:knnDecisionFunction}):
\begin{equation}\label{eq:knnDecisionFunction}
	y_a = F(x_a) = \argmax_{v} \sum_{(x_i, y_i) \in D_k} w_i \times I(v = y_i)
\end{equation}
where $(x_a, y_a)$ is an anchor sample, $(x_i, y_i) \in D_k$ is one of the $k$ nearest neighbours, $D_k$ is the set of all the $k$ nearest training data points to the query sample, $I(\textit{condition})$ is the identity function which outputs a $1$ if the condition is true, $0$ otherwise and $w_i$ is the inverse of the square distance between the current nearest neighbour and the query sample, as depicted in equation \ref{eq:knnWeight}:
\begin{equation}\label{eq:knnWeight}
	w_i = \frac{1}{d(x_a, x_i)^2}
\end{equation}
where $d(a, b)$ is the \textit{Euclidean} distance between data points $a$ and $b$.

The mean trends (varying the number of k Nearest Neighbours '$k$') of the classification statistics resulted from the evaluation of the contrastive model were then computed and plotted along with the confusion matrix corresponding to the value of '$k$' which produced the best accuracy.
...
Both the \textbf{ALOHA} \cite{ruddALOHA} and \textbf{Joint Embedding} \cite{DucauAMDATSE} models were therefore implemented in \textit{Python}/\textit{Pytorch} following the descriptions in the respective papers and having as a starting point the example code of Sorel20M dataset.

\section{Implementation}
Both the implementations developed for this project only loosely followed the structure of the original models as described in the respective papers. Here are presented the actual implementation details.
...
The ALOHA model implementation developed for this project (shown in figure \ref{fig:ALOHA_Implementation_model}) consists of a shared base topology and a number of output heads. The shared base topology is composed of 5 blocks, each consisting of Dropout, a dense layer, a Batch Normalization layer, and an Exponential Linear Unit (\textit{ELU}) activation function, with 1024, 768, 512, 512, and 512 hidden units respectively. The exact number of layers, along with the layer dimensions, the dropout probability, the normalization function (layer normalization or batch normalization) and the activation function (\textit{ELU}, \textit{ReLU}, \textit{PReLU} or \textit{LeakyReLU})  used, however, are dynamically set in the code at network definition, thus providing a good amount of customizability. The shared model base, given an input sample feature vector \textbf{x} (of size 2381) got from Sorel20M dataset, outputs an intermediate representation $h=f(\textbf{x})$ of size 512 which is then used as input to the different parallel output heads.
...
\begin{algorithm}[h!]
	\caption{ALOHA Net class, Forward function}\label{alg:alohaNetForward}
	\begin{algorithmic}[1]
		\algrestore{bkbreak}
...
\begin{algorithm}[h!]
	\caption{ALOHA Net class, Compute Loss function}\label{alg:alohaNetComputeLoss}
	\begin{algorithmic}[1]
		\algrestore{bkbreak}
...
The model \textit{base topology} consists of a series of 5 dense blocks, each composed by Dropout, a linear layer, a Batch Normalization layer and ELU (Exponential Linear Unit) activation function, of output sizes 1024, 768, 512, 512 and 512 respectively. Similarly to the ALOHA implementation, at network definition it is again possible to dynamically set the exact number of layers, together with the linear layer sizes, the dropout probability, the actual normalization function to use between \textit{Batch Normalization} and \textit{Layer Normalization} and the activation function (between \textit{ELU}, \textit{ReLU}, \textit{PReLU} and \textit{LeakyReLU}) for the base topology. Anyway, the model base topology outputs an intermediate representation $h$ of size 512 given an input sample feature vector \textbf{x} (of size 2381). This intermediate representation is then used as input to a further linear layer with output size equal to the chosen \textit{\textbf{Joint Embedding}} size (set to 32 throughout all the experiments). The output of this further layer is the one used as input to the \textit{Prediction Layer} and corresponds to the representation of the input file features $x$ in the Joint Embedding space.
...
\begin{algorithm}[h!]
	\caption{Joint Embedding Net class, Init function}\label{alg:jointEmbeddingNetInit}
	\begin{algorithmic}[1]
		\Class{Net}
...
\begin{algorithm}[h!]
	\caption{Joint Embedding Net class, Forward function (with \textit{Dot Product})}\label{alg:jointEmbeddingNetForward}
	\begin{algorithmic}[1]
		\algrestore{bkbreak}
			\Function{Forward}{\textit{self}, \textit{data}}
			\State \textit{rv} $\gets$ \{\}
...
Algorithm \ref{alg:jointEmbeddingNetInit} (\textbf{\textit{Joint Embedding} model, \textit{Init} function}) shows how the \textbf{\textit{Joint Embedding}} model was implemented in \textit{Python}/\textit{Pytorch} code. More specifically, it describes how the network is initialized/defined: first a \textbf{base topology} is defined similarly to the one used in the \textbf{\textit{ALOHA}} model implementation previously described (a series of linear layers interleaved by normalization layers, activation functions and dropout). The \textbf{\textit{PE embedding}} sub-network, on the other hand, is defined as a simple linear layer with output size equal to the chosen \textit{embedding\_dimension}, followed by a Normalization layer and an activation function. Finally, the \textbf{\textit{Tags embedding}} is defined as an embedding matrix of size (\textit{n\_tags}, \textit{embedding\_dimension}), constraining the maximum norm of each tag embedding to \textit{max\_embedding\_norm} (set to 1 throughout the experiments).

Alg. \ref{alg:jointEmbeddingNetForward} (\textbf{\textit{Joint Embedding} model, \textit{Forward} function with \textit{dot product}}), instead, describes how the input data (the numerical feature vectors of PE files) is forwarded through the network to produce the final tag predictions. In particular, the input data is first fed into the model base topology to produce an intermediate representation \textbf{\textit{base\_out}}. This transient output is then used as input for the \textbf{\textit{PE Embedding}} topology to produce the representation of samples in the joint embedding space (\textit{pe\_embedding}). On the other hand, the tags embeddings (one per tag) are extracted from the \textit{tags\_embedding} matrix. The \textbf{similarity scores} between the embeddings of samples and tags are computed via a simple \textbf{\textit{dot product}} (implemented through a matrix multiplication). Finally the tag probabilities are obtained by mapping the similarity scores to the range $[0,1]$ through the \textit{sigmoid} function.

\newpage

In alg. \ref{alg:jointEmbeddingNetForwardCosine} (\textbf{\textit{Joint Embedding} model, \textit{Forward} function with \textit{cosine similarity}}) is presented an alternative to the aforementioned \textbf{Model forward function} that uses the \textbf{\textit{Cosine similarity}} instead of the \textbf{\textit{dot product}} to compute the similarity scores. For the most part this version is equal to the previous one but for how the similarity scores are computed. In fact, in this case they correspond to the \textbf{\textit{Cosine similarities}} between the \textbf{\textit{pe\_embedding}} and the \textbf{\textit{tags\_embedding}}, which are values in the range $[-1, 1]$, mapped to values in the $[0, 1]$ interval. Moreover, in this case, since the similarity scores are already in the $[0, 1]$ range, the tag probabilities coincide with the similarity scores.

\begin{algorithm}[h!]
	\caption{Joint Embedding Net class, Forward function (with \textit{Cosine similarity})}\label{alg:jointEmbeddingNetForwardCosine}
	\begin{algorithmic}[1]
		\algrestore{bkbreak}
		\Function{Forward}{\textit{self}, \textit{data}}
		\State ...
...
A further alternative to the \textit{Model forward function} which uses one of the \textit{Inverted Pairwise Euclidean Distances} proposed in sec. \ref{sec:jointEmbeddingModelImplementation} instead of the \textit{dot product}, is presented in alg. \ref{alg:jointEmbeddingNetForwardPairwiseDistance} (\textbf{\textit{Joint Embedding} model, \textit{Forward} function with \textit{Inverted Pairwise Euclidean (IPE) Distance}}). Again, this alternative is pretty much equal to the original forward function (alg. \ref{alg:jointEmbeddingNetForward}). In this case, however, the similarity scores are produced by inverting the euclidean distances between the embeddings of samples and tags in the joint latent space through the use of the specially crafted \textbf{DistanceToSimilarity} function presented in alg. \ref{alg:distanceToSimilarityFunction} (which simply applies the selected distance inversion formula among the ones mentioned in sec. \ref{sec:jointEmbeddingModelImplementation}).

\begin{algorithm}[h!]
	\caption{Joint Embedding Net class, Forward function (with \textit{\textbf{IPE} Distance})}\label{alg:jointEmbeddingNetForwardPairwiseDistance}
	\begin{algorithmic}[1]
		\algrestore{bkbreak}
		\Function{Forward}{\textit{self}, \textit{data}}
		\State ...
...
Alg. \ref{alg:jointEmbeddingNetGetEmbedding} (\textbf{\textit{JointEmbedding} model, \textit{GetEmbedding} function}) presents the \textit{GetEmbedding} function which is used to extract the PE embedding representation corresponding to the input file feature vector. This is particularly useful when performing different kinds of model performance evaluations together with the \textbf{GetSimilarity function}, which instead returns the similarity between two different input embeddings. Of course, depending on the similarity function selected at model definition the \textit{GetSimilarity} function has to be different. In particular, if the \textbf{dot product} is selected the function, shown in algorithm \ref{alg:jointEmbeddingNetGetSimilarity} (\textbf{\textit{JointEmbedding} model, \textit{GetSimilarity} function with \textit{Dot Product}}), simply computes the similarity scores as the dot product between the two input embeddings and then defines the probabilities by further applying the sigmoid activation function on those scores. On the other hand, if the \textbf{cosine similarity} is selected, the function (alg. \ref{alg:jointEmbeddingNetGetSimilarityCosine} - \textbf{\textit{JointEmbedding} model, \textit{GetSimilarity} function with \textit{Cosine Similarity}}) assigns to both similarity scores and resulting probabilities the normalized (to the $[0,1]$ range) cosine similarity between the two embeddings. Finally, in case the \textbf{Inverted Pairwise Euclidean Distance} is selected, the function (alg. \ref{alg:jointEmbeddingNetGetSimilarityPairwiseDistance} - \textbf{\textit{JointEmbedding} model, \textit{GetSimilarity} function with \textit{Inverted Pairwise Euclidean (IPE) Distance}}) first computes the euclidean distance between the two input embeddings and then assigns to both similarity scores and resulting probabilities the inversion of such distances through the use of the \textit{DistanceToSimilarity} function.

\begin{algorithm}[h!]
	\caption{Joint Embedding Net class, GetEmbedding function}\label{alg:jointEmbeddingNetGetEmbedding}
	\begin{algorithmic}[1]
		\algrestore{bkbreak}
...
\begin{algorithm}[h!]
	\caption{Joint Embedding Net class, GetSimilarity function (with \textit{Dot Product})}\label{alg:jointEmbeddingNetGetSimilarity}
	\begin{algorithmic}[1]
		\algrestore{bkbreak}
...
\begin{algorithm}[h!]
	\caption{Joint Embedding Net class, GetSimilarity function (with \textit{Cosine similarity})}\label{alg:jointEmbeddingNetGetSimilarityCosine}
	\begin{algorithmic}[1]
		\algrestore{bkbreak}
		\Function{GetSimilarity}{\textit{self}, \textit{first\_embedding}, \textit{second\_embedding}}
...
\begin{algorithm}[h!]
	\caption{Joint Embedding Net class, GetSimilarity function (with \textit{\textbf{IPE} Distance})}\label{alg:jointEmbeddingNetGetSimilarityPairwiseDistance}
	\begin{algorithmic}[1]
		\algrestore{bkbreak}
		\Function{GetSimilarity}{\textit{self}, \textit{first\_embedding}, \textit{second\_embedding}}
...
\begin{algorithm}[h!]
	\caption{Joint Embedding Net class, Compute Loss function}\label{alg:jointEmbeddingNetComputeLoss}
	\begin{algorithmic}[1]
		\algrestore{bkbreak}
		\Function{ComputeLoss}{\textit{predictions}, \textit{labels}, \textit{loss\_wts}}
		\State \textit{loss\_dict} $\gets$ \{'total': 0.\}
...
\section{Experiments}
Both models were trained (and validated) for 10 \textit{epochs} each on the first $6M$ training and $1.153.846$ validation samples of the pre-processed Sorel20M dataset (loaded with generator \textit{\textbf{alt3}}) using different numbers of layers, layer sizes (numbers of nodes per layer), activation functions, normalization functions and dropout values for the model base topology. Moreover, various combinations optimizers (\textbf{Adam} or \textbf{SGD}), learning rates (LR), momentum, weight decay and loss weights were used in separate training runs. Moreover, in the case of the \textbf{Joint Embedding} model also different similarity score functions such as the cosine similarity and the inverse of the Euclidean distance were tested instead of the dot product. The models which performed best during validation were then evaluated on the first $1.846.154$ test samples of Sorel20M dataset looking for their tag and/or malicious/benign label (present only in the ALOHA model implementation) prediction binary cross entropy statistics and mean per-sample scores such as \textit{TPR}, \textit{accuracy}, \textit{recall}, \textit{precision}, \textit{f1} score, \textit{Jaccard similarity} and \textit{mean-per-sample-accuracy} at FPR $= 1\%$ as well as the resulting tag and/or malicious/benign \textit{ROC} curves and \textit{AUC} scores.
...
On the other hand, the base topology of the final \textbf{Joint Embedding} architecture consisted of 5 layers of sizes 1024, 768, 512, 512, and 512 respectively, with dropout probability $p_d=5\%$ and \textbf{ELU} activation function. The overall model was trained with a learning rate $LR = 10^{-3}$, \textbf{Adam} optimizer, no momentum nor weight decay. Moreover, the final model used the dot product as similarity score between embeddings.
...
\begin{figure}[H]
	\centering
	\includegraphics[width=\textwidth]{./images/workflow_sorel.png}
	\caption[Training and evaluation workflow steps]{Base Model Training and Evaluation Workflow steps}
	\label{fig:workflow_train_eval_plot}
\end{figure}

Both the \textbf{\textit{ALOHA}} and the \textbf{\textit{Joint Embedding}} model implementations (and also the proposed \textbf{\textit{MTJE}} model that will be defined in next chapter) are trained and evaluated following the procedures depicted by algorithms \ref{alg:trainNetworkFunction} (\textbf{Train network function}) and \ref{alg:evaluateNetworkFunction} (\textbf{Evaluate network function}).

\subsection{Training}
\begin{figure}[H]
	\centering
	\includegraphics[width=\textwidth]{./images/workflow_train.png}
	\caption[Training workflow step]{Base Model Training Workflow step}
	\label{fig:workflow_train}
\end{figure}

In particular, as shown in alg. \ref{alg:trainNetworkFunction}, the training function first imports the chosen Net and Dataset generator (dataloder) implementations and some run variables situated in a \textit{config} file, depending on the network and generator types selected by the user. Next, the model itself is defined by calling the \textit{init} member function of the previously imported \textit{Net} class with all the needed parameters (such as: number of tags, input feature dimension, layer sizes, dropout probability, etc.). Then, the selected optimizer (\textbf{Adam} or \textbf{SGD}) is instantiated by passing to the corresponding \textit{Pytorch} function the model parameters to optimize, the chosen learning rate, weight decay and momentum. This is followed by the definition of the training and validation generators with the proper parameters. Next, the model is trained for '\textit{epochs}' epochs. At each epoch the model is first set into training mode, then, for all the mini-batches randomly sampled from the training generator the optimizer gradients are re-set and the input features are forwarded through the network to get the output predictions. The loss for the current mini batch is then computed and logged. Finally the model parameters are updated by computing the loss gradients and back-propagating them through the network. Moreover, after each training epoch the model is also validated on samples provided by the validation generator. In particular, the model is first set into \textit{evaluation} mode, then, for all the mini-batches of data got from the validation generator the input features are passed through the network to produce the output predictions which are in turn used to compute the loss term. The aggregate loss, in this case, is simply logged (no gradients are computed). Finally, at the end of each epoch the model and optimizer state parameters are saved to file.
...
\begin{figure}[H]
	...
	\caption[Evaluation workflow step]{Base Model Evaluation Workflow step}
	\label{fig:workflow_eval}
\end{figure}

The evaluation function, presented in alg. \ref{alg:evaluateNetworkFunction}, begins in a similar way as the training function by importing the Net and Dataset generator classes along with some run parameters. Then, the function continues by calling the model (\textit{init}) member function with all the needed parameters and by loading the model state checkpoint chosen by the user. This function, however, then sets the model into \textit{evaluation} mode and defines the test dataset generator. Then, for all the mini-batches of data got from the test generator the feature vectors of the input samples are passed to the network and the corresponding predictions are produced and saved to file.
...
\begin{algorithm}[h!]
	...
	\begin{algorithmic}[1]
		...
		\State \hskip12.1em \textit{test\_n\_samples}, \textit{evaluate\_malware},
		\State \hskip12.1em \textit{evaluate\_count}, \textit{evaluate\_tags},
		...
		\State \textbf{save\_to\_file}(\textit{shas}, \textit{labels}, \textit{predictions})
		\EndFor
		\EndFunction
	\end{algorithmic}
\end{algorithm}
...
\begin{figure}[H]
	...
	\caption[Results Computation and plotting workflow step]{Base Model Results Computation and Plotting Workflow step}
	\label{fig:workflow_results}
\end{figure}

The actual binary cross entropy statistics (such as \textit{TPR}, \textit{Accuracy}, \textit{Recall}, etc.. at different FPRs) and the resulting AUC-ROC scores and ROC curves are computed, plotted and saved to file by a distinct set of functions which are not presented in this document given that they simply implement and apply the corresponding mathematical formulas. Moreover, after $n\_runs$ another set of functions computes the mean and standard deviation of the scores resulting from the different runs and plots the mean ROC curves.

\chapter{Proposed Models}
\section{Multi Task Joint Embedding (\textbf{MTJE}) Model}
The \textbf{ALOHA} model (described in section \ref{sec:alohaModelImplementation}) is based on the idea of enhancing the model performance and generalization on the task of \textit{Malicious/Benign} label prediction with respect to a more simple single-task model by exploiting additional tasks (\textit{Vendor Count} prediction, \textit{SMART Tags} prediction, etc.) which are jointly optimized in a Multi-objective/multi-task learning fashion. Multi-objective learning has, in fact, proven to be beneficial for the model generalization since it provides a regularization effect which results in overall better training/validation loss curves and arguably better evaluation results.

On the other hand, the \textbf{Joint Embedding} model (presented in section \ref{sec:jointEmbeddingModelImplementation}) was designed with the idea of constructing an embedding space in which both the samples and tag labels can be mapped to, such that each sample ends up close to the tags it is associated with (and possibly far from the other unrelated tags). The \textit{Joint Embedding} model is trained by optimizing its parameters on the single tag prediction task by means of the similarity between tags and samples embeddings, without additional tasks.

The natural follow-up to those models is a fusion between their two base ideas ultimately creating a model with the main objective of creating a joint embedding space, as done by the \textbf{Joint Embedding} model, while exploiting Multi-objective learning during model training, as done by the \textbf{ALOHA} model. This model, which is called \textbf{Multi Task Joint Embedding} (\textbf{MTJE}) model throughout this document, could potentially perform better in the SMART tag prediction task than the \textbf{Joint Embedding} model implementation given that during the training procedure its parameters are updated considering also the additional targets. Moreover, it could also perform better than the \textbf{ALOHA} model implementation in the \textit{Malicious/Benign} label prediction task since the SMART tags prediction task is solved by the \textbf{MTJE} model in a more difficult way - by learning the joint embedding and calculating similarities - compared to the original solution based on multi-label binary classification with one head for each tag. Harder complementary tasks are, in fact, often used when exploiting \textit{multi-task} learning because they help in finding a more informed latent representation of samples which in turn leads to better performances during testing/deployment.

Moreover, the fact that the learned embedding should pull samples close to their associated tags (and far from the others) also means that samples which are similar or exhibit similar capabilities (sharing the same tags) should be close to each other in the latent space. In turn, the representation of PE files in the learned embedding space, along with the distance function used in this space, enables the computation of file-to-file similarities. This makes it possible to measure how similar two malware samples are in terms of their capabilities. The \textbf{Joint Embedding} and \textbf{MTJE} model approaches are therefore interesting from the perspective of predicting the specific malware family PE files belong to based on their distances with respect to known malware samples. Notice, however, that both the \textbf{Joint Embedding} net and the \textbf{MTJE} model are trained with a different objective - that of predicting the SMART tags associated to each malware sample (and also the \textit{malicious/benign} label and \textit{vendor count} for the \textbf{MTJE} model). It is not uncommon in the ML field to train a model on a specific task and then testing (or deploying) it on a different - but most probably related - task with little degradation in model performance. Moreover, the models are trained on a dataset of malware samples which intrinsically belong to different malware families (although the effective family labels are not provided in the dataset), therefore the information about the family of each sample is implicitly present.

Therefore the \textbf{MTJE} model and the \textbf{Joint Embedding} model implementations are not only evaluated on the tasks they ware trained for (\textit{SMART tag}, \textit{Malicious/Benign} label and \textit{Vendor Count} prediction tasks), but also on the \textbf{f-way Malware Family Classification} task and on the \textbf{Malware Family Ranking} task. Again, for the reasons mentioned above, the \textbf{MTJE} model should perform better than the \textbf{Joint Embedding} model implementation also in these last tasks.

If the \textbf{MTJE} model performs well enough on the malware family \textit{prediction} and \textit{ranking} tasks, it could be possible to use its learned embedding space to define implicit signatures of malware samples belonging to a specific malware family using the corresponding representation in this space, a.k.a. the embedding. These signatures could then be used to craft Yara Rules with the use of a specially crafted custom Yara module that dynamically extracts the embedding representation for new PE files and compares them to known signatures. The resulting rules could in turn be used to detect similar samples as belonging to the specific family represented by each rule. Yara Rules created in this way could potentially inherit the good properties of ML methods such as being able to detect not only known malware strains but also novel variants, being less susceptible to obfuscation attempts, etc. while being compatible with the highly expressive approach that is Yara.

\subsection{Implementation}\label{sec:mtjeModelImplementation}
\begin{figure}[h!]
	\centering
	\includegraphics[width=\textwidth]{./images/MTJE_Model.png}
	\caption[MTJE Model architecture]{\textbf{MTJE} Model architecture}
	\label{fig:MTJE_Model}
\end{figure}

The \textbf{MTJE} model implementation (fig. \ref{fig:MTJE_Model}) consists of 3 main topologies: the \textit{Shared Model Base} sub-network, the \textit{Tag Embedding} and the multiple parallel \textit{Output Heads}.

The model \textit{shared base} topology is composed of 5 sequential dense blocks of output sizes 1024, 768, 512, 512 and 512 respectively. These dense blocks have the same form as those used in the \textit{ALOHA} and \textit{Joint Embedding} model implementations described above (in \ref{sec:alohaModelImplementation} and \ref{sec:jointEmbeddingModelImplementation} respectively): in particular a linear layer with dropout is used, followed by batch normalization and ReLU activation function. Again, at network definition it is possible to dynamically set the parameters for this base topology such as the exact size and number of linear layers, the dropout probability $p_d$, the normalization function and the non-linear activation function used. This \textit{shared base} topology transforms a given input sample feature vector $\textbf{x}$ (of size 2381) into its intermediate representation $h$ of size 512 which is then used as input to the parallel output heads. 

The \textit{Tag Embedding} matrix $\textbf{E} \in \mathbb{R}^{T \times 32}$, which maps the tags to their corresponding representation in the Joint Embedding space, is the same as the one used in the \textit{Joint Embedding} model implementation (\ref{sec:jointEmbeddingModelImplementation}) and is therefore also equal to the one used in the original implementation described in \cite{DucauAMDATSE}. This matrix is used as second input for the \textit{Prediction Layer} of the tags prediction output head.

Similarly to the \textit{ALOHA} model implementation (\ref{sec:alohaModelImplementation}), this model uses multiple parallel \textit{output heads}, each relative to a different task, whose parameters are jointly optimized during training by back-propagating the gradients computed from an aggregate loss. In particular, the tasks used in this case are: the \textit{Malicious/Benign label} prediction task (optional), the \textit{Vendor Count} estimation task (optional) and the \textit{SMART Tags} prediction task. The first two output heads (the \textit{Malicious/Benign label} prediction head and the \textit{Vendor Count} estimation head) are the same as those used in the \textit{ALOHA} model implementation (\ref{sec:alohaModelImplementation}) and can optionally be turned on or off at network definition.

In particular, the \textit{Malicious/Benign} label prediction head is composed of a single dense layer followed by the \textit{sigmoid} non-linearity. The loss between the output of the malicious/benign label prediction head $f_{mal}(x)$ and the ground truth label $y_{mal}$ is computed using the binary cross-entropy loss function.
...
\begin{algorithm}[h!]
	\caption{MTJE Model Net class, Forward function (with \textit{Cosine similarity})}\label{alg:mtjeModelNetForwardCosine}
	\begin{algorithmic}[1]
		\algrestore{bkbreak}
		\Function{Forward}{\textit{self}, \textit{data}}
		\State ...
...
\begin{algorithm}[h!]
	\caption{MTJE Model Net class, Forward function (with \textit{\textbf{IPE} Distance})}\label{alg:mtjeModelNetForwardPairwiseDistance}
	\begin{algorithmic}[1]
		\algrestore{bkbreak}
		\Function{Forward}{\textit{self}, \textit{data}}
		\State ...
...
If, however, the user chooses to try a different similarity function other than the \textbf{\textit{dot product}}, two alternatives are available: the \textbf{\textit{cosine similarity}} or the \textbf{\textit{inverted pairwise euclidean (IPE) distance}}. In particular both the \textbf{\textit{MTJE} model \textit{Forward} function (with \textit{cosine similarity})}, shown in alg. \ref{alg:mtjeModelNetForwardCosine}, and the \textbf{\textit{MTJE} model \textit{Forward} function (with \textit{inverted pairwise Euclidean (IPE) distance})}, shown in alg. \ref{alg:mtjeModelNetForwardPairwiseDistance}, are for the most part identical to the original forward function but for the computation of the similarity scores. The version of this function that uses the \textit{cosine similarity}, in fact, computes the similarity scores as the cosine similarities between the sample and tag embeddings, which are values in the range $[-1,1]$, normalizing them to the $[0,1]$ interval. On the other hand, the version which uses the \textit{inverted pairwise Euclidean distance} computes those scores as the euclidean distances between the two embeddings, inverted using the \textbf{\textit{DistanceToSimilarity}} function already mentioned in sec. \ref{sec:jointEmbeddingNetDefinitionCode}. Again, similarly to what was done in the \textit{Joint Embedding} model implementation, in both cases the final tag probabilities coincide with the computed similarity scores.

The \textbf{MTJE} model implementation uses exactly the same \textbf{\textit{GetEmbedding}} and \textbf{\textit{GetSimilarity}} functions as those used in the \textbf{\textit{Joint Embedding}} model implementation, shown in alg. \ref{alg:jointEmbeddingNetGetEmbedding}. Again, these functions are particularly useful when performing evaluations on the learned embedding.

\begin{algorithm}[h!]
	\caption{MTJE Model Net class, Compute Loss function}\label{alg:mtjeModelNetComputeLoss}
	\begin{algorithmic}[1]
		\algrestore{bkbreak}
			\Function{ComputeLoss}{\textit{predictions}, \textit{labels}, \textit{loss\_wts}}
				\If{\textit{loss\_wts} \textbf{is} None}
...
Finally, algorithm \ref{alg:mtjeModelNetComputeLoss} (\textbf{\textit{MTJE} model, \textit{ComputeLoss} function}) describes how the \textbf{MTJE} model loss computation is implemented in \textit{Python}/\textit{Pytorch} code. In particular the final total loss corresponds to the weighted sum of the losses from each enabled output head. The weights used in this computation can be defined by the user at network definition. The loss computation code regarding the \textit{Malicious/Benign} label prediction and the \textit{Vendor Count} prediction heads was taken from the \textbf{\textit{ALOHA}} model implementation. The first loss is thus implemented as the \textbf{binary cross entropy loss} between the predicted malware label and the respective ground truth label, the latter instead corresponds to the \textbf{Negative Log Likelihood with Poisson distribution} between the predicted count and the ground truth value. On the other hand, the \textit{Tags} prediction loss computation is taken from the \textbf{\textit{Joint Embedding}} model implementation and is therefore the mean of the per-sample sum of all tag losses, each of which is computed as the \textbf{binary cross entropy} loss between the predicted tag probability and the ground truth tag.

\subsection{Experiments}
The proposed \textbf{MTJE} model was trained multiple times for 10 \textit{epochs} on the first $6M$ training and $1.153.846$ validation samples of the pre-processed Sorel20M dataset (loaded with generator \textit{\textbf{alt3}}) with both the optional output heads (\textit{Malicious/Benign} label prediction and \textit{Vendor Count} prediction) on. Each run the model was trained with different network hyper-parameters like the size (number of hidden units per layer) and number of layers, normalization function, activation function and dropout value for the base topology. Other hyper-parameters that were iteratively changed were the used optimizer (\textbf{Adam} or \textbf{SGD}), learning rate (LR), momentum, weight decay, loss weights and the similarity score function used (chosen between the \textit{dot product}, the \textit{cosine similarity} and one of the \textit{inverted Euclidean Distances}: \textit{exp}, \textit{inv} or \textit{inv\_pow}).
...
Moreover, this model learned representation (embedding) of PE samples, together with the \textit{Joint Embedding} model one, was also tested on the \textbf{Fresh Dataset} - which contains $10.000$ malicious samples belonging to $10$ different malware families - to asses its performance in the malware family classification and ranking tasks with respect to the mentioned previous methods. The malware families represented by the fresh dataset were chosen among the most wide-spread PE malware families present in Italy at the time of writing (as reported by \cite{CERTAGID}) for which \textit{Malware Bazaar} \cite{MalwareBazaar} provided at least $1.000$ samples.

In particular, the learned representation was first evaluated on the task of \textit{f-way} family classification via nearest neighbour search as previously described in \ref{par:fWayFamilyClassificationEvaluation} by randomly sampling $k$ ($\in [1, 10]$) anchor samples and $q$ ($=23$) query samples, per-family, and assigning to the $(f \times q = 230)$ query samples the same class as their closest anchor in embedding space. The sampling (for both anchors and query samples), classification and evaluation processes were repeated 15 times per value of $k$ to obtain uncertainty estimates for the results. The trends of some binary cross entropy statistics like \textit{accuracy}, \textit{precision}, \textit{recall} and \textit{f1-score} were computed along with the ROC-AUC score trends and some confusion matrixes.

Then, the learned embedding was evaluated on the malware family ranking task (as described in \ref{par:FamilyRankingEvaluation}) by randomly selecting \textit{\textbf{q}} ($= 100$) query samples from the \textit{Fresh Dataset} and computing, for each of them, the ranking of the other $n_{samples} - 1 = 9.999$, ordering them by similarity. Then, given the resulting rankings, the \textbf{MAP} and \textbf{MRR} scores were calculated.

The final \textbf{MTJE} model architecture consisted of 5 layers of sizes 1024, 768, 512, 512, and 512, respectively (keeping them consistent to the \textit{ALOHA} and \textit{Joint Embedding} model implementations in order to be able to compare their results), with dropout probability $p_d=5\%$, \textit{Batch} normalization, \textbf{ReLU} activation function and the \textit{dot product} as similarity score function. The training was carried out with a learning rate $LR=10^{-3}$, \textbf{Adam} optimizer, no momentum nor weight decay, and loss weights set to 1.0 for the malicious/benign head, 0.1 for the count head and 1.0 for the SMART tag (joint embedding) head.

The \textbf{MTJE} model evaluation results are reported in chapter \ref{chap:results}.
...
\begin{figure}[H]
	\centering
	\includegraphics[width=\textwidth]{./images/workflow_sorel.png}
	\caption[MTJE Model Training and evaluation workflow steps]{MTJE Model Training and Evaluation Workflow steps}
	\label{fig:workflow_proposed_train_eval_plot}
\end{figure}
\vspace{-0.2cm}

As previously mentioned in section \ref{sec:alohaAndJointEmbeddingTrainAndEvaluate}, the \textbf{\textit{MTJE}} model training and evaluation procedures are the same as those used for the \textbf{\textit{ALOHA}} and \textbf{\textit{Joint Embedding}} model implementations. In particular, the training procedure is presented in alg. \ref{alg:trainNetworkFunction}, while the model evaluation is shown in alg. \ref{alg:evaluateNetworkFunction}.
...
\begin{figure}[H]
	\centering
	\includegraphics[width=\textwidth]{./images/workflow_base_fresh_dataset.png}
	\caption[MTJE Model fresh evaluation workflow steps]{MTJE Model Evaluation on Fresh Dataset Workflow steps}
...
As previously mentioned, both the \textbf{\textit{MTJE}} model and the \textbf{\textit{Joint Embedding}} model implementations are also evaluated on the \textit{f-way} malware family classification and family ranking tasks. In particular the function presented in algorithm \ref{alg:evaluateFreshFunction} (\textbf{Evaluate fresh function}) is used to produce both the mentioned evaluation results. More specifically, it calls the following two functions in succession: \textbf{\textit{EvaluateFreshScores}} and \textbf{\textit{EvaluateFreshRankings}}.

\subsubsection{Model Evaluation on f-way Malware Family Classification/Ranking tasks}
\begin{figure}[H]
	\centering
	\includegraphics[width=\textwidth]{./images/workflow_fresh_eval.png}
	\caption[MTJE Model fresh evaluation workflow step]{MTJE Model Evaluation on Fresh Dataset Workflow step}
...
\begin{algorithm}[H]
	...
	\begin{algorithmic}[1]
			...
			\ForAll{\textit{n\_anchors} $\in$ \textbf{range}(\textit{min\_anchors}, \textit{max\_anchors} $+ 1$)}
				...
				\ForAll{\textit{j} $\in$ \textbf{range}(\textit{n\_evaluations})}
					...
					\State \hskip1.9em 'predictions': \textit{anchors}['labels'][\textbf{argmax}(\textit{similarity\_scores})],
...
\begin{algorithm}[h!]
	...
	\begin{algorithmic}[1]
			...
			\State \textit{ranking\_scores}['MRR'].\textbf{append}(\textbf{mean\_reciprocal\_rank}(\textit{rs}))
			\State \textit{ranking\_scores}['MAP'].\textbf{append}(\textbf{mean\_average\_precision}(\textit{rs}))
			\State
			...
			\State \textit{global\_ranks\_to\_save}.\textbf{append\_if\_relevant}(ranks\_to\_save)
			...
			\ForAll{\textit{shas}, \textit{features}, \textit{labels} $\in$ \textit{generator}}
				...
				\ForAll{\textit{n} $\in$ \textbf{range}(\textit{n\_families})}
					...
					\If{\textit{samples} \textbf{is} None}
						\State \textit{samples} $\gets$ \{'shas': \textit{shas}[\textit{indices}],
...
\begin{figure}[H]
	...
	\caption[MTJE Fresh results computation and plotting workflow step]{MTJE Fresh Results Computation and Plotting Workflow step}
	\label{fig:workflow_fresh_results}
\end{figure}
\vspace{-0.3cm}

The actual trends of the binary cross entropy statistics for the \textit{f-way} malware family classification task (\textit{Accuracy}, \textit{Recall}, \textit{precision} etc..) and the resulting \textit{AUC-ROC} scores are computed, plotted and saved to file by a distinct set of functions which are not presented in this document given that they simply implement and apply the corresponding mathematical formulas.

\section{Malware Family Classifier}
Given the \textbf{MTJE} model relatively poor results in the Malware Family Classification task (which it was never trained for), the next step was the development of a \textbf{Malware Family Classifier} specifically trained (and tested) on the \textbf{Fresh Dataset} - which contains information about the malware family each sample belongs to - for such task. However, this dataset, which contains $10.000$ samples divided in $7.000$ training, $1.000$ validation and $2.000$ test samples, is arguably too small to produce a general enough model able to reliably predict the family label for each malware sample without overfitting on the training sub-split. This problem was therefore tackled with the aid of \textbf{transfer learning}.
...
\begin{itemize}
	...
	\item \textbf{Fine Tuning}. Here, a few of the last layers of a frozen pre-trained model base are un-freezed and jointly trained together with the newly-added classifier layers. The higher-order feature representations are thus \textit{fine-tuned} in the model base topology in order to make them more relevant for the specific target task.
\end{itemize}

The intuition behind using Transfer Learning in this context is the following: a model trained on a large and general enough dataset of PE samples (such as the \textbf{MTJE} model trained on Sorel-20m dataset) could effectively serve as a generic model of the world of PE files. These learned feature representations can then be taken advantage of while solving the different but related task of Malware Family Classification without having to start from scratch by training a model on a large dataset. This in turn should make it possible to train a Malware Family Classifier using the relatively small Fresh Dataset while obtaining meaningful results.

A Malware Family Classifier was thus constructed by adding a multi-class classifier sub-network on top of the PE embedding layer of the trained \textbf{MTJE} model - thus exploiting the learned embedding representation of PE files. The resulting model was trained (and validated) on the training (and validation) sub-split of the fresh dataset such that the imported \textbf{MTJE} model parameters are just \textit{fine-tuned} while those of the newly added classifier are learned from scratch. This practice should provide improved results in the Malware Family Classification task with respect to using just the \textbf{MTJE} model learned representation of PE files (as presented in last section) as well as with respect to a similarly complex family classifier trained entirely from scratch with the '\textit{small}' fresh dataset.

In order to evaluate the effectiveness of this approach with respect to simply training an equivalent family classifier entirely from scratch, two models were defined and trained with the same model hyper-parameters (learning rate, number of layers, etc.) and for the same amount of epochs: one had some of its parameters imported from a pre-trained \textbf{MTJE} model and \textit{fine-tuned} while the other was trained entirely from scratch. The resulting two models were then evaluated by recording and comparing their accuracy and some \textit{micro} (global average), \textit{macro} (unweighted per-class average) and \textit{weighted} (per-class average, weighted by support) multi-class classification statistics such as \textit{Jaccard-score}, \textit{recall}, \textit{precision} and \textit{f1-score} along with the corresponding \textit{AUC-ROC} scores.
...
In particular, the \textit{Shared Base} topology and the \textit{PE Embedding} layer are defined to be equal to those used by the \textbf{MTJE} model (and \textbf{Joint Embedding} Model) implementation. This is done in order to be able to apply transfer Learning by importing the model parameters from a previous \textbf{MTJE} model training run on the Sorel-20m dataset. These parameters can be then \textit{fine-tuned} (further optimized) on the new family classification task using samples from the fresh dataset. On the other hand, the \textit{Family Classifier} output head, which is built on top of the \textit{PE Embedding} layer, is new and its parameters are therefore always learned from scratch.

The \textit{Family Classifier} topology consists of 5 dense blocks of output sizes 128, 256, 128, 64 and 10, respectively. These dense blocks are designed in the same way as the ones used in the \textbf{MTJE} model base topology - a linear layer with dropout, followed by batch normalization and ReLU activation function - but for the last one which simply consists of a simple linear layer trailed by the \textit{Sigmoid} non-linearity. As always, at network definition it is possible to dynamically set the hyper-parameters for this topology (as well as for the shared base topology) such as the exact size and number of layers, the dropout probability $p_d$, the normalization function and the activation function used. However, most of the hyper-parameters for the \textit{Family classifier} topology are shared with the model \textit{shared base} with the exception of the size and number of layers, which can be independently set. The \textit{Family classifier} output head transforms a given input sample PE embedding (of size 32) into a set of 10 class scores (one per family) which can be used to produce the family label probabilities by applying the \textit{softmax} function. The final family prediction, for each sample, is then computed by taking the family label with the highest score/probability. The loss between the family class output prediction $f_{fam}(x)$ and the ground truth family label $y_{fam}$, on the other hand, is computed using the Pytorch \textit{Cross Entropy Loss} function. This corresponds to computing the mean of the per-sample losses as depicted by equation \ref{eq:familyClassifierLoss}.
\begin{equation}\label{eq:familyClassifierLoss}
	L = \frac{\sum_{i=1}^{N} L(i, y_{fam_i})}{N}
\end{equation}
where $N$ is the total number of samples for the current training mini-batch and $L(i, y_{fam_i})$ is the loss for sample $i$ having ground truth family label $y_{fam_i}$ as defined in equation \ref{eq:FamilyClassifierPerSampleLoss}.
...
\vspace{-0.2cm}
\begin{algorithm}[H]
	\caption{Malware Family Classifier Net class, Init function, part 1}\label{alg:malwareFamilyClassifierNetInitPt1}
	\begin{algorithmic}[1]
		\Class{Net}
...
\begin{algorithm}[h!]
	\caption{Malware Family Classifier Net class, Init function, part 2}\label{alg:malwareFamilyClassifierNetInitPt2}
	\begin{algorithmic}[1]
		\algrestore{bkbreak}
			...
			\ForAll{\textit{i}, \textit{ls} $\in$ \textbf{enumerate}(\textit{fam\_class\_layer\_sizes})}
				...
				\Else
				\State \textit{fam\_class\_layers}.\textbf{append}(\textbf{nn.Linear}(\textit{fam\_class\_layer\_sizes}[\textit{i} $-1$], \textit{ls}))
			\EndIf
			\State
			\State \textit{fam\_class\_layers}.\textbf{append}(\textit{normalization\_function}(\textit{ls}))
			\State \textit{fam\_class\_layers}.\textbf{append}(\textit{activation\_function}())
			\State \textit{fam\_class\_layers}.\textbf{append}(\textbf{nn.Dropout}(\textit{dropout\_p}))
		\EndFor
		\State
...
Algorithms \ref{alg:malwareFamilyClassifierNetInitPt1}-\ref{alg:malwareFamilyClassifierNetInitPt2} (\textbf{\textit{Malware Family Classifier}, \textit{Init} function}) shows how the Family Classifier model is initialized. In particular the function first defines the model \textit{shared base} topology (\textit{model\_base}) as a sequence of blocks composed each of a linear layer, a Normalization layer, an Activation function and dropout, exactly as previously done in the \textbf{MTJE} model implementation. Next, the \textit{pe\_embedding} topology is constructed as a single linear layer with output size equal to the \textit{embedding\_dimension} ($=32$), followed by a Normalization layer and an Activation function. Thereafter, the function defines the \textit{Family Classifier} topology as a sequence of dense blocks - equal to the ones used in the \textit{shared base} part - trailed by a final linear layer of output size equal to the number of family labels ($=10$) and, optionally, a \textit{Softmax} non-linearity.

\begin{algorithm}[h!]
	\caption{Malware Family Classifier Net class, Forward function}\label{alg:malwareFamilyClassifierNetForward}
	\begin{algorithmic}[1]
		\algrestore{bkbreak}
		\Function{Forward}{\textit{self}, \textit{data}}
		\State \textit{base\_out} $\gets$ \textit{self}.\textit{model\_base}(\textit{data})
		\State \textit{pe\_embedding} $\gets$ \textit{self}.\textit{pe\_embedding}(\textit{base\_out})
		\State \textit{scores} $\gets$ \textit{self}.\textit{family\_classifier}(\textit{pe\_embedding})
...
Algorithm \ref{alg:malwareFamilyClassifierNetForward} (\textbf{\textit{Malware Family Classifier}, \textit{Forward} function}), on the other hand, shows how the input data (numerical feature vectors of PE files) is forwarded through the model. In particular, the function feeds the model base topology (\textit{model\_base}) with the input data producing an intermediate representation \textit{base\_out}. This intermediate representation is then forwarded to the \textit{pe\_embedding} subnet which outputs the PE latent representation of the input samples (of size 32). At the beginning of the family classifier model training procedure, the mapping function between the features of the input samples and the produced PE embeddings corresponds, if transfer Learning was applied, to the one learned by the \textbf{MTJE} model using the Sorel-20m dataset. The output class (family) scores are computed by feeding the \textit{Family classifier} topology with the produced PE embeddings. Finally, the family label probabilities are simply obtained by applying the \textit{Softmax} function to the previously computed family \textit{scores}.

\begin{algorithm}[h!]
	\caption{Malware Family Classifier Net class, Compute Loss function}\label{alg:malwareFamilyClassifierNetComputeLoss}
	\begin{algorithmic}[1]
		\algrestore{bkbreak}
...
Finally, as presented by algorithm \ref{alg:malwareFamilyClassifierNetComputeLoss} (\textbf{\textit{Malware Family Classifier}, \textit{ComputeLoss} function}), the loss between the predicted family scores and the ground truth family label is simply computed by applying the selected loss criterion, which corresponds in this case to the Pytorch \textbf{cross\_entropy\_loss} function.

\subsection{Experiments}
The resulting model was trained (and validated) for 25 \textit{epochs} on the training and validation sub-splits of the \textbf{Fresh Dataset} using different numbers of layers and numbers of nodes per layer (layer sizes) for the Family classifier additional output head while keeping the same model shared base topology inherited by the previously described \textbf{MTJE} model implementation. Moreover, both the \textit{Adam} and \textit{SGD} optimizers were used in separate family classifier training runs with various combinations of learning rates (LR), momentum, weight decay and loss weights. The model which performed best during validation was then evaluated on the test sub-split of the \textit{Fresh Dataset} producing the corresponding accuracy and multi-class classification \textit{micro} (global average), \textit{macro} (unweighted per-class average) and \textit{weighted} (per-class average, weighted by support) scores such as \textit{Jaccard Similarity}, \textit{recall}, \textit{precision}, \textit{f1} score, as well as the resulting \textit{OVO} (One Vs. One) and \textit{OVR} (One Vs. Rest) \textit{AUC-ROC} scores.

The final Family Classifier output head architecture consisted of 5 layers of sizes 128, 256, 128, 64, and 10 respectively, with dropout probability $p_d=5\%$ and \textbf{ReLU} activation function and was trained with a learning rate $LR = 10^{-3}$, \textbf{Adam} optimizer, no \textit{momentum}, $weight decay = 0.01$ and $batch size = 250$.
...
\begin{algorithm}[h!]
	\caption{Family Classifier Train function, part 1}\label{alg:trainFamilyClassifierFunctionPt1}
	\begin{algorithmic}[1]
...
\begin{algorithm}[h!]
	\caption{Family Classifier Train function, part 2}\label{alg:trainFamilyClassifierFunctionPt2}
	\begin{algorithmic}[1]
		\algrestore{bkbreak}
				\State \Comment Continued from previous page..
				\ForAll{\textit{epoch} $\in$ \textbf{range}(\textit{epochs})}
...
The \textbf{\textit{Family Classifier Training} function}, as shown in alg. \ref{alg:trainFamilyClassifierFunctionPt1}-\ref{alg:trainFamilyClassifierFunctionPt2}, first opens and loads the \textit{Fresh Dataset} splitting it in training, validation and test subsets following the selected proportions. The \textit{Family Classifier} model then gets instantiated by providing the necessary parameters to its \textit{init} function. Next, if a checkpoint of a \textbf{MTJE} model training run is provided, it is used to initialize the state parameters of the \textit{shared base} topology and \textit{PE embedding} layer of the newly instantiated \textbf{Family Classifier} model. Then, the learning rates for the parameters of the different parts of the model architecture are set such that those of the \textit{shared base} and \textit{PE embedding} topologies ones are just \textit{fine-tuned} while training the model (by using a smaller learning rate). Then, the selected optimizer (\textbf{Adam} or \textbf{SGD}) gets instantiated given the model parameters, selected learning rate, weight decay and momentum. At this point a scheduler is also instantiated with the purpose of modifying the learning rate (multiplying it by $0.1$) after $3/4$ of the total number of epochs. After these preliminary steps, the function trains (and validates) the \textbf{Family Classifier} model for '\textit{epochs}' epochs. More specifically, at each epoch, the model is set into training mode and then, for all the mini-batches of data randomly sampled from the \textit{fresh dataset} training generator, the input features are fed into the model to obtain the corresponding output class (family) scores. Next, the loss between the predicted class scores and the ground truth family labels is computed and used to produce the gradients which are back-propagated through the network. Moreover, the function also computes the predicted family label by selecting, for each sample, the family label with the maximum class score, and the model accuracy for the current mini-batch by dividing the number of correct predictions by the total amount of predictions. Then, the previously computed model loss and accuracy are logged and the model is set into \textit{validation} mode after having updated the scheduler. Finally, for each mini-batch of data from the \textit{fresh dataset} validation generator, the function computes and logs the model loss and accuracy.

\subsubsection{Family Classifier Evaluation}
\begin{figure}[H]
	\centering
	\includegraphics[width=\textwidth]{./images/workflow_family_classifier_evaluate.png}
	\caption[Family Classifier Evaluation Workflow step]{Family Classifier Evaluation Workflow step}
	\label{fig:workflow_fam_eval}
\end{figure}

\begin{algorithm}[h!]
	\caption{Family Classifier Evaluate function}\label{alg:evaluateFamilyClassifierFunction}
	\begin{algorithmic}[1]
...
Algorithm \ref{alg:evaluateFamilyClassifierFunction} (\textbf{\textit{Family Classifier Evaluation} function}) shows how the \textbf{Family Classifier} model evaluation function is implemented in Python/Pytorch code. In particular, the function first opens and loads the \textit{Fresh Dataset} splitting it in training, validation and test subsets following the selected proportions, as done by the training function. The \textit{Family Classifier} model then gets instantiated by providing the necessary arguments to its \textit{init} function and the checkpoint state parameters from a previous training run are loaded. Thereafter, the model is set into \textit{evaluation} mode and, for all the mini-batches of data from the \textit{Fresh Dataset} test generator, the model predictions, overall accuracy and family label probabilities are computed and saved to file/logged.
...
The Family classifier evaluation \textit{micro} (global average), \textit{macro} (unweighted per-family average) and \textit{weighted} (per-family average, weighted by support) scores, such as \textit{Jaccard similarity score}, \textit{recall}, \textit{precision} and \textit{f1-score}, are computed and saved to file, together with the resulting \textit{OVO} (One Vs. One) and \textit{OVR} (One Vs. Rest) AUC-ROC scores and confusion matrix, by a distinct set of functions - which simply apply the respective mathematical formulas - not presented in this document.

\section{Contrastive Learning Model}
Although the \textit{Family Classifier} presented in the previous section manages to provide better results in the malware family prediction/classification task compared to the original \textbf{MTJE} model learned representation of PE files, it still presents some limitations. One of the shortcomings of such model is the fact that, since the family prediction is obtained by applying the \textit{softmax} function on the model output tensor - of fixed size - which contains the per-family probabilities, it only works when a fixed number of families is defined. At model deployment time, however, it is very much convenient for malware analysts and system administrators to have a model that works with an indefinite number of malware families. In fact, with the ever increasing number of new malware families and variants that are appearing every day it is important to allow some degree of flexibility to the model by enabling the periodic updating of the model learned parameters with up-to-date samples even belonging to malware families never seen before. Furthermore, the Family Classifier model directly produces the malware family prediction and therefore cannot be used to produce rankings or to query samples based on their similarity to some anchor. However, these are very useful tasks in the field of Information Security since they allow to quickly obtain samples similar to the currently analysed one, facilitating its study.

A new model was therefore introduced consisting of a \textit{\textbf{Siamese Network}} which refines, in a contrastive learning setting, the implicit representation of PE files learned by a previous \textbf{MTJE} model training run on the Sorel20M dataset with the aid of \textit{Transfer Learning}. In particular, this was done by training the new model on the training subset of the \textit{Fresh Dataset} using as loss function the \textbf{\textit{Triplet Loss}}.

The \textbf{\textit{Triplet loss}} is a loss function frequently used in image recognition which was first introduced by the paper \cite{SchroffFN} from Google for solving the face recognition task.

In this project the \textit{Triplet loss} was used in order to learn good embeddings for the input PE files such that, in the embedding space, PE files from the same family are close to each other and form well separated clusters.

The goal of the \textit{Triplet loss} is to make sure that:
\begin{itemize}
	\item Two examples with the same label have their embeddings close to each other in the embedding space
	\item Two examples with different labels have their embeddings far away
\end{itemize}

At the same time, the loss function should not push the training embeddings of each label to collapse into very small clusters during training.

The \textit{Triplet loss} function is defined over triplets of embeddings:
\begin{itemize}
	\item an \textbf{anchor} sample '$a$'
	\item a \textbf{positive} sample '$p$' of the same class as the anchor
	\item a \textbf{negative} sample '$n$' of a different class
\end{itemize}

Therefore the loss to minimize during training, given a triplet $(a, p, n)$ and a distance function $d(a, b)$ between two data points $a$ and $b$ (which was set to be the \textit{Squared Euclidean Norm} throughout the experiments), is:
\begin{equation}\label{eq:tripletLossFormula}
	L = \max(d(a, p) - d(a, n) + margin, 0)
\end{equation}

This pushes the distance $d(a, p)$ to $0$ and $d(a, n)$ to be greater than $d(a, p) + margin$. Moreover, as soon as '$n$' becomes an '\textit{easy negative}' the loss becomes $0$. The \textit{margin} present in the triplet loss, which is very similar to the margin used in SVMs, here is used to separate the clusters of each class.

Given how the triplet loss was defined there are three categories of triplets:
\begin{itemize}
...
Since each of these definitions depend on where the negative is, relatively to the anchor and positive, the previously defined categories can be extended to the negatives samples: \textbf{hard negatives}, \textbf{semi-hard negatives} or \textbf{easy negatives}.

There are two possible approaches to produce triplets from a dataset of samples belonging to different classes:
\begin{itemize}
	\item \textbf{Offline Triplet Mining}: following this approach the triplets are produced offline at the beginning of each epoch. In particular, all the embeddings on the training set are computed, and then only the hard or semi-hard triplets are selected. These triplets can then be used to train the model for one epoch.
	
	In practice, first a list of triplets $(i, j, k)$ is produced, then a series of batches of these triplets of size $B$ is created. This means that during training it is needed to compute $3B$ embeddings to get the $B$ triplets, compute the loss of these $B$ triplets and then back-propagate it through the network.
	
	This makes this approach not very efficient since it requires a full pass on the training set to generate triplets and to update the offline mined triplets regularly.
	
	\item \textbf{Online Triplet Mining}: following this approach the useful triplets are computed on the fly, for each batch of inputs. In practice, given a batch of $B$ examples (for instance $B$ PE files), $B$ embeddings are computed obtaining a maximum of $B^3$ triplets, most of which are not \textbf{valid}. In particular a triplet $(i, j, k)$, with $i, j, k \in [1, B]$, is said to be \textbf{valid} if samples $i$ and $j$ have the same label but are distinct, and sample $k$ has a different label.
	
	This technique provides more triplets for a single batch of inputs, and does not require any offline mining. It is therefore much more efficient.
\end{itemize}

The approach chosen for this project is that of \textbf{Online Triplet Mining} since it is more efficient and doesn't require substantial (if any) modifications to the current \textit{Fresh Dataset} dataloader implementation.

When using \textbf{Online Triplet Mining}, supposing to have a batch of input samples of size $B = FK$, composed of $F$ different families with $K$ samples each, there are two possible strategies to pick triplets among the \textbf{valid} ones to be used to compute the loss of the model:
\begin{itemize}
	\item \textbf{Batch All}: in this case all the valid triplets are selected and the resulting loss is averaged only on the \textbf{hard} and \textbf{semi-hard} triplets. It is crucial in this case to not take into account the \textbf{easy} triplets (those with loss $0$), as averaging on them would make the overall loss very small. This strategy therefore produces a total of $FK(K-1)(FK-K)$ triplets with $FK$ anchors, $K-1$ possible positives per anchor and $FK-K$ possible negatives.
	
	\item \textbf{Batch Hard}: in this case, on the other hand, only the hardest positive (with the biggest distance $d(a,p)$) and the hardest negative (with the smallest distance $d(a,n)$) among the batch are selected. This strategy thus produces $FK$ triplets which are the hardest among the batch. Some papers, such as \cite{hermansIDOTLFPRI}, affirm that the \textbf{batch hard} strategy generally yields the best performance among the two, however this really depends on the dataset and task it is used for. In any case this strategy is generally less stable than the other and renders the model harder to optimize.
\end{itemize}

Both strategies were implemented and used in different experiments to train the new \textit{siamese} model. However, only the results of the \textbf{Batch All} strategy, which led to the best validation performance, are reported in chapter \ref{chap:results}.

\subsection{Implementation}
\begin{figure}[H]
	\centering
	\includegraphics[width=\textwidth]{./images/Contrastive_Model.png}
...
The \textbf{Contrastive Learning Model}, as explained in the previous section and as it is shown in figure \ref{fig:ContrastiveLearningModel}, theoretically consists of a \textit{Siamese} network obtained by duplicating the \textit{Shared Model Base} and \textit{PE Embedding} layer of the \textbf{MTJE} model in order to effectively have one model per triplet element. Being a \textit{Siamese} network, the inner weights of the parallel models are shared during all phases of the training procedure (forward pass, gradients computation and back-propagation) and are jointly optimized in order to minimize the \textbf{Triplet loss}. Following this interpretation, each of these parallel models gets as input one sample - an anchor, a positive and a negative, respectively - and outputs the corresponding \textbf{PE Embedding} (of size 32). The \textit{Triplet loss} is then computed on those 3 embeddings and used for optimizing the model - following the selected triplet mining strategy - only if the current triplet is \textbf{valid}.

\begin{figure}[H]
	\centering
	\includegraphics[width=\textwidth]{./images/Contrastive_Model_Online.png}
...
However, as shown in figure \ref{fig:ContrastiveLearningOnlineModel}, it is much more convenient to consider the \textit{siamese} model as a single model which takes as input, at different times, the elements of a triplet - an anchor, a positive and a negative - and outputs the corresponding \textbf{PE Embeddings} (of size 32). The \textbf{Triplet loss} is then computed on those 3 embeddings and it is used to optimize the (single) model parameters. In practice, for every batch of data from the \textit{Fresh Dataset} each sample is passed through the model to obtain the corresponding \textbf{PE Embedding}, then all the possible combinations of 3 embeddings that produce \textbf{valid} triplets (or only the hardest ones, depending on the selected triplet mining strategy) are generated and used as input to the \textbf{Triplet loss} function.

Overall, the model is therefore simply composed of a \textbf{Shared Model Base} topology and a \textbf{PE embedding} layer situated on top of it, which are identical to the ones used in the previously defined \textbf{MTJE} model. This is done in order to be able to apply \textit{Transfer Learning} by importing the model parameters from a previous \textbf{MTJE} model training run on the Sorel-20m dataset. These parameters can then be \textit{fine-tuned} (further optimized) by minimizing the \textbf{Triplet loss} function using samples from the training subset of the \textit{Fresh Dataset}, thus generating a better implicit representation of PE files (PE Embedding) belonging to the same domain as those in the test subset of the \textit{Fresh Dataset}.

After training, the learned Embeddings of PE files can be evaluated on the malware family prediction/classification and ranking tasks in a similar way as it was done when evaluating the \textbf{MTJE} model learned embeddings on the same tasks.

\subsubsection{Net Definition}
As presented by algorithm \ref{alg:contrastiveLearningModelNetInit} (\textbf{\textit{Contrastive Learning Model}, \textit{Init} function}), the \textit{Contrastive (Siamese) Model} is initialized by first defining the model \textit{shared base} topology (\textit{model\_base}) as a sequence of blocks composed each of a linear layer, a Normalization layer, an Activation function and dropout, exactly as previously done for the \textbf{MTJE} model (and \textbf{Family Classifier}) implementation. Next, the \textit{pe\_embedding} topology is defined as a single linear layer with output size equal to the \textit{embedding\_dimension} ($=32$), followed by a Normalization layer and an Activation function.

\begin{algorithm}[h!]
	\caption{Contrastive Learning Model Net class, Init function}\label{alg:contrastiveLearningModelNetInit}
	\begin{algorithmic}[1]
		\Class{Net}
...
The input data (numerical feature vectors of PE files) is forwarded through the model as shown in alg. \ref{alg:contrastiveLearningModelNetForward} (\textbf{\textit{Contrastive Learning Model}, \textit{Forward} function}). In particular, it first passes through the model base topology (\textit{model\_base}) to produce an intermediate representation \textit{base\_out} which is then forwarded to the \textit{pe\_embedding} subnet which outputs the corresponding PE Embedding (of size 32). At the beginning of the Contrastive Model training procedure, the mapping function between the features of the input samples and the produced PE embeddings corresponds, if transfer Learning was applied, to the one learned by the \textbf{MTJE} model using the Sorel-20m dataset.

\begin{algorithm}[h!]
	\caption{Contrastive Learning Model Net class, Forward function}\label{alg:contrastiveLearningModelNetForward}
	\begin{algorithmic}[1]
		\algrestore{bkbreak}
			\Function{Forward}{\textit{self}, \textit{data}}
				\State \textit{base\_out} $\gets$ \textit{self}.\textit{model\_base}(\textit{data})
...
\subsection{Experiments}
The \textit{Contrastive (Siamese)} model defined in the previous section was trained (and validated) for $25$ \textit{epochs} on the training and validation subsets of the \textit{Fresh Dataset} using different combinations of learning rates (\textit{LR}), momentum, weight decay, \textit{Online Triplet Mining} strategy (between \textbf{Batch Hard} and \textbf{Batch All}) and margin while keeping the same model shared base topology inherited by the previously described \textbf{MTJE} model implementation. Moreover, both the \textit{Adam} and \textit{SGD} optimizers were used in different runs. The model which performed best during validation was then evaluated using the test subset of the \textit{Fresh Dataset} first on the malware family prediction/classification task - using the \textbf{Distance Weighted k-NN} approach with different values of $k$ - producing the corresponding accuracy and multi-class classification \textit{micro} (global average), \textit{macro} (unweighted per-class average) and \textit{weighted} (per-class average, weighted by support) scores such as the \textit{Jaccard Similarity}, \textit{recall}, \textit{precision} and \textit{f1 score} along with the corresponding \textit{confusion matrixes}. The same model was then also evaluated, again using the test subset of the \textit{Fresh Dataset}, on the family ranking task producing the corresponding ranking \textit{MRR} and \textit{MAP} scores, along with some example rankings which produced the best and worst \textit{AP} (Average Precision) and \textit{RR} (Reciprocal Rank).

The final \textit{Contrastive (Siamese)} model was trained with a learning rate $LR = 0.01$, \textbf{Adam} optimizer, $weight\_decay = 0.1$, $batch\_size = 250$, no \textit{momentum}, using the \textbf{Batch All} \textit{Online Triplet Mining} strategy with $margin = 0.3$, $rank\_size = 100$, $knn\_k\_min = 1$, $knn\_k\_max = 11$ and using the squared euclidean norm as distance function.

The evaluation results of the \textit{Contrastive (Siamese)} model implementation in the malware family classification and ranking tasks are reported in chapter \ref{chap:results}.
...
\begin{figure}[H]
	...
	\includegraphics[width=\textwidth]{./images/workflow_contrastive_model.png}
	\caption[Contrastive Model Training and Evaluation workflow steps]{Contrastive Model Training and Evaluation Workflow steps}
	\label{fig:workflow_contr_train_eval}
\end{figure}

Algorithms \ref{alg:trainContrastiveLearningModelFunctionPt1}-\ref{alg:trainContrastiveLearningModelFunctionPt2} (\textbf{Contrastive Model Train function}) and \ref{alg:evaluateContrastiveLearningModelFunctionPt1}-\ref{alg:evaluateContrastiveLearningModelFunctionPt2} (\textbf{Contrastive Model Evaluate function}) show how the \textbf{\textit{Contrastive Model}} implementation gets trained and evaluated using samples of the \textit{Fresh Dataset}.
...
\begin{algorithm}[h!]
	\caption{Contrastive Learning Model Train function, part 1}\label{alg:trainContrastiveLearningModelFunctionPt1}
	\begin{algorithmic}[1]
			\Function{TrainNetwork}{\textit{fresh\_ds\_path}, \textit{checkpoint\_path}, \textit{batch\_size}, \textit{epochs}, \textit{train\_split\_proportion}, \textit{valid\_split\_proportion}, \textit{test\_split\_proportion}, \textit{workers}, ...}
			\State
			\State \textit{split\_proportions} $\gets$ [\textit{train\_split\_proportion},
...
\begin{algorithm}[h!]
	\caption{Contrastive Learning Model Train function, part 2}\label{alg:trainContrastiveLearningModelFunctionPt2}
	\begin{algorithmic}[1]
		\algrestore{bkbreak}
			\State \Comment Continued from previous page..
			\State
			\ForAll{\textit{epoch} $\in$ \textbf{range}(\textit{epochs})}
				...
				\ForAll{\textit{shas}, \textit{features}, \textit{labels} $\in$ \textit{train\_gen}}
					...
					\If{\textit{run\_params}['hard']}
						\State \textit{loss} $\gets$ \textbf{BatchHardTripletLoss}(\textit{labels}, \textit{pe\_embeddings},
						\State \hskip1.9em margin=\textit{run\_params}['margin'],
						\State \hskip1.9em squared=\textit{run\_params}['squared'])
					\Else
						\State \textit{loss}, \textit{pos\_fraction} $\gets$ \textbf{BatchAllTripletLoss}(\textit{labels}, \textit{pe\_embeddings},
						\State \hskip1.9em margin=\textit{run\_params}['margin'],
						\State \hskip1.9em squared=\textit{run\_params}['squared'])
...
In particular, as shown in alg. \ref{alg:trainContrastiveLearningModelFunctionPt1}-\ref{alg:trainContrastiveLearningModelFunctionPt2}, the \textit{Contrastive Model} training function first opens and loads the \textit{Fresh Dataset} splitting it into training, validation and test subsets following the selected proportions. Then, the function instantiates the \textit{Contrastive Model} itself by providing the necessary parameters to the \textit{Contrastive\_Net} class \textit{init} method. Next, if a checkpoint of a previous \textbf{MTJE} model training run is provided it is used to initialize the state parameters of the \textit{shared base} topology and \textit{PE embedding} layer of the newly instantiated \textbf{Contrastive Model}. Then, the learning rates for the parameters of the different parts of the model architecture are set such that those of the \textit{shared base} topology are just \textit{fine-tuned} while training the model (by using a smaller learning rate). Then, the selected optimizer (\textbf{Adam} or \textbf{SGD}) gets instantiated given the model parameters, selected learning rate, weight decay and momentum. At this point a scheduler is also instantiated with the purpose of modifying the learning rates for the model parameters - multiplying them by $0.1$ - after $3/4$ of the total number of epochs. After these preliminary steps, the function trains (and validates) the \textbf{Contrastive Model} for '\textit{epochs}' epochs. In particular, at each epoch the model is set into \textit{training} mode and then, for all the mini-batches of data randomly sampled from the \textit{Fresh Dataset} training generator, the features of the input samples are fed into the model to obtain the corresponding \textit{pe\_embeddings}. Next, according to the selected \textit{Online Triplet Mining} Strategy (\textbf{Batch Hard} or \textbf{Batch All}) the \textbf{\textit{Triplet Loss}} is computed using function \textit{BatchHardTripletLoss} or \textit{BatchAllTripletLoss}. The computed loss is then logged and used to compute the gradients which are back-propagated through the network. Moreover, in case the \textbf{Batch All} strategy is used, the function also computes and logs the fraction of positive triplets mined (where triplet loss $> 0$) '\textit{pos\_fraction}' which can be used as an additional indication of model convergence since in such strategy the loss tends to oscillate and never settle. Finally, the model is set into \textit{validation} mode and for each mini-batch of data from the \textit{Fresh Dataset} validation generator the function computes and logs the triplet loss (and fraction of positive triplets mined, if required) following the same \textit{Online Triplet Mining} Strategy.
...
\begin{algorithm}[h!]
	...
	\begin{algorithmic}[1]
		\Function{BatchHardTripletLoss}{\textit{labels}, \textit{embeddings}, \textit{margin}, \textit{squared}}
			...
			\State
			\State \textit{mask\_anchor\_positive} $\gets$ \textbf{GetAnchorPositiveTripletMask}(\textit{labels})
			...
			\State
			\State \textit{mask\_anchor\_negative} $\gets$ \textbf{GetAnchorNegativeTripletMask}(\textit{labels})	
...
\vspace{-0.2cm}
Algorithm \ref{alg:batchHardTripletLossFunction} (\textbf{BatchHardTripletLoss function}) describes how the \textit{Triplet loss} is computed when using the \textbf{Batch Hard} \textit{Online Triplet Mining} Strategy. In particular, the \textbf{BatchHardTripletLoss} function begins by computing a 2D matrix called \textit{pairwise\_dist}, which contains the euclidean distances between all possible pairs of sample embeddings, by using function \textit{PairwiseDistances}. Next, the function finds the \textbf{hardest positive} for each anchor by computing a 2D mask of valid pairs $(a, p)$ (such that $a \ne p$ and $a$ and $p$ have the same labels) using function \textit{GetAnchorPositiveTripletMask}, setting to $0$ any element of matrix \textit{pairwise\_dist} outside of the mask and then selecting the pair $(a, p)$ with maximum distance. The selected hardest positive surely forms a \textbf{valid} pair since all \textbf{invalid} pairs have their distance set to $0$ by the mask. The function then finds the \textbf{hardest negative} by computing a 2D mask of pairs $(a, n)$ using the function \textit{GetAnchorNegativeTripletMask} and selecting for each row the valid pair $(a, n)$ with the minimum distance. In this case, the invalid pairs are ignored in the selection of the hardest negative by getting the maximum distance for each row of \textit{pairwise\_dist} and then adding it to all invalid pairs $(a, n)$ of the same row. Finally, the function computes the actual \textit{Triplet loss} by averaging the result of formula \ref{eq:tripletLossFormula} applied using the \textit{hardest positive} and \textit{hardest negative} for each anchor sample.
...
The previously mentioned \textbf{GetAnchorPositiveTripletMask} function, which returns a 2D mask where $mask[a, p]$ is \textit{True} if and only if $a$ and $p$ are distinct and have the same label, is detailed in alg. \ref{alg:getAnchorPositiveTripletMaskFunction}. This function first produces a 2D mask called \textit{indices\_not\_equal} by selecting the non-identical samples (inverting an identity matrix), then it produces a 2D mask named \textit{labels\_equal} by selecting the samples with equal label and finally applies the logical \textit{and} function to obtain the final 2D mask called \textit{mask}.

The \textbf{GetAnchorNegativeTripletMask} function, on the other hand, returns a 2D mask where $mask[a, n]$ is \textit{True} if and only if $a$ and $n$ have distinct labels, as shown in alg. \ref{alg:getAnchorNegativeTripletMaskFunction}. In particular, it first produces a 2D mask called \textit{labels\_equal} by selecting the samples with equal label and then applies the logical \textit{not} function to obtain the final 2D mask named \textit{mask}.

The computation of the \textit{Triplet loss} using the \textbf{Batch All} \textit{Online Triplet Mining Strategy} is described by algorithm \ref{alg:batchAllTripletLossFunction} (\textbf{BatchAllTripletLoss function}). More specifically, the \textbf{BatchAllTripletLoss} function begins in a similar way as the \textbf{BatchHardTripletLoss} function by computing a 2D matrix called \textit{pairwise\_dist} containing the euclidean distances between all possible pairs of sample embeddings, using function \textit{PairwiseDistances}. It then creates a 3D tensor called \textit{triplet\_loss} of shape $(B, B, B)$ where the element at index $(i, j, k)$ contains the loss for triplet $(i, j, k)$. This 3D tensor thus contains the losses of all possible triplets. Next, the function generates a 3D mask named \textit{mask} of valid triplets with function \textit{GetTripletMask} which is used to set to $0$ the loss for all invalid triplets (where $label(a) \ne label(p)$ or $label(n) = label(a)$ or $a = p$) in the 3D matrix named \textit{triplet\_loss}. Finally, the function removes the negative losses (i.e. the easy triplets), calculates the fraction of positive triplets (where triplet loss $> 0$) with respect to the valid ones and computes the final \textbf{Triplet loss} by averaging over the positive triplets.

The \textbf{GetTripletMask} function, as described by algorithm \ref{alg:getTripletMaskFunction}, returns a 3D mask where $mask[a, p, n]$ is \textit{True} if and only if the triplet $(a, p, n)$ is \textit{valid}, where a triplet $(i, j, k)$ is considered valid if $i$, $j$ and $k$ are distinct and $labels[i] = labels[j]$ and $labels[i] \ne labels[k]$. More specifically, the function first creates 3 3D masks named \textit{i\_not\_equal\_j}, \textit{i\_not\_equal\_k} and \textit{j\_not\_equal\_k}, respectively, by extending a 2D mask called \textit{indices\_not\_equal} obtained by selecting the non-identical samples (inverting an identity matrix). These 3 masks are then combined using the logical \textit{and} function to create the 3D mask named \textit{distinct\_indices} which is used to select the triplets where samples $i$, $j$ and $k$ are distinct. The function then creates 2 more 3D masks called \textit{i\_equal\_j} and \textit{i\_equal\_k} by extending a 2D mask named \textit{label\_equal} obtained by selecting the samples with the same label. These 2 masks are then combined using the logical \textit{and} function after having inverted the \textit{i\_equal\_k} mask to produce the 3D mask called \textit{valid\_labels} which is used to select the triplets where $labels[i] = labels[j]$ and $labels[i] \ne labels[k]$. Finally, the function combines the \textit{distinct\_indices} and \textit{valid\_labels} masks using the logical \textit{and} function to obtain the final 3D mask called \textit{mask}.
...
\begin{algorithm}[h!]
	...
	\begin{algorithmic}[1]
		\Function{BatchAllTripletLoss}{\textit{labels}, \textit{embeddings}, \textit{margin}, \textit{squared}}
			...
			\State
			\State \textit{mask} $\gets$ \textbf{GetTripletMask}(\textit{labels})
			\State \textit{triplet\_loss} $\gets$ \textbf{torch.mul}(\textit{mask}, \textit{triplet\_loss})
			\State
			\State \textit{triplet\_loss} $\gets$ \textbf{torch.maximum}(\textit{triplet\_loss}, \textbf{torch.zeros\_like}(\textit{triplet\_loss}))
			\State
			\State \textit{valid\_triplets} $\gets$ \textbf{torch.gt}(\textit{triplet\_loss}, 1e-16)
...
\begin{figure}[H]
	...
	\caption[Contrastive Model Evaluation Workflow step]{Contrastive Model Evaluation Workflow step}
	\label{fig:workflow_contrastive_model_evaluate}
\end{figure}

The \textit{Contrastive Model} evaluation function, presented in algorithm \ref{alg:evaluateContrastiveLearningModelFunctionPt1}-\ref{alg:evaluateContrastiveLearningModelFunctionPt2}-\ref{alg:evaluateContrastiveLearningModelFunctionPt3} (\textbf{\textit{Contrastive Model}, \textit{Evaluate} function}), begins (as shown in alg. \ref{alg:evaluateContrastiveLearningModelFunctionPt1}) by opening and loading the \textit{Fresh Dataset} splitting it in training, validation and test subsets following the selected proportions, similarly to what was done in the training function. The actual \textit{Contrastive Model} then gets instantiated by providing the necessary arguments to its \textit{init} member function. Next, the checkpoint state parameters from a previous training run are loaded and the model is set into \textit{evaluation} mode.

\begin{algorithm}[h!]
	\caption{Contrastive Learning Model Evaluate function, part 1}\label{alg:evaluateContrastiveLearningModelFunctionPt1}
	\begin{algorithmic}[1]
		\Function{EvaluateNetwork}{\textit{fresh\_ds\_path}, \textit{checkpoint\_path}, \textit{batch\_size}, \textit{train\_split\_proportion}, \textit{valid\_split\_proportion}, \textit{test\_split\_proportion}, \textit{rank\_size}, \textit{knn\_k\_min}, \textit{knn\_k\_max}, \textit{workers}, ...}
			...
			\State \hskip1.9em num\_workers=\textit{workers}, ..)
			\State
			\State \textit{label\_to\_sig} $\gets$ \textit{test\_gen}.\textit{dataset}.\textit{label\_to\_sig}
			\State \textit{n\_families} $\gets$ \textit{test\_gen}.\textit{dataset}.\textit{n\_families}
			\State
			\State \textit{model} $\gets$ \textit{Contrastive\_Net}(
			\State \hskip1.9em feature\_dimension=2381,
			\State \hskip1.9em embedding\_dimension=32,
...
Then, as shown in alg. \ref{alg:evaluateContrastiveLearningModelFunctionPt2}, the function considers the samples from the test subset of the \textit{Fresh Dataset} as query samples for the computation of the corresponding ranking and family predictions. In practice, for all the mini-batches of data from the \textit{Fresh Dataset} test generator, the feature vectors of the input samples are fed into the model to obtain the corresponding \textit{PE Embedding} representations called \textit{query\_pe\_embeddings}. Next, the function cycles through all the mini-batches of data from the \textit{Fresh Dataset} train generator in order to obtain the rankings for the current test mini-batch considering the samples from the training subset of the \textit{Fresh Dataset} as anchors. More specifically, the function obtains the \textit{PE Embedding} representation of the training samples - \textit{anchor\_pe\_embeddings} - by forwarding the corresponding features through the (same) model and then computes the \textit{Euclidean} distances between the embeddings of the \textit{query} and \textit{anchor} samples. These distances are used to update a set of 3 tensors of fixed size (equal to the selected \textit{rank\_size}) called \textit{top\_shas}, \textit{top\_labels} and \textit{top\_distances}, respectively, which contain the shas, labels and distances (to the current query sample) of the top '\textit{rank\_size}' most similar samples, per query, ordered by similarity.

\begin{algorithm}[h!]
	\caption{Contrastive Learning Model Evaluate function, part 2}\label{alg:evaluateContrastiveLearningModelFunctionPt2}
	\begin{algorithmic}[1]
		\algrestore{bkbreak}
			\State \Comment Continued from previous page..
			\ForAll{\textit{query\_shas}, \textit{query\_features}, \textit{query\_labels} $\in$ \textit{test\_gen}}
				\State \textbf{torch.no\_grad}()
				\State \textit{query\_pe\_embeddings} $\gets$ \textit{model}(\textit{query\_features})
				...
				\ForAll{\textit{anchor\_shas}, \textit{anchor\_features}, \textit{anchor\_labels} $\in$ \textit{train\_generator}}
					...
					\State
					\State \textit{distances} $\gets$ \textbf{torch.cdist}(\textit{query\_pe\_embeddings}, \textit{anchor\_pe\_embeddings}, p=2.0)
					\State
					\If{\textit{top\_distances} \textbf{is} None}
						...
						\State
						\State \textit{top\_shas} $\gets$ \textit{anchor\_shas}[\textit{indices}[:\textit{rank\_size}]]
						\State \textit{top\_labels} $\gets$ \textit{anchor\_labels}[\textit{indices}[:\textit{rank\_size}]]
						\State \textit{top\_distances} $\gets$ \textit{top\_distances}[\textit{indices}[:\textit{rank\_size}]]
					\Else
						\State \textit{top\_shas} $\gets$ \textbf{np.concatenate}(\textit{top\_shas}, \textit{anchor\_shas})
						\State \textit{top\_labels} $\gets$ \textbf{torch.cat}(\textit{top\_labels}, \textit{anchor\_labels})
						\State \textit{top\_distances} $\gets$ \textbf{torch.cat}(\textit{top\_distances}, \textit{distances})
						\State
						\State \textit{indices} $\gets$ \textit{top\_distances}.\textbf{argsort}(dim=1)
						\State
						\State \textit{top\_shas} $\gets$ \textit{top\_shas}[\textit{indices}[:\textit{rank\_size}]]
						\State \textit{top\_labels} $\gets$ \textit{top\_labels}[\textit{indices}[:\textit{rank\_size}]]
						\State \textit{top\_distances} $\gets$ \textit{top\_distances}[\textit{indices}[:\textit{rank\_size}]]
					\EndIf
				\EndFor
				\State \Comment Continues on next page..
...
At the end of the inner cycle, as shown in algorithm \ref{alg:evaluateContrastiveLearningModelFunctionPt3}, these 3 tensors (\textit{top\_shas}, \textit{top\_labels} and \textit{top\_distances}) are used to generate the actual rankings for the query samples of the current test mini-batch. Thereafter, the function applies the \textbf{Distance Weighted k-NN} approach using the generated rankings to obtain the actual family predictions for the test samples of the current mini-batch. In particular, for each odd value of \textit{k} from \textit{knn\_k\_min} to \textit{knn\_k\_max}, the function gets the first \textit{k} labels from the current \textit{top\_labels} tensor, computes each nearest neighbour weight inverting the square of its distance to the query sample and assigns to the current query the family with the highest cumulative score. Finally, the function computes the ranking scores (\textbf{MAP and MRR}) and saves to file both the produced rankings and family predictions.
	
\begin{algorithm}[h!]
	\caption{Contrastive Learning Model Evaluate function, part 3}\label{alg:evaluateContrastiveLearningModelFunctionPt3}
	\begin{algorithmic}[1]
		\algrestore{bkbreak}
				\State \Comment Continued from previous page..
				\For{\textit{k}, \textit{s} $\in$ \textbf{enumerate}(\textit{query\_shas})}
					\State \textit{ranks}.\textbf{append}(\{
					\State \hskip1.9em 'query\_sha': \textit{s},
					\State \hskip1.9em 'ground\_truth\_label': \textit{query\_labels}[\textit{k}],
					\State \hskip1.9em 'ground\_truth\_family': \textbf{label\_to\_sig}(\textit{query\_labels}[\textit{k}]),
					\State \hskip1.9em 'rank\_shas': \textit{top\_shas}[\textit{k}],
					\State \hskip1.9em 'rank\_labels': \textit{top\_labels}[\textit{k}],
					\State \hskip1.9em 'rank\_families': [\textbf{label\_to\_sig}(\textit{lab}) $\forall$ \textit{lab} $\in$ \textit{top\_labels}[\textit{k}]]
					\State \})
				\EndFor
				\State
...
The \textit{Contrastive (Siamese) Model} evaluation accuracy and \textit{micro} (global average), \textit{macro} (unweighted per-family average) and \textit{weighted} (per-family average, weighted by support) scores, such as the \textit{Jaccard similarity score}, \textit{recall}, \textit{precision} and \textit{f1-score}, are computed for each odd value of $k$ and saved to file - together with the resulting confusion matrixes - by a distinct set of functions which are not presented in this document since they simply apply the respective mathematical formulas. Moreover, after $n\_runs$ another set of functions plots the trends of the previously computed multi-class classification scores as the number of nearest neighbours $k$ changes from \textit{knn\_k\_min} to \textit{knn\_k\_max}.

\chapter{Results}\label{chap:results}
This chapter presents the evaluation results of the different models implemented. In particular, in the first section (\ref{sec:malwareDetectionResults}) are depicted the results of the different base models (\textit{ALOHA}, \textit{Joint Embedding} and \textbf{MTJE} model) on the task of malware detection (Malicious/Benign label prediction). The evaluation results of the same base models on the task of malware description via SMART tags are presented in a separate section (\ref{sec:malwareDescriptionResults}). Then, another section (\ref{sec:FreshEvaluationResults}) presents the results of the base models on the family prediction and ranking tasks using samples from the Fresh Dataset. Sections (\ref{sec:FamilyClassificationResults}) and (\ref{sec:ContrastiveLearningResults}), on the other hand, present the evaluation results of the Family Classifier and of the Contrastive Model, respectively, on the family classification and ranking (when possible) tasks. Finally, section (\ref{sec:computationTime}) presents the computation time needed to complete one experiment using the computational power of the available instance along with the total computation time spent to complete all the experiments conducted for this project.

\section{Malware Detection results}\label{sec:malwareDetectionResults}
In this section the result of the different models concerning the \textbf{Malicious/Benign} label prediction task are presented. In particular for each model are shown the corresponding \textbf{AUC-ROC} (Area Under ROC Curve) score (table \ref{tab:malware_auc}) and the various binary cross entropy loss statistics (\textbf{TPR} (True Positive Rate), \textbf{Accuracy}, \textbf{Recall}, \textbf{Precision} and \textbf{F1 Score}) at different \textbf{FPRs} (False Positive Rates) (table \ref{tab:malware_results_at_fpr}). Moreover, for the \textbf{TPR} statistic, also the \textbf{MTJE} model Error and Standard Deviation Reductions with respect to the other models are presented, in percentage. Next, the \textbf{ROC} (Receiving Operating Characteristic) curves of the different models are shown in figures \ref{fig:malwareRocAlohaMB}, \ref{fig:malwareRocAloha} and \ref{fig:malwareRocMtje}.
...
\begin{itemize}
	...
	\item \textbf{MTJE} model: final proposed \textbf{MTJE} model implementation described in section \ref{sec:mtjeModelImplementation}.
\end{itemize}
...
\subsection{Comments}
\vspace{-0.25cm}
As shown in table \ref{tab:malware_auc}, the proposed \textbf{MTJE} model implementation provided a slightly better \textit{AUC-ROC} score than both the \textbf{ALOHA} model implementations when tested on the malware detection - Malicious/Benign label prediction - task. Moreover, table \ref{tab:malware_results_at_fpr} shows that the \textbf{MTJE} model implementation consistently provided better scores for all the binary cross entropy statistics used as performance measures than the other two models at different \textit{FPRs} (False Positive Rates). Most importantly, the \textbf{MTJE} model implementation performed better than the other models at $FPR = 1\%$, as summarized in table \ref{tab:malware_result_summary}. These results prove the effectiveness of using the \textit{harder} problem of creating a \textit{Joint Embedding} space where samples are close to their associated SMART tags as an additional task to improve the performance of the model at the malware detection task.

\section{Malware Description via SMART tags results}\label{sec:malwareDescriptionResults}
\vspace{-0.4cm}
In this section the evaluation results of the different models in the SMART tagging prediction task are presented. More specifically, table \ref{tab:tag_aucs} shows, for each predicted tag, the \textbf{AUC-ROC} scores relative to the different models. Table \ref{tab:tag_results_summary}, on the other hand, presents the per-tag binary cross entropy statistics at \textbf{FPR} $=1\%$. Finally, figures \ref{fig:allMeanRocAloha}, \ref{fig:allMeanRocJointEmbedding} and \ref{fig:allMeanRocMtje} present, for each model, a single graph showing the mean \textbf{ROC} curves of all predicted tags plus the Malware label (if available).
...
Here (table \ref{tab:meanPerSampleScores}) are presented the \textit{Mean per-sample scores}, more specifically the \textbf{Jaccard Similarity} and \textbf{Mean per-Sample Accuracy} scores, achieved by the various models at different \textbf{FPRs}.

\meanPerSampleScores

\vspace{-0.5cm}
\subsection{Comments}
As shown in table \ref{tab:tag_aucs}, the \textbf{MTJE} model implementation provided better \textit{AUC-ROC} scores than the \textbf{ALOHA} and \textbf{Joint Embedding} model implementations for most of the target SMART tags. More specifically, it performed better when dealing with the \textit{Adware}, \textit{Downloader}, \textit{Dropper}, \textit{File-infector}, \textit{Flooder}, \textit{Installer} and \textit{Packed} tags. The \textbf{Joint Embedding} model implementation, on the other hand provided a higher \textit{AUC-ROC} score than the other models for the \textit{Crypto-miner}, \textit{Flooder} (on par with the \textbf{MTJE} model implementation), \textit{Ransomware} and \textit{Spyware} tags. Finally, the \textbf{ALOHA} model implementation performed better than the other models only on the \textit{Worm} tag. It is interesting to notice, however, that the \textbf{MTJE} model implementation results for the tags on which it did not perform best, were always the second best and anyway comparable to the best results.

On top of that, table \ref{tab:tag_results_summary} shows that the \textbf{MTJE} model implementation provided the best scores for all the binary cross entropy statistics used as performance measures (at $FPR = 1\%$) when dealing with the \textit{Downloader}, \textit{Flooder}, \textit{Installer}, \textit{Packed} and \textit{Worm} tags. It also provided the best \textit{TPR}, \textit{Precision}, \textit{Recall} and \textit{F1 score} at $FPR = 1\%$ for the \textit{Adware} tag, while having the same \textit{accuracy} than the \textbf{Joint Embedding} model implementation but with a higher standard deviation. On the other hand, the \textbf{MTJE} model implementation provided worse scores for all binary cross entropy statistics than the \textbf{Joint Embedding} model implementation for the \textit{Crypto-miner}, \textit{File-infector} (for which it provided the same \textit{precision}), \textit{Ransomware} (for which it provided the same \textit{accuracy}) and \textit{Spyware} tags. Finally, the \textbf{MTJE} model implementation provided in general worse results than the \textbf{Joint Embedding} model implementation for the \textit{Dropper} tag but for the \textit{Accuracy} score, for which it provided the same mean value with a lower standard deviation. Again, it is interesting to notice that when the \textbf{MTJE} model did not provide the best results it was always the second best (and the scores were comparable).

Finally, table \ref{tab:meanPerSampleScores} shows that the \textbf{MTJE} model implementation provided, in general, better \textit{Jaccard Similarity} and \textit{Mean per-Sample Accuracy} than the \textbf{ALOHA} and \textbf{Joint Embedding} model implementations at different \textit{FPRs} (False Positive Rates). More importantly, it provided better results at $FPR = 1\%$ than the other two models.
...
\vspace{-0.5cm}
\section{Family Prediction and Ranking Results}\label{sec:FreshEvaluationResults}
\vspace{-0.5cm}
This section shows the performance evaluation results of the different models in the \textbf{Malware Family Prediction and Ranking} tasks. More specifically, figures \ref{fig:AccuracyTrends}, \ref{fig:AucRocMicroTrends} and \ref{fig:AucRocMacroTrends}, present, respectively, the (mean and standard deviation) accuracy and (\textit{micro} and \textit{macro} averaged) AUC ROC trends of the models obtained by varying the number of anchor samples used during the evaluation. Table \ref{tab:ScoresAtMaxAccuracy}, on the other hand shows the scores of the models - while figures \ref{fig:alohaMBConfMatrixMaxAcc}, \ref{fig:alohaConfMatrixMaxAcc}, \ref{fig:jointEmbeddingConfMatrixMaxAcc} and \ref{fig:mtjeConfMatrixMaxAcc} show the confusion matrixes corresponding to the best predictions - when using the number \textbf{k} of anchor samples which produced the overall best accuracy, resulting from the evaluation on the Malware Family Prediction task. Finally, table \ref{tab:MrrAndMapResults} shows the mean and standard deviation of the \textbf{MRR} (Mean Reciprocal Rank) and \textbf{MAP} (Mean Average Precision) scores corresponding to the produced rankings of the different model implementations.

In particular, the joint embedding representations of PE files (of size 32) learned by the \textit{Joint Embedding} and \textbf{MTJE} model implementations were compared also to the intermediate implicit latent representation \textbf{h} (of size 512) of the ALOHA and ALOHA (M/B only) model implementations. In fact, it is possible to compare the results of the different models (including both ALOHA model implementations) on the family prediction and ranking tasks, given that also the implicit latent representation \textbf{h} of the ALOHA-based implementations can be used to compute sample distances. However, when comparing the different scores also the difference in size of the chosen representations should be take into consideration. A smaller representation has, in fact, much higher value than a bigger one, provided it performs well enough, since it allows quicker comparisons between files consequently making the tasks of storing, indexing and querying large databases of malware more efficient.
...
Here are presented, for each model, 4 example rankings which produced the maximum and minimum AP (Average Precision) and RR (Reciprocal Rank) scores.
...
\subsection{Comments}
Figures \ref{fig:AccuracyTrends}, \ref{fig:AucRocMicroTrends} and \ref{fig:AucRocMacroTrends} and table \ref{tab:ScoresAtMaxAccuracy} show that the representation of PE files in the joint embedding space (of size 32) leaned by the \textbf{MTJE} model performed worse than the one learned by the \textbf{Joint Embedding} model (again of size 32) at the f-way malware family prediction task. Moreover, the learned embeddings of both the \textbf{MTJE} model and \textbf{Joint Embedding} model implementations performed worse than the implicit latent representations - outputted by the base shared topology - of the \textbf{ALOHA (M/B only)} and \textbf{ALOHA} models (of size 512) at the same task. Figures \ref{fig:alohaMBConfMatrixMaxAcc}, \ref{fig:alohaConfMatrixMaxAcc}, \ref{fig:jointEmbeddingConfMatrixMaxAcc} and \ref{fig:mtjeConfMatrixMaxAcc}, which show the confusion matrixes corresponding to the best predictions, reflect these results by showing a better confusion matrix for the \textbf{ALOHA} model implementation and worse confusion matrixes for both the \textbf{MTJE} model and \textbf{Joint Embedding} model implementations. Again, it is to be reminded that the representations of PE files in the joint embedding space learned by these last two models (\textbf{MTJE} model and \textbf{Joint Embedding}), of size $= 32$, are 16 times smaller than the implicit latent representations of the \textbf{ALOHA} and \textbf{ALOHA (M/B only)} models, of size $= 512$. Ideally, these smaller representations of PE files should encode the most important characteristics of input samples to be used for the \textit{f-way} family prediction task; however, they were not constructed for such purpose, but rather for assigning a set of SMART tags to each sample in order to describe its functionality. Furthermore, the fact that some families may share the same set of tags can further hinder the generation of a representation in the joint embedding space that can be used to distinguish between such families. Consequently, in this case, the bigger implicit latent representation provided by the \textbf{ALOHA} model, being more generic does a better job at encoding those characteristics resulting in higher scores on the \textit{f-way} family prediction task.

Table \ref{tab:MrrAndMapResults}, on the other hand, shows that the representations of PE files in the joint embedding space learned by the \textbf{MTJE} model and \textbf{Joint Embedding} model provide similar \textbf{MRR} (Mean Reciprocal Rank) and \textbf{MAP} (Mean Average Precision) scores in the Family Ranking task. Again, the \textbf{ALOHA} model implicit latent representation - outputted by the base shared topology - performs generally better in this task than the other models. Anyway, in this case the scores are comparable. In particular, the \textbf{MAP} scores of the different models are practically the same, while the \textbf{MRR} of the \textbf{ALOHA} model is slightly higher than the others. This is reflected by the example rankings shown in tables \ref{tab:maxApExampleRank}, \ref{tab:maxRrExampleRank}, \ref{tab:minApExampleRank} and \ref{tab:minRrExampleRank}.

These results underline the inadequacy of the representation of PE files in the joint embedding space learned by the \textbf{MTJE} model when used for the Malware Family Prediction and Ranking tasks, thus highlighting the need for an alternative solution specifically designed for the Malware Family Classification task.

\section{Family Classification Results}\label{sec:FamilyClassificationResults}
In this section the results of the different Family Classifier implementations are presented and compared. In particular, table \ref{tab:FamilyClassifierAccuracyResults} presents the mean and standard deviation of the classification accuracy obtained by evaluating the Family Classifiers built on top of the \textbf{Joint Embedding} and \textbf{MTJE} model implementations along with a Family Classifier model trained entirely from scratch with no Transfer Learning applied (\textbf{Family Classifier Only}). Table \ref{tab:FamilyClassifierScoresTable}, on the other hand, shows the \textbf{\textit{Micro}} averaged, \textbf{\textit{Macro}} averaged and \textbf{\textit{Weighted}} averaged multi-class classification scores resulting from the evaluation of the different Family Classifiers. Finally, figures \ref{fig:FamilyClassifierJointEmbeddingConfusionMatrix}, \ref{fig:FamilyClassifierMtjeConfusionMatrix} and \ref{fig:FamilyClassifierOnlyConfusionMatrix} show the resulting confusion matrixes of the different models.
...
\FamilyClassifierMtjeConfusionMatrix

\FamilyClassifierOnlyConfusionMatrix

\subsection{Comments}
As shown in table \ref{tab:FamilyClassifierAccuracyResults}, the \textbf{Family Classifier} built on top of the \textbf{MTJE} model provided a higher \textit{Accuracy} than both the \textbf{Family Classifier} built on top of the \textbf{Joint Embedding} model and the equally complex \textbf{Family Classifier} with no transfer learning applied.

Table \ref{tab:FamilyClassifierScoresTable} confirms the better performance of the \textbf{Family Classifier} built on top of the \textbf{MTJE} model in the \textit{Malware Family Classification} task by showing that it consistently provided the best \textit{Micro}, \textit{Macro} and \textit{Weighted} averaged scores for all multi-class classification statistics used as performance metrics. It is interesting to notice that the \textbf{Family Classifier} built on top of the \textbf{Joint Embedding} model was, instead, always the second best, while the \textbf{Family Classifier} with no transfer learning applied was always last. This proves the effectiveness of utilizing the technique of \textit{transfer learning} to reduce overfitting when training the classifier using the training subset of the relatively \textit{small} \textit{Fresh Dataset}.

Interestingly, looking at the confusion matrixes resulting from the evaluation of the different \textbf{Family Classifiers} on the test sub-split of the Fresh Dataset, shown in figures \ref{fig:FamilyClassifierJointEmbeddingConfusionMatrix}, \ref{fig:FamilyClassifierMtjeConfusionMatrix} and \ref{fig:FamilyClassifierOnlyConfusionMatrix}, it is possible to notice some similarities in the errors committed (some of which are even similar to the ones done when using the representation of PE files learned by the base model for the \textit{f-way} family prediction task). In fact, in all three cases these observations can be made:
\begin{itemize}
	...
	\item samples of class \textbf{\textit{Gozi}} are often misclassified as belonging to class \textbf{\textit{TrickBot}}
	...
	\item samples of class \textbf{\textit{RemcosRAT}} are often misclassified as belonging to class \textbf{\textit{FormBook}} or \textbf{\textit{AgentTesla}}
	
	\item samples of class \textbf{\textit{TrickBot}} are often misclassified as belonging to class \textbf{\textit{Heodo}}
	
	\item samples of class \textbf{\textit{GuLoader}} are often misclassified as belonging to class \textbf{\textit{Loki}}
	
	\item samples of class \textbf{\textit{AveMariaRAT}} are often misclassified as belonging to class \textbf{\textit{AgentTesla}}, \textbf{\textit{FormBook}} or \textbf{\textit{Loki}}
\end{itemize}
...
This section shows the performance evaluation results of the different Contrastive models at the \textbf{Malware Family Prediction/Classification and Ranking} tasks. More specifically, table \ref{tab:ContrLearnAccuracyResults} presents the mean and standard deviation of the family classification accuracy obtained by evaluating the Contrastive Models based on the \textbf{Joint Embedding} and \textbf{MTJE} model implementations along with a Contrastive Model with no \textit{Transfer Learning} applied (\textbf{Contrastive Model Only}) using the k-NN algorithm with the value of $k$ which provided the best accuracy. Figures \ref{fig:ContrLearnAccuracyTrends}, on the other hand, show the classification (mean and standard deviation) accuracy trends of the different models as the number of \textit{Nearest Neighbours} $k$ changes. Next, table \ref{tab:ContrLearnScoresTable} presents the \textbf{Micro}, \textbf{Macro} and \textbf{Weighted} averaged multi-class classification scores of the the same models obtained using the k-NN algorithm with the value of $k$ which provided the best accuracy. The corresponding Confusion Matrixes of the different Contrastive models are instead depicted in figures \ref{fig:ContrLearnJointEmbeddingConfusionMatrix}-\ref{fig:ContrLearnMtjeConfusionMatrix}-\ref{fig:ContrLearnOnlyConfusionMatrix}. Finally, table \ref{tab:ContrLearnMrrAndMapResults} shows the mean and standard deviation of the \textbf{MRR} and \textbf{MAP} scores corresponding to the produced rankings of the different Contrastive Models.

\ContrLearnAccuracyResultsTable

\ContrLearnAccuracyTrends

\ContrLearnScoresTable

\ContrLearnJointEmbeddingConfusionMatrix

\ContrLearnMtjeConfusionMatrix

\ContrLearnOnlyConfusionMatrix

\ContrLearnMrrAndMapResultsTable

\subsection{Example rankings}
Here are presented, for each Contrastive Learning model, 4 example rankings which produced the maximum and minimum AP (Average Precision) and RR (Reciprocal Rank) scores.

\ContrLearnMaxApExampleRankTable

\ContrLearnMaxRrExampleRankTable

\ContrLearnMinApExampleRankTable

\ContrLearnMinRrExampleRankTable

\subsection{Comments}
As shown in table \ref{tab:ContrLearnAccuracyResults}, by properly choosing the number of \textit{Nearest Neighbours} $k$ to consider when using the k-NN approach on the learned embedding space, the \textbf{Contrastive Model} based on the \textbf{MTJE} model provided a higher \textit{Accuracy} than both the \textbf{Contrastive Model} based on the \textbf{Joint Embedding} model implementation and the identical \textbf{Contrastive Model} with no \textit{transfer learning} applied (\textbf{Contrastive Model Only}). Moreover, table \ref{tab:ContrLearnScoresTable} shows that, with the same values of $k$ the \textbf{Contrastive Model} based on the \textbf{MTJE} model also performed better than the other contrastive models for all \textbf{micro}, \textbf{macro} and \textbf{weighted} averaged multi-class classification scores. This is reflected by the resulting \textbf{Confusion Matrixes}, shown in figures \ref{fig:ContrLearnJointEmbeddingConfusionMatrix}-\ref{fig:ContrLearnMtjeConfusionMatrix}-\ref{fig:ContrLearnOnlyConfusionMatrix}. The results of the \textbf{Contrastive Model} based on the \textbf{MTJE} model in the malware family classification task are not as good as those obtained with the \textbf{Family Classifier}, however the \textbf{Contrastive Model} learned implicit representation of PE files (PE embeddings) can be used not only to classify the samples but also to quickly query and/or rank samples based on the similarity between their (small) embeddings. Moreover, the \textbf{Contrastive Model} can learn \textit{good} embeddings for an indefinite number of families while the \textbf{Family Classifier} works only with a finite (and fixed) amount. The \textbf{Contrastive Model} is therefore more flexible than the \textbf{Family Classifier} at the cost of having a slightly worse performance in the family classification task.

Additionally, table \ref{tab:ContrLearnMrrAndMapResults} and the example rankings shown in \ref{tab:ContrLearnMaxApExampleRank}, \ref{tab:ContrLearnMaxRrExampleRank}, \ref{tab:ContrLearnMinApExampleRank} and \ref{tab:ContrLearnMinRrExampleRank}, highlight the better performance of the \textbf{Contrastive Model} based on the \textbf{MTJE} model with respect to the other contrastive models in the family ranking task.

It should be noted that the results just discussed are not directly comparable with those obtained when evaluating the \textbf{MTJE} model (and others) on the family prediction and ranking tasks given the different evaluation procedures employed. In fact, while evaluating the \textbf{MTJE} model on the same tasks the whole \textit{Fresh Dataset} was considered by randomly selecting some anchor and query samples out of it, and the evaluation was repeated $15$ times per different number of selected anchor samples. On the other hand, when evaluating the \textbf{Contrastive Model} the samples from the \textit{test} subset of the \textit{Fresh Dataset} were considered as queries and the samples from the \textit{training} subset as anchors. This was done to reflect a real world scenario where the samples on which the model is deployed appear temporally later than the samples the model was trained on. This means that the evaluation results of the learned representation of the \textbf{MTJE} model are more optimistic than those obtained with the \textbf{Contrastive Model}. This accentuates the fact that the implicit representations of PE files learned with the \textbf{Contrastive Model} performed much better with respect to those obtained with the \textbf{MTJE} model.

Finally, by looking at the \textbf{Confusion matrixes} resulting from the evaluation of the \textbf{Contrastive Model} on the family classification task it can be noticed how these are similar to those obtained with the \textbf{Family Classifier} model. In particular, the errors made by the models while classifying the samples are very similar. This seems to confirm the hypothesis discussed in section \ref{sec:resultsCommentsFamiliesStudy} as to why the models make so many mistakes.

\section{Computation Time}\label{sec:computationTime}
...
\begin{table}[H]
	...
	\begin{tabular}{|P{4,5cm}||P{2,6cm} P{2,6cm} P{2,6cm}|}
		...
		Contrastive Model Training & $\sim$ 32s & 3 & $\sim$ 1m 36s \\
		Contrastive Model Evaluation & $\sim$ 19m 7s & 3 & $\sim$ 57m 21s \\
		Contrastive Model Results Computation & $\sim$ 10s & 3 & $\sim$ 30s \\
		Base Model Mean Results Computation & - & 1 & $\sim$ 5m 24s \\
		Contrastive Model Mean Scores Computation & - & 1 & $\sim$ 7s \\
		\hline
		Complete Workflow & $\sim$ 3h 44m 24s & - & $\sim$ 11h 18m 43s \\
		\hline
	\end{tabular}
...
\begin{table}[H]
	...
	\begin{tabular}{|P{4,5cm} P{4,5cm}|}
		...
		\hline
		$\sim$ 90 & $\sim$ 1018h 4m 30s \\
		\hline
	\end{tabular}
	\caption[Overall Computation Time]{Approximated overall computation time required to complete all experiments conducted for this project.}
	\label{tab:OverallComputationTime}
\end{table}

As it can be seen in table \ref{tab:ComputationTimePerExperiment}, a base model (\textit{ALOHA (M/B only)} \textit{ALOHA}, \textit{Joint Embedding} or \textit{MTJE} model) training run took, on average, a little over 3 hours to complete when using generator \textit{alt3} and considering only the first $6M$ training samples of the Sorel20M dataset for $10$ epochs. The base model performance evaluation step and the computation of the corresponding results took each 5 minutes (per run). The evaluation of the base model on the \textit{Fresh Dataset} took instead approximately 8 minutes, on average, while the \textit{Family Classifier} training, evaluation and results computation steps required less than 1 minute each (per run). Moreover, the \textit{Contrastive Model} training, evaluation and results computation steps took, on average, approximately 30 seconds, 20 minutes and 10 seconds, respectively. A single workflow cycle thus took approximately 3 hours and 45 minutes to complete. By multiplying the computation time of each step by the number of runs executed for each experiment and then aggregating the results together considering also the 5 minutes and a half it takes, on average, to compute the mean results of the base model and the 7 seconds it takes to compute the \textit{Contrastive Model} mean results, it is possible to obtain the approximated overall time spent to complete one whole experiment. In particular, table \ref{tab:ComputationTimePerExperiment} shows that in order to complete one full experiment the \textbf{Colab Pro} instance approximately took, on average, 11 hours and 20 minutes.

The experiments conducted for this project were approximately 90, as reported in the table \ref{tab:OverallComputationTime}. Thereafter, the overall time spent for completing all the experiments was of approximately 1018 hours and 5 minutes, which corresponds to $\sim$ 42 days, 10 hours and 5 minutes of continuous computation.

\chapter{Conclusions}
The objective of this thesis was the creation of an automatic malware signature generation tool capable of generating (implicit or explicit) signatures of PE malicious files with high recall at low false positive rates, possibly capable of detecting not only known malware strains but also novel variants, while also being less susceptible to obfuscation attempts.

To achieve this goal an attempt was made by first designing the \textbf{MTJE} model, described in \ref{sec:mtjeModelImplementation}, which improved the results of the \textbf{ALOHA} model implementation \ref{sec:alohaModelImplementation} (from work \cite{ruddALOHA}) in the Malicious/Benign label prediction - also called Malware detection - task while also providing slightly better results than the \textbf{Joint Embedding} model implementation \ref{sec:jointEmbeddingModelImplementation} (from work \cite{DucauAMDATSE}) in the task of malware description via SMART tags. The learned representation of PE files in the Joint Embedding space, which is used to classify a sample as malicious or benign and to assign it the corresponding tags based on its similarity (or distance) to the tag labels in the same space, can in fact be thought as the \textbf{implicit} signature of the malicious samples seen during training. These signatures, however, have little interpretability. It is in fact difficult to understand how the model decides to map a file to a specific location in the Joint Embedding space, therefore assigning it a specific set of tags.

Moreover, when tested on the \textit{Family Prediction} and \textit{Family Ranking} tasks, these signatures did not performe as good as in the previous tasks. This, however, can be due to the relatively small size of the dataset (half of the Sorel-20M dataset) used during training and to the code approximations introduced in the dataset loading process to speed up the code. In fact, even the \textbf{Joint Embedding} model implementation - which provided good results, as stated in paper \cite{DucauAMDATSE}, when trained on a considerably larger number of samples (70M samples) and tested on samples much more similar to the training ones - did not perform well when trained with half of the Sorel-20M dataset (6M samples) and tested on the \textit{Family Prediction} and \textit{Family Ranking} tasks using the Fresh Dataset. Regarding the similarity between test and training samples it has to be noted that in paper \cite{DucauAMDATSE} the authors used the test dataset of the joint embedding model, which contains samples recorded only a few months later than those used for training, also for the f-way family classification task; the Fresh Dataset used for this project, on the other hand, consists of samples obtained in mid 2021 while the Sorel-20m dataset used for training contains samples dating back to 2018 - 2019.

Anyway, the introduction of a \textbf{Family Classifier} - specifically trained for the malware family classification task - on top of the \textbf{MTJE} model PE embedding layer provided more meaningful results, although not exceptional, in the family classification task while also demonstrating the potential of using Transfer learning in this context. This new model, however, could not be used to produce family rankings nor to query samples based on their similarity and was limited to working only with a fixed number of predefined families. The adoption of a \textbf{Contrastive (Siamese) Model} obtained by transferring the knowledge from a previous \textbf{MTJE} model training run on the Sorel-20m dataset (with \textit{transfer learning}) overcame such limitations by producing '\textit{good}' (and small) embeddings of PE executables which can be used to address both the family prediction/classification and ranking tasks with meaningful results with an indefinite number of families at the cost of having a slightly worse performance in the family classification task.

\section{Future Work}
The \textbf{MTJE} model, \textbf{Family Classifier} and \textbf{Contrastive Model} implementations have the potential to be very useful tools to the IT-Security field in the current scenario. Indeed, they can be used to detect, describe (via semantic attribute tags) and classify malicious software in an infected system. This is particularly useful for countering a malware infection on a system/network, defining a remediation procedure, identifying possible root causes, and evaluating the severity and potential consequences of the attack. Moreover, by providing a more general description of PE files, they should also be able to detect not only known malwares but also novel malware strains/variants, providing some degree of proactive detection.

The current implementation of the \textbf{MTJE} model provided very good results in the tasks of \textbf{Malware detection} and \textbf{Malware description via SMART tags}, considering the number of samples it was trained on. Moreover, the \textbf{Family Classifier} and the \textbf{Contrastive Model} performed relatively well on the \textbf{Malware Family Classification} and \textbf{Ranking} (when possible) tasks considering the small and low quality dataset (\textit{Fresh Dataset}) they were trained on. However, these models have also some limitations, such as the results in the \textbf{family classification} and \textbf{ranking} tasks which could be much better if a bigger and higher quality dataset was used during training, and the lack of interpretability of the resulting implicit signatures. Future works capable of overcoming these shortcomings may be extremely helpful to malware analysts, antivirus software developers and system administrators and could even enable the generation of explicit (and thus more interpretable) signatures derived from the learned implicit ones.

Here are listed some suggestions that can be taken into account for overcoming the first limitation:
\begin{itemize}
	\item Training the \textbf{MTJE} model on the entire Sorel20M dataset (and not just a sub-part of it) with no code optimizations/approximations on a more powerful instance - which can be done with little modifications to the code developed for this project - alone should provide improved results in both the Malware detection and description (via SMART tags) tasks. Moreover, this could also improve the performance of the resulting learned representation of PE Files on the Family prediction and ranking tasks.
	
	\item A higher-quality, larger and more recent dataset of PE samples with respect to the Sorel20M dataset, could further enhance the performance of the resulting model in all three tasks (especially for the family prediction and ranking tasks). Moreover, a dataset providing also the information regarding the family each sample belongs to could remove the need for a specially crafted \textit{Fresh Dataset}. This could be beneficial given that the \textit{Fresh Dataset} was built by downloading samples from Malware Bazaar, which is a website where analysts can share malware samples with the InfoSec community and as such may provide many misclassified samples.
	
	\item On the other hand, a higher-quality (and possibly larger) dataset of PE malicious files with the corresponding family label with respect to the current \textit{Fresh Dataset} could improve the performance of the \textbf{Family Classifier} and \textbf{Contrastive Model} implementations.
	
	\item Finally, a model designed following the idea of paper "\textbf{Learning from Context: Exploiting and Interpreting File Path Information for Better Malware Detection}" \cite{KyadigeLFCEIFPIBMD} - described in section \ref{sec:learningFromContext} - of exploiting additional orthogonal input features derived statically from the samples has the potential of providing a more informed representation of PE files which may result in better implicit signatures and improved results in all previously mentioned tasks. For example, additional information like the \textbf{File Path} where the malicious sample was seen on an endpoint host (as suggested in work \cite{KyadigeLFCEIFPIBMD}), or the image representation of malware samples byte code (as described in paper \cite{BensaoudCMICNNM}) could be forwarded through a series of convolutional layers to automatically extract the best latent representation of samples to be used in parallel with the currently developed shared base topology output. This, however, implies the need for a dataset providing such additional features.
\end{itemize}

On the other hand, the fact that the signatures generated with the proposed approach are implicit, and therefore not easily interpretable, can be considered as another limitation. In fact, in order to trust the decisions of the designed model malware analysts need to know how they are taken, what aspects of the input features are more relevant in the decision, why some samples are mapped close to each other in the latent space, etc. However, this is difficult - although not impossible - to do when working with Neural Networks. In order to overcome the lack of interpretability of the generated implicit signatures the following attempts could be made:
\begin{itemize}
