...
In this section the models' evaluation results for the SMART tagging prediction task are presented. More specifically, table \ref{tab:tag_aucs} shows, for each predicted tag, the \textbf{AUC-ROC} scores relative to the different models. Table \ref{tab:tag_results_summary}, on the other hand, presents the tag prediction per-tag binary cross entropy statistics at \textbf{FPR} $=1\%$. Finally, for each model it is presented a single graph showing the \textbf{ROC} curves of all predicted tags (figures \ref{fig:allMeanRocAloha}, \ref{fig:allMeanRocJointEmbedding} and \ref{fig:allMeanRocProposedModel}).
...
This section shows the different models' performance evaluations at the \textbf{Malware Family Prediction and Ranking} tasks. More specifically, a table shows the mean and standard deviation of the \textbf{MRR} (Mean Reciprocal Rank) and \textbf{MAP} (Mean Average Precision) of the produced rankings for the different model implementations (and original features). For the malware family prediction task, on the other hand, here are presented the models' (mean and std) accuracy trends by varying the number of used anchor samples during the evaluation and also the confusion matrixes corresponding to the best and worst overall predictions.

It is interesting to notice that even if only the \textit{Joint Embedding} and \textit{Proposed Model} implementations map samples to a specially crafted joint embedding space, also the implicit latent representation of the ALOHA model and the original features can be used to compute samples distances. It is therefore possible to compare the results of the different models (including ALOHA model implementation) on the family prediction and ranking tasks.
