...
\begin{algorithm}[h]
	...
	\begin{algorithmic}[1]
		...
		\Function{Init}{\textit{self}, \textit{use\_malware}, \textit{use\_counts}, \textit{use\_tags}, \textit{n\_tags}, \textit{feature\_dimension}, \textit{layer\_sizes}, \textit{dropout\_p}, \textit{activation\_function}}			
			...
			\State
			\State \textit{self}.\textit{tag\_head} $\gets$ \textbf{nn.Sequential}(\textbf{nn.Linear}(\textit{layer\_sizes}[-1], 64),
			\State \hskip14.3em \textbf{nn.ELU}(),
			\State \hskip14.3em \textbf{nn.Linear}(64, 64),
			\State \hskip14.3em \textbf{nn.ELU}(),
			\State \hskip14.3em \textbf{nn.Linear}(64, \textit{n\_tags}),
			\State \hskip14.3em \textbf{nn.Sigmoid}())
		\EndFunction
		\algstore{bkbreak}
	\end{algorithmic}
\end{algorithm}
...
\color{Green}
Insert Joint Embedding network initialization, forward and compute loss algorithms.
\color{Black}
...
\color{Green}
Insert ALOHA and Joint Embedding training and evaluation algorithms.
\color{Black}
...
\color{Green}
Insert Proposed Model network initialization, forward and compute loss algorithms.
\color{Black}
...
\color{Green}
Cite Proposed Model training and evaluation algorithms. Insert Proposed Model evaluate fresh algorithm.
\color{Black}
