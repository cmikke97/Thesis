...
Throughout the experiments conducted and reported in this document, the following two datasets were used in order to train the different models and evaluate their relative performances:
\begin{itemize}
...
\begin{figure}[h!]
	...
	\caption{Workflow steps using the Sorel20M dataset}
...
\begin{figure}[h!]
	...
	\caption{Workflow steps using the Fresh dataset}
...
\begin{itemize}
	\item a probability score based on their Euclidean distance to the most similar anchor sample of that family for the \textit{\textbf{soft}} variant
...
\begin{algorithm}[h]
	...
	\begin{algorithmic}[1]
		...
		\Function{Init}{\textit{self}, \textit{use\_malware}, \textit{use\_counts}, \textit{use\_tags}, \textit{n\_tags}, \textit{feature\_dimension}, \textit{layer\_sizes}, \textit{dropout\_p}, \textit{activation\_function}}
			...
			\State \textit{self}.\textit{count\_head} $\gets$ \textbf{nn.Linear}(\textit{layer\_sizes}[-1], 1)
...
\begin{figure}[h!]
	\centering
	\includegraphics[width=\textwidth]{./images/workflow_sorel.png}
	\caption{Workflow Model Training and Evaluation steps}
	\label{fig:workflow_train_eval_plot}
\end{figure}

Both the \textbf{\textit{ALOHA}} and the \textbf{\textit{Joint Embedding}} model implementations (and also the \textbf{\textit{Proposed Model}} that will be defined in next chapter) are trained and evaluated following the procedure depicted by algorithms \ref{alg:trainNetworkFunction} (\textbf{Train network function}) and \ref{alg:evaluateNetworkFunction} (\textbf{Evaluate network function}).

\subsection{Training}
\begin{figure}[h!]
	\centering
	\includegraphics[width=\textwidth]{./images/workflow_train.png}
	\caption{Workflow Model Training step}
...
\begin{figure}[h!]
	...
	\caption{Workflow Model Evaluation step}
...
\begin{figure}[h!]
	...
	\caption{Workflow Results Computation and Plotting step}
...
The \textbf{\textit{Proposed Model}} implementation also uses exactly the same \textbf{\textit{get\_embedding}} function as the one used in the \textbf{\textit{Joint Embedding}} model implementation, shown in alg. \ref{alg:jointEmbeddingNetGetEmbedding}. Again, this function is particularly useful when performing evaluations on the learnt embedding.
...
\begin{figure}[h!]
	\centering
	\includegraphics[width=\textwidth]{./images/workflow_fresh_dataset.png}
	\caption{Workflow Model Evaluation on Fresh Dataset steps}
	\label{fig:workflow_fresh_eval_all}
\end{figure}

\subsection{Model Evaluation on Malware Family Prediction/Ranking task}
\begin{figure}[h!]
	\centering
	\includegraphics[width=\textwidth]{./images/workflow_fresh_eval.png}
	\caption{Workflow Model Evaluation on Fresh Dataset step}
...
\begin{algorithm}[h!]
	...
	\begin{algorithmic}[1]
		...
		\State \textit{distances} $\gets$ \textbf{torch.cdist}(\textit{embeddings}, \textit{anchors}['embeddings'], p=2.0)
		\State \textit{similarity\_scores} $\gets$ \textbf{distance\_to\_similarity}(\textit{distances}, \textit{sim\_function\_a}, \textit{sim\_function})
		\State
		\State \textit{rv} $\gets$ \{\}
...
\begin{algorithm}[h!]
	...
	\begin{algorithmic}[1]
		...
		\State \hskip5.4em 'labels': \textit{labels}[:\textit{n\_queries}]\}
		\EndIf
		\State
		\State \textit{distances} $\gets$ \textbf{torch.cdist}(\textit{queries}['embeddings'], \textit{embeddings}, p=2.0)
		\State \textit{similarity\_scores} $\gets$ \textbf{distance\_to\_similarity}(\textit{distances}, \textit{sim\_function\_a}, \textit{sim\_function})
		\State
