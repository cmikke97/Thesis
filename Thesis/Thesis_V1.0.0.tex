...
The actual model evaluation on the \textit{\textbf{fresh dataset}} is computed by another function called \textit{\textbf{evaluate\_fresh}}. This function first randomly samples '\textit{\textbf{q}}' query samples among the ones in the $fresh\_dataset$. Then, it computes the similarity between each query sample with the other '$n-1$' samples present in the $fresh\_dataset$ ordering the latter by similarity. At this point, for each query sample there is the ranking of all the other $n-1$ samples sorted by similarity. Using those rankings the function finally calculates the \textbf{MRR} (\textit{Mean Reciprocal Rank}) and \textbf{MAP} (\textit{Mean Average Precision}) scores for the model learned representation.
...
\begin{figure}[h]
	...
	\caption{Joint Embedding model implementation}
...
The \textit{Tag Embedding} matrix $\textbf{E} \in \mathcal{R}$, on the other hand, which is the same as the original implementation \cite{DucauAMDATSE}, maps a each tag $t_n$ to the corresponding representation in the Joint Embedding space and is therefore used as second input to the \textit{Prediction Layer}.
...
\end{equation}
where $d(\textbf{a}, \textbf{b})$ is the Euclidean distance between points $\textbf{a}$ and $\textbf{b}$ as defined in eq. \ref{eq:EuclideanDistance} and $\alpha$ is a multiplicative factor that cab be arbitrarily set at network definition when using one of the inverted \textit{Euclidean distance} functions. In the final Joint Embedding model implementation $\alpha$ is set to $1.0$.
...
\section{Experiments}
Both the models were trained (and validated) each for 12 \textit{epochs} on the first $6M$ training and $1.153.846$ validation samples of the pre-processed Sorel20M dataset (loaded with generator \textit{\textbf{alt3}}) using different number of layers, number of nodes per layer (layer sizes), activation functions and dropout value for the model base topology, different optimizers (\textbf{Adam} or \textbf{SGD}), learning rates (LR), momentum, weight decay and loss weights. Moreover, in the case of the \textbf{Joint Embedding} model also different similarity score functions such as the cosine similarity and the inverse of the Euclidean distance were tested instead of the dot product. The resulting models were then evaluated on the first $1.846.154$ test samples of Sorel20M dataset looking for their tag and/or malicious/benign label (present only in the ALOHA model implementation) prediction \textit{TPR}, \textit{accuracy}, \textit{recall}, \textit{precision}, \textit{f1} score, \textit{jaccard similarity} and \textit{mean-per-sample-accuracy} at a FPR of 1\%, as well as the resulting tag and/or malicious/benign \textit{ROC} curves and \textit{AUC} scores.
...
On the other hand, the base topology of final \textbf{Joint Embedding} architecture consisted of 3 layers of sizes 512, 512, and 128 respectively, with dropout probability $p_d=5\%$ and \textbf{ELU} activation function. The overall model was trained with a learning rate $LR = 10^{-3}$, \textbf{Adam} optimizer, no momentum nor weight decay and loss weights set to 1.0 for the malicious/benign head, 0.1 for the count head and 1.0 for the tag head. Moreover, the final model uses the dot product as similarity score between tags' and samples' embeddings.
...
\begin{figure}[h]
	\centering
	\includegraphics[width=\textwidth]{./images/Proposed_Joint_Embedding.png}
	\caption{Proposed Joint Embedding model}
	\label{fig:Proposed_Joint_Embedding_model}
\end{figure}

\section{Experiments}

\chapter{Results}
\section{Malware label results}
\begin{table}[H]
	\centering
	\begin{tabular}{|p{3,2cm}||p{1,8cm}|p{1,8cm}|p{1,8cm}|p{1,8cm}|p{1,8cm}|}
		\hline
		Malware label & \multicolumn{5}{|c|}{FPR} \\
		\textbf{TPR} & $10^{-5}$ & $10^{-4}$ & $10^{-3}$ & $10^{-2}$ & $10^{-1}$ \\
		\hline
		\hline
...
\begin{table}[H]
	...
	\begin{tabular}{|p{3,2cm}||p{1,8cm}|p{1,8cm}|p{1,8cm}|p{1,8cm}|p{1,8cm}|}
		\hline
		Malware label & \multicolumn{5}{|c|}{FPR} \\
		\textbf{Accuracy} & $10^{-5}$ & $10^{-4}$ & $10^{-3}$ & $10^{-2}$ & $10^{-1}$ \\
		\hline
		\hline
		...
		\hline
		\hline
		Error Reduction wrt \newline ALOHA & 8.9\% & 2.0\% & 4.4\% & 0.0\% & 0.0\% \\
		Error Reduction wrt \newline Joint Embedding & - & - & - & - & - \\
		\hline
	\end{tabular}
\end{table}
...
\begin{table}[H]
	...
	\begin{tabular}{|p{3,2cm}||p{1,8cm}|p{1,8cm}|p{1,8cm}|p{1,8cm}|p{1,8cm}|}
		\hline
		Malware label & \multicolumn{5}{|c|}{FPR} \\
		\textbf{Recall} & $10^{-5}$ & $10^{-4}$ & $10^{-3}$ & $10^{-2}$ & $10^{-1}$ \\
		\hline
		\hline
		ALOHA & 0.478$\pm$0.000 & 0.746$\pm$0.000 & 0.885$\pm$0.000 & 0.955$\pm$0.000 & \textBF{0.987$\pm$0.000} \\
		Joint Embedding & - & - & - & - & - \\
...
\begin{table}[H]
	...
	\begin{tabular}{|p{3,2cm}||p{1,8cm}|p{1,8cm}|p{1,8cm}|p{1,8cm}|p{1,8cm}|}
		\hline
		Malware label & \multicolumn{5}{|c|}{FPR} \\
		\textbf{Precision} & $10^{-5}$ & $10^{-4}$ & $10^{-3}$ & $10^{-2}$ & $10^{-1}$ \\
		\hline
		\hline
		ALOHA & 1.000$\pm$0.000 & 1.000$\pm$0.000 & 0.998$\pm$0.000 & 0.984$\pm$0.000 & 0.862$\pm$0.000 \\
		Joint Embedding & - & - & - & - & - \\
		Proposed Model & $\pm$0.000 & $\pm$0.000 & $\pm$0.000 & $\pm$0.000 & $\pm$0.000 \\
		\hline
		\hline
		Error Reduction wrt \newline ALOHA & - & - & - & - & - \\
		Error Reduction wrt \newline Joint Embedding & - & - & - & - & - \\
...
\begin{table}[H]
	...
	\begin{tabular}{|p{3,2cm}||p{1,8cm}|p{1,8cm}|p{1,8cm}|p{1,8cm}|p{1,8cm}|}
		\hline
		Malware label & \multicolumn{5}{|c|}{FPR} \\
		\textbf{F1 Score} & $10^{-5}$ & $10^{-4}$ & $10^{-3}$ & $10^{-2}$ & $10^{-1}$ \\
		\hline
		\hline
		ALOHA & 0.646$\pm$0.000 & 0.855$\pm$0.000 & 0.938$\pm$0.000 & 0.969$\pm$0.000 & 0.920$\pm$0.000 \\
		Joint Embedding & - & - & - & - & - \\
		Proposed Model & $\pm$0.000 & $\pm$0.000 & $\pm$0.000 & $\pm$0.000 & $\pm$0.000 \\
		\hline
		\hline
...
\section{Per-tag results}
\subsection{Adware tag results}
\begin{table}[H]
	\centering
	\begin{tabular}{|p{3cm}||p{1,4cm}|p{1,4cm}|p{1,4cm}|p{1,4cm}|p{1,4cm}|p{1,4cm}|}
		\hline
		\multicolumn{7}{|c|}{Adware tag (at FPR $=1\%$)} \\
		\hline
		Model & TPR & Accuracy & Precision & Recall & F1 score & AUC \\
		\hline
		ALOHA & 0,667029 & 0,971413 & 0,802893 & 0,667029 & 0,728682 & \textbf{0,969776} \\
		Joint Embedding & \textbf{0,674237} & \textbf{0,971827} & \textbf{0,804589} & \textbf{0,674237} & \textbf{0,733668} & 0,964693 \\
		\hline
	\end{tabular}
\end{table}
...
\begin{table}[H]
	...
	\begin{tabular}{|p{3cm}||p{1,4cm}|p{1,4cm}|p{1,4cm}|p{1,4cm}|p{1,4cm}|p{1,4cm}|}
		\hline
		\multicolumn{7}{|c|}{Crypto-miner tag (at FPR $=1\%$)} \\
		\hline
		Model & TPR & Accuracy & Precision & Recall & F1 score & AUC \\
		\hline
		ALOHA & 0,880128 & 0,988483 & 0,552073 & 0,880128 & 0,678529 & 0,980590 \\
		Joint Embedding & \textbf{0,893202} & \textbf{0,988664} & \textbf{0,555730} & \textbf{0,893202} & \textbf{0,685166} & \textbf{0,986034} \\
		\hline
	\end{tabular}
\end{table}
...
\begin{table}[H]
	...
	\begin{tabular}{|p{3cm}||p{1,4cm}|p{1,4cm}|p{1,4cm}|p{1,4cm}|p{1,4cm}|p{1,4cm}|}
		\hline
		\multicolumn{7}{|c|}{Downloader tag (at FPR $=1\%$)} \\
		\hline
		Model & TPR & Accuracy & Precision & Recall & F1 score & AUC \\
		\hline
		ALOHA & \textbf{0,622224} & \textbf{0,959254} & \textbf{0,850217} & \textbf{0,622224} & \textbf{0,718569} & \textbf{0,974264} \\
...
\begin{table}[H]
	...
	\begin{tabular}{|p{3cm}||p{1,4cm}|p{1,4cm}|p{1,4cm}|p{1,4cm}|p{1,4cm}|p{1,4cm}|}
		\hline
		\multicolumn{7}{|c|}{Dropper tag (at FPR $=1\%$)} \\
		\hline
		Model & TPR & Accuracy & Precision & Recall & F1 score & AUC \\
		\hline
		ALOHA & 0,693257 & 0,952120 & 0,910273 & 0,693257 & 0,787080 & \textbf{0,972347} \\
		Joint Embedding & \textbf{0,731457} & \textbf{0,956997} & \textbf{0,914564} & \textbf{0,731457} & \textbf{0,812826} & 0,969605 \\
		\hline
	\end{tabular}
\end{table}
...
\begin{table}[H]
	...
	\begin{tabular}{|p{3cm}||p{1,4cm}|p{1,4cm}|p{1,4cm}|p{1,4cm}|p{1,4cm}|p{1,4cm}|}
		\hline
		\multicolumn{7}{|c|}{File-infector tag (at FPR $=1\%$)} \\
		\hline
		Model & TPR & Accuracy & Precision & Recall & F1 score & AUC \\
		\hline
		ALOHA & \textbf{0,863338} & \textbf{0,969537} & \textbf{0,943298} & \textbf{0,863338} & \textbf{0,901548} & \textbf{0,981291} \\
...
\begin{table}[H]
	...
	\begin{tabular}{|p{3cm}||p{1,4cm}|p{1,4cm}|p{1,4cm}|p{1,4cm}|p{1,4cm}|p{1,4cm}|}
		\hline
		\multicolumn{7}{|c|}{Flooder tag (at FPR $=1\%$)} \\
		\hline
		Model & TPR & Accuracy & Precision & Recall & F1 score & AUC \\
		\hline
		ALOHA & 0,891511 & 0,989828 & 0,139040 & 0,891511 & 0,240562 & \textbf{0,985615} \\
		Joint Embedding & \textbf{0,905915} & \textbf{0,989846} & \textbf{0,140876} & \textbf{0,905915} & \textbf{0,243834} & 0,982012 \\
		\hline
	\end{tabular}
\end{table}
...
\begin{table}[H]
	...
	\begin{tabular}{|p{3cm}||p{1,4cm}|p{1,4cm}|p{1,4cm}|p{1,4cm}|p{1,4cm}|p{1,4cm}|}
		\hline
		\multicolumn{7}{|c|}{Installer tag (at FPR $=1\%$)} \\
		\hline
		Model & TPR & Accuracy & Precision & Recall & F1 score & AUC \\
		\hline
		ALOHA & 0,729779 & 0,985375 & 0,569100 & 0,729779 & 0,639501 & \textbf{0,973206} \\
		Joint Embedding & \textbf{0,739438} & \textbf{0,985546} & \textbf{0,572321} & \textbf{0,739438} & \textbf{0,645234} & 0,969592 \\
		\hline
	\end{tabular}
\end{table}
...
\begin{table}[H]
	...
	\begin{tabular}{|p{3cm}||p{1,4cm}|p{1,4cm}|p{1,4cm}|p{1,4cm}|p{1,4cm}|p{1,4cm}|}
		\hline
		\multicolumn{7}{|c|}{Packed tag (at FPR $=1\%$)} \\
		\hline
		Model & TPR & Accuracy & Precision & Recall & F1 score & AUC \\
		\hline
		ALOHA & 0,695981 & 0,949865 & 0,916687 & 0,695981 & 0,791231 & 0,978469 \\
		Joint Embedding & \textbf{0,773711} & \textbf{0,960476} & \textbf{0,924429} & \textbf{0,773711} & \textbf{0,842381} & \textbf{0,978678} \\
		\hline
	\end{tabular}
\end{table}
...
\begin{table}[H]
	...
	\begin{tabular}{|p{3cm}||p{1,4cm}|p{1,4cm}|p{1,4cm}|p{1,4cm}|p{1,4cm}|p{1,4cm}|}
		\hline
		\multicolumn{7}{|c|}{Ransomware tag (at FPR $=1\%$)} \\
		\hline
		Model & TPR & Accuracy & Precision & Recall & F1 score & AUC \\
		\hline
		ALOHA & 0,853121 & 0,982846 & 0,824796 & 0,853121 & 0,838719 & 0,972115 \\
		Joint Embedding & \textbf{0,856171} & \textbf{0,983003} & \textbf{0,825277} & \textbf{0,856171} & \textbf{0,840440} & \textbf{0,980650} \\
		\hline
	\end{tabular}
\end{table}
...
\begin{table}[H]
	...
	\begin{tabular}{|p{3cm}||p{1,4cm}|p{1,4cm}|p{1,4cm}|p{1,4cm}|p{1,4cm}|p{1,4cm}|}
		\hline
		\multicolumn{7}{|c|}{Spyware tag (at FPR $=1\%$)} \\
		\hline
		Model & TPR & Accuracy & Precision & Recall & F1 score & AUC \\
		\hline
		ALOHA & 0,612292 & 0,949644 & 0,879868 & 0,612292 & 0,722089 & 0,959670 \\
		Joint Embedding & \textbf{0,667107} & \textbf{0,955501} & \textbf{0,888646} & \textbf{0,667107} & \textbf{0,762103} & \textbf{0,965487} \\
		\hline
	\end{tabular}
\end{table}
...
\begin{table}[H]
	...
	\begin{tabular}{|p{3cm}||p{1,4cm}|p{1,4cm}|p{1,4cm}|p{1,4cm}|p{1,4cm}|p{1,4cm}|}
		\hline
		\multicolumn{7}{|c|}{Worm tag (at FPR $=1\%$)} \\
		\hline
		Model & TPR & Accuracy & Precision & Recall & F1 score & AUC \\
		\hline
		ALOHA & 0,579624 & 0,925008 & 0,916021 & 0,579624 & 0,709992 & \textbf{0,965917} \\
		Joint Embedding & \textbf{0,648632} & \textbf{0,935937} & \textbf{0,924279} & \textbf{0,648632} & \textbf{0,762302} & 0,951480 \\
		\hline
	\end{tabular}
\end{table}

\section{Mean per-sample scores}
\begin{table}[H]
	\centering
	\begin{tabular}{|p{4cm}||p{3cm}|p{3cm}|}
		\hline
		\multicolumn{3}{|c|}{Mean per sample scores (at FPR $=1\%$)} \\
		\hline
		Model & Jaccard Similarity & Mean-per-sample Accuracy \\
		\hline
		ALOHA & 0,864428 & 0,788886 \\
		Joint Embedding & \textbf{0,874371} & \textbf{0,799156} \\
		\hline
	\end{tabular}
\end{table}
