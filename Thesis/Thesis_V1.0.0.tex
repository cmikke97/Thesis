...
As explained by Ducau et Al. in \cite{DucauAMDATSE}, in order to counter and remediate a malware infection on a system/network it is of vital importance to understand the nature of the attack in progress. In fact, knowing the malicious capabilities associated with each suspicious file found on the system/network gives important clues to defenders that help them define and prioritize counter-measures. 
...
Furthermore, the learned low dimensional embedding space (and the associated embedding function) for malicious files enables the comparison between malware samples in terms of \textit{type} of malicious content. This in turns allows the introduction of a similarity index between malware files, allowing for efficient indexing, searching and querying of malware corpora along interpretable dimensions.
