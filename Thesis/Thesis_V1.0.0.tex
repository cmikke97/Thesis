...
Algorithm \ref{alg:batchHardTripletLossFunction} (\textbf{BatchHardTripletLoss function}) describes how the \textit{Triplet loss} is computed when using the \textbf{Batch Hard} \textit{Online Triplet Mining} Strategy. In particular, the \textbf{BatchHardTripletLoss} function starts by computing a 2D matrix of euclidean distances between all the embeddings called '\textit{pairwise\_dist}' by using function '\textit{PairwiseDistances}', which simply computes the pairwise euclidean distance between all the possible pairs of sample embeddings in a similar way as the \textit{Pytorch} '\textit{cdist}' function does. Next, the function finds the \textit{hardest positive} for each anchor by computing a 2D mask of the valid pairs $(a, p)$ (such that $a \ne p$ and $a$ and $p$ have the same labels) using function '\textit{GetAnchorPositiveTripletMask}' and, setting to $0$ any element of matrix '\textit{pairwise\_dist}' outside of the mask and then selecting the pair $(a, p)$ with maximum distance. The selected \textit{hardest positive} is surely \textbf{valid} since all \textbf{invalid} elements are set to $0$ in by the mask. The function then finds the \textit{hardest negative} by computing a 2D mask of pairs $(a, n)$,
