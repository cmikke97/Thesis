...
This section shows the performance evaluation of the different Contrastive models at the \textbf{Malware Family Prediction/Classification and Ranking} tasks. More specifically, table \ref{tab:ContrLearnAccuracyResults} presents the mean and standard deviation of the family classification accuracy obtained by evaluating the Contrastive Models based on the \textbf{Joint Embedding} and \textbf{MTJE} model implementations along with a Contrastive Model with no \textit{Transfer Learning} applied (\textbf{Contrastive Model Only}) using the k-NN algorithm with the value of '$k$' which provided the best accuracy. Figures \ref{fig:ContrLearnAccuracyTrends}, on the other hand, show the classification (mean and standard deviation) accuracy trends of the different models as the number of \textit{Nearest Neighbours} '$k$' changes. Next, table \ref{tab:ContrLearnScoresTable} presents the '\textbf{Micro}' averaged, '\textbf{Macro}' averaged and '\textbf{Weighted}' averaged multi-class classification scores of the the same models obtained using the k-NN algorithm with the value of '$k$' which provided the best accuracy. The corresponding Contrastive models' Confusion Matrixes are instead depicted in figures \ref{fig:ContrLearnJointEmbeddingConfusionMatrix}-\ref{fig:ContrLearnMtjeConfusionMatrix}-\ref{fig:ContrLearnOnlyConfusionMatrix}. Finally, table \ref{tab:ContrLearnMrrAndMapResults} shows the mean and standard deviation of the \textbf{MRR} and \textbf{MAP} scores corresponding to the produced rankings of the different Contrastive Models.
...
\subsection{Example rankings}
Here are presented, for each Contrastive Learning model, 4 examples rankings which produced the maximum and minimum AP (Average Precision) and RR (Reciprocal Rank) scores.
...
As it can be seen in table \ref{tab:ComputationTimePerExperiment}, a base model (\textit{ALOHA (M/B only)} \textit{ALOHA}, \textit{Joint Embedding} or \textbf{MTJE} model) implementation training run took, on average, a little over 3 hours to complete when using generator \textit{alt3} and considering only the first $6M$ training samples of the Sorel20M dataset for $10$ epochs. The base model performance evaluation step and the computation of the corresponding results took each 5 minutes (per run). The evaluation of the base model on the \textit{Fresh Dataset} took instead approximately 8 minutes, on average, while the \textit{Family Classifier} training, evaluation and results computation steps required less than 1 minute each (per run). Moreover, the \textit{Contrastive Model} training, evaluation and results computation steps took, on average, approximately 30 seconds, 20 minutes and 10 seconds, respectively. A single workflow cycle thus took approximately 3 hours and a 45 minutes to complete. By multiplying each step's computation time by the number of runs executed for each experiment and then aggregating the results together considering also the 5 minutes and a half it takes, on average, to compute the base model's mean results and the 7 seconds it takes to compute the \textit{Contrastive Model} mean results, it is possible to obtain the approximated overall time spent to complete one whole experiment. In particular, table \ref{tab:ComputationTimePerExperiment} shows that in order to complete one full experiment the \textbf{Colab Pro} instance approximately took, on average, 11 hours and 20 minutes.
