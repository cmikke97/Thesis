...
\textbf{\textit{Formbook}}, nowadays also called \textit{XLoader}, is an Info Stealer trojan that harvests user credentials from various web browsers and emails, collects screenshots, monitors and logs keystrokes, and can execute commands on the infected device - including rebooting or shutting down systems, downloading and executing files, and unpacking archive files - according to the orders received from Command-and-Control (C2) servers \cite{AnyRunFormBook}.

Its code, which is written in C, uses a number of evasion techniques to make it harder for researchers to analyse it.

\subsubsection{AgentTesla}
\textbf{\textit{AgentTesla}} is a .NET based spyware, keylogger and RAT (Remote Access Trojan) that collects information about the actions of its victims by recording keystrokes and user interactions. The collected information is then sent back to the Command and Control (C2) servers. Samples of this malware family often come equipped with multiple persistence mechanisms that help them avoid detection \cite{AnyRunAgentTesla}.

\subsubsection{Gozi}
\textbf{\textit{Gozi}}, also known as \textit{Ursnif}, is one of the most widely spread banking trojans in the world. As such, it is aimed at stealing banking credentials and usually targets corporate victims. It can silently collect the system activity of the victims, record keystrokes, and keep track of network traffic and browser activity. The malware is stealthy and stores the collected data in an archive before sending it to the C2 \cite{AnyRunGozi}.

\subsubsection{QuakBot}
\textbf{\textit{QuakBot}}, also known as \textit{Qakbot} and \textit{QBot}, is a modular information stealer/banking Trojan which main purpose is to steal banking credentials and other financial information from infected systems. It is also known to be a loader. Samples of this malware family are usually equipped with various sophisticated evasion techniques, info-stealing functions, worm-like functionality, and a strong persistence mechanism. In fact, its persistence mechanism is believed (by some researchers) to be one of the best in its class. Moreover, \textbf{\textit{QuakBot}} is polymorphic, has anti-VM, anti-debug and anti-sandbox functionally and therefore it is quite difficult to be analysed \cite{AnyRunQuakBot}. 

Nowadays, \textbf{\textit{QuakBot}} is frequently being dropped by other malware such as \textbf{\textit{Heodo}} (\textit{Emotet}). Otherwise it gets usually distributed via spam campaigns using context-aware spam.

\subsubsection{Loki}
\textbf{\textit{Loki}} Bot is an information stealer malware that collects credentials and other private data from from infected machines, and then submit that info to a command and control server. This private data includes stored passwords, login credential information from Web browsers, and a variety of cryptocurrency wallets. Samples belonging to this malware family employ several obfuscation techniques such as encryption in order to evade detection; moreover some of them also utilize function hashing to obfuscate the libraries they utilize (not all, but the majority of them) \cite{AnyRunLoki}.

\subsubsection{Heodo}
\textbf{\textit{Heodo}}, also known as \textit{Emotet}, is an extremely sophisticated and destructive Trojan used to download and install other malware. First versions of the \textbf{\textit{Heodo}} malware functioned as a banking trojan aimed at stealing banking credentials from infected hosts. Anyway, the authors soon updated the trojan and reconfigured it to work primarily as a loader. In particular, the second-stage payloads can be any type of executable code, from Emotet's own modules to malware developed by other cybercrime gangs. For example, \textbf{\textit{Heodo}} is known to download on infected systems the \textbf{\textit{TrickBot}} and \textbf{\textit{QuakBot}} banking trojans. \textbf{\textit{Heodo}} can act like a worm and spread using local networks, which makes it extremely hard to clean up. In addition to this, the Trojan is polymorphic and has advanced persistence and anti-evasion mechanics, such as anti-sandbox and anti-VM \cite{AnyRunHeodo}.

\subsubsection{RemcosRAT}
\textbf{\textit{RemcosRAT}} is a Remote Access Trojan (RAT) used to remotely control infected systems. This malware, upon installation, opens a backdoor on the computer, granting full access to the remote user. It can also capture screenshots, record keystrokes on infected machines, and send the collected information to host servers. Moreover, samples belonging to this malware family often use obfuscation techniques, such as encryption, to stay hidden from antivirus software \cite{AnyRunRemcosRAT}.

\subsubsection{TrickBot}
\textbf{\textit{Trickbot}} is an advanced banking trojan that attackers can use to steal payment credentials from the victims. It is typically delivered via a spam email containing a malicious document or malicious URL. Once this document has been opened, a macro gets executed which downloads the next stage of the infection process. In some cases, the second stage of this infection chain is a loader like Emotet which in turn drops Trickbot. Moreover, Trickbot has also been commonly observed to drop other malware families. Finally, Trickbot employs several obfuscation and evasion techniques \cite{AnyRunTrickBot}.

As previously stated, \textbf{\textit{TrickBot}} is frequently loaded on infected hosts by \textbf{\textit{Heodo}}. Therefore it is not that surprising to see the family classifier mistaking \textbf{\textit{TrickBot}} samples for samples belonging to the \textbf{\textit{Heodo}} family.

\subsubsection{GuLoader}
\textbf{\textit{GuLoader}} (also known as \textit{CloudEyE}) is a small downloader/dropper written in Visual Basic. It typically downloads RATs/Stealers, such as \textbf{\textit{AgentTesla}}, \textbf{\textit{Formbook}}, \textbf{\textit{Loki}}, \textbf{\textit{RemcosRAT}} and \textit{Netwire}, often but not always from Google Drive. \textbf{\textit{GuLoader}} gets constantly updated: the newer versions of this malware family exhibit sandbox evasion techniques, code randomization features, C\&C URL encryption, and additional payload encryption \cite{MalpediaGuLoader}.

\subsubsection{AveMariaRAT}
\textbf{\textit{AveMariaRAT}}, also called \textit{Warzone RAT}, is an is a Remote Access Trojan, info-stealer and keylogger. It is used to remotely control and steal information from infected systems. When researchers first discovered this Trojan, it was thought to be rather simplistic. However, it is actually is a modular RAT with an advanced design. It is capable of stealing a wide range of data from infected machines and has also advanced functions such as privilege escalation and remote camera control. Moreover, upon installation, it downloads additional plug-ins and sometimes even other malwares such as \textbf{\textit{Loki}} to the infected machine \cite{AnyRunAveMariaRAT}.
