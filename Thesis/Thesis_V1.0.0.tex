...
Ducau et al. stated in \cite{DucauAMDATSE} that 'the Joint Embedding network learns a more informative representation of binary files, guided by the ability to model, and therefore exploit tag relationships (labels structure) in the latent space'. The representations of malware binary files learnt during training can be taught as implicit signatures describing their capabilities.

Moreover, it is also possible to perform file similarity searches: using one malware sample other samples with similar malicious characteristics can be retrieved by simply getting the nearest files in the joint embedding space.
...
In particular, because of how the joint embedding model is designed and trained, two pieces of malware that are close to each other in latent space should exhibit similar malicious capabilities. This theoretically makes it possible to to measure how similar two pieces of malware are in terms of their capabilities by use their latent representations. Moreover, the joint embedding representation, given that its size (32 dimensions) is 32 times smaller than the original file feature representation (1024 dimensions), is particularly efficient for storing, indexing and querying large malware databases.

Additionally, given the fact that the model is indirectly trained with malware families' information generated by a set of different anti-malware vendors, Ducau et al. \cite{DucauAMDATSE} expects it to be able, to a certain extent, to differentiate malware campaigns from each other, even if they are described by the same set of tags.
