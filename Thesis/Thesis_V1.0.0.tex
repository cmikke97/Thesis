...
A new model was therefore introduced in order to solve this problem

\textit{\textbf{Siamese Network}}

trained in a contrastive learning setting using the \textit{\textbf{Online Triplet Loss Mining}} approach.

Triplet loss is a loss function frequently used in image recognition which was first introduced by the paper \cite{Schroff_FN} from Google for solving the face recognition task.

Triplet loss in this case will be used to learn good embeddings for each input PE file. In the embedding space, PE files from the same family should be close together and form well separated clusters.
