...
In the first implementation of the pre-processed data loading code the original \textit{Pytorch} Dataloader was used by passing it a specially crafted class called "\textit{Dataset}" (derived from the \textit{Pytorch} "torch.utils.data.Dataset" class) which is responsible for loading the pre-processed version of the dataset and for retrieving the data (features, labels and sha256) for each sample '\textit{i}'.
