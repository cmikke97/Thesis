...
\subsection{Experiments}
The resulting model was trained (and validated) for 25 \textit{epochs} on the training and validation sub-splits of the Fresh Dataset using different numbers of layers and numbers of nodes per layer (layer sizes) for the Family classifier additional output head while keeping the same model shared base topology inherited by the previously described \textbf{Proposed Model} implementation. Moreover, both the \textit{Adam} and \textit{SGD} optimizers were used in separate family classifier training runs with various combinations of learning rates (LR), momentum, weight decay and loss weights. The model which performed best during validation was then evaluated on the test sub-split of the Fresh Dataset producing the corresponding accuracy and multiclass classification '\textit{micro}' (global average), '\textit{macro}' (unweighted per-class average) and '\textit{weighted}' (per-class average, weighted by support) scores such as \textit{Jaccard Similarity}, \textit{recall}, \textit{precision}, \textit{f1} score, as well as the resulting '\textit{OVO}' (One Vs. One) and '\textit{OVR}' (One Vs. Rest) \textit{AUC-ROC} scores.
...
Overall, the model is therefore simply composed of a \textbf{Shared Model Base} topology and a \textbf{PE embedding} layer situated on top of it, which are identical to the ones used in the previously defined \textbf{Proposed Model}. This is done in order to be able to apply \textit{Transfer Learning} by importing the model parameters from a previous \textbf{Proposed Model} training run on the Sorel-20m dataset. These parameters can then be 'fine-tuned' (further optimized) by minimizing the \textbf{Triplet loss} function using samples from the training subset of the \textit{Fresh Dataset}, thus generating a better implicit representation of PE files (PE Embedding) belonging to the same domain as the ones in the \textit{Fresh Dataset}.

After training, the learned Embeddings of PE files can be evaluated on the malware family prediction/classification and ranking tasks in a similar way as it was done when evaluating the \textbf{Proposed Model} learned embeddings on the same tasks.

\subsubsection{Net Definition}
As presented by algorithm \ref{alg:contrastiveLearningModelNetInit} (\textbf{\textit{Contrastive Learning Model} Init function}), the \textit{Contrastive (Siamese) Model} is initialized by first defining the model \textit{shared base} topology (\textit{model\_base}) as a sequence of blocks composed each of a linear layer, a Normalization layer, an Activation function and dropout, exactly as previously done for the \textbf{\textit{Proposed Model}} (and \textbf{Family Classifier}) implementation. Next, the \textit{pe\_embedding} topology is defined as a single linear layer with output size equal to the \textit{embedding\_dimension} ($=32$), followed by a Normalization layer and an Activation function.
...
\ContrLearnProposedModelConfusionMatrix
