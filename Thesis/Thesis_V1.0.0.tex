...
\textbf{\textit{Triplet loss}} is a loss function frequently used in image recognition which was first introduced by the paper \cite{SchroffFN} from Google for solving the face recognition task.

\textit{Triplet loss} in this case will be used to learn good embeddings for each input PE file. In the embedding space, PE files from the same family should be close together and form well separated clusters.
...
At the same time, the loss function should also not push the train embeddings of each label to collapse into very small clusters during training.

The triplet loss function the is defined over triplets of embeddings:
\begin{itemize}
