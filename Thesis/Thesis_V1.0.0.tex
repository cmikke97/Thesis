...
\textbf{\textit{Triplet loss}} is a loss function frequently used in image recognition which was first introduced by the paper \cite{Schroff_FN} from Google for solving the face recognition task.
...
\begin{itemize}
	...
	In practice, first a list of triplets $(i, j, k)$ is produced, then a series of batches of these triplets of size $B$ is created. This means that during training it is needed to compute $3B$ embeddings in order to get the $B$ triplets, compute the loss of these $B$ triplets and then back-propagate it through the network.
