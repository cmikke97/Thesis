...
\begin{algorithm}
	\caption{Dataset class}\label{Dataset}
	\begin{algorithmic}[1] % uncomment [1] for line numbers
		\Class{FastTensorDataloader}
			\Function{Init}{self, ds\_root, mode, n\_samples, ...}
			...
			...
			\State
			\State \textit{self.S} $\gets$ \textbf{np.memmap}(\textit{S\_path}, \textit{dtype}=\textbf{np.dtype}('\textit{U64}'), \textit{mode="r+"})
			
			\State \textit{self.N} $\gets$ \textit{self.S}.\textbf{shape}[0]
			\State
			\State \textit{self.y} = \textbf{np.memmap}(\textit{y\_path}, \textit{dtype}=\textbf{np.float32}, \textit{mode="r+"}, \textit{shape}=(\textit{self.N}, \textit{labels\_dim}))
			\State
			\State \textit{self.X} $\gets$ \textbf{np.memmap}(\textit{X\_path}, \textit{dtype}=\textbf{np.float32}, \textit{mode="r+"}, \textit{shape}=(\textit{self.N}, \textit{ndim}))		
			\EndFunction
