...
\paragraph{Tags Prediction - Implementation Details}\label{par:tagPredictionImplementation}
...
The proposed model implementation (fig. \ref{fig:Proposed_Joint_Embedding_model}) consists of 3 main topologies: the \textit{Shared Model Base} sub-network, the \textit{Tag Embedding} and the multiple parallel \textit{Output Heads}.
...
Similarly to the \textit{ALOHA} model implementation (\ref{sec:alohaModelImplementation}), this model uses multiple parallel \textit{output heads}, each relative to a different task, whose values are jointly optimized during training by back-propagating the gradients computed from an aggregate loss. In particular, the tasks used in this case are: the \textit{Malicious/Benign label} prediction, the \textit{Vendor Count} estimation and the \textit{SMART Tag} prediction based on the similarity between samples' and tags' embeddings. The first two output heads (\textit{Malicious/Benign label} prediction head and \textit{Vendor Count} estimation head) are the same ones used in the \textit{ALOHA} model implementation (\ref{sec:alohaModelImplementation}).
...
\adwareTagResultsAucTable

\adwareTagResultsSummaryTable

\subsection{Crypto-miner tag results}
\cryptoMinerTagResultsAucTable

\cryptoMinerTagResultsSummaryTable

\subsection{Downloader tag results}
\downloaderTagResultsAucTable

\downloaderTagResultsSummaryTable

\subsection{Dropper tag results}
\dropperTagResultsAucTable

\dropperTagResultsSummaryTable

\subsection{File-infector tag results}
\fileInfectorTagResultsAucTable

\fileInfectorTagResultsSummaryTable

\subsection{Flooder tag results}
\flooderTagResultsAucTable

\flooderTagResultsSummaryTable

\subsection{Installer tag results}
\installerTagResultsAucTable

\installerTagResultsSummaryTable

\subsection{Packed tag results}
\packedTagResultsAucTable

\packedTagResultsSummaryTable

\subsection{Ransomware tag results}
\ransomwareTagResultsAucTable

\ransomwareTagResultsSummaryTable

\subsection{Spyware tag results}
\spywareTagResultsAucTable

\spywareTagResultsSummaryTable

\subsection{Worm tag results}
\wormTagResultsAucTable

\wormTagResultsSummaryTable

\section{Mean per-sample tagging scores}
\meanPerSampleScores

\section{Family Ranking Evaluation}
