...
The results show that the \textbf{Proposed Model} generates implicit signatures (samples embeddings) which provide comparable TPRs, Accuracies, Recalls, Precisions and F1 Scores at low false positive rates with respect to the ones produced by the mentioned previous methods on the corresponding tasks. When testing the \textbf{Proposed Model}'s learned representations on the Malware Family prediction and ranking tasks, however, the results were less promising. Therefore, a new Malware Family Classifier model was created and trained (and evaluated) on a specially crafted dataset of $5.000$ PE files, exploiting the parameters from a previous \textbf{Proposed Model} training run, with the aid of transfer learning.

...

Nevertheless, future (and more performant) works capable of overcoming some of the \textbf{Proposed Model} limitations may be very useful to the IT-Security field in the current scenario and could even enable the generation of explicit (and thus more interpretable) signatures derived from the learnt implicit ones.
...
Reverse engineers frequently rely on control-flow and call graphs to better understand a program's high-level logic. In particular a call graph represents calling relationships between subroutines (functions) in a computer program. Each node of a call graph represents a procedure and each edge (\textbf{f} -> \textbf{g}) indicates that function \textbf{f} eventually calls procedure \textbf{g}. By making the call graph harder to interpret, obfuscators can hinder the reverse engineers' capability of understanding the program behaviour \cite{DangPRE}. In order to do so, following two techniques can be used:
...
However, \textit{metamorphic} malware, in order to efficiently evolve its code it need to be able to recognise, parse and mutate its own body during propagation. This is far from being easy. Moreover, creating a true \textit{metamorphic} malware without arbitrarly increasing its code size is also challenging \cite{NamanyaTWM}.

\chapter{Detection Techniques}
Malware detection is the process of identifying malicious from benign code with the purpose of protecting systems from malwares and/or eventually recover from their nefastus effects \cite{NamanyaTWM}.

In order to counter malware attacks and threats, in recent years many anti-malware tools have been developed. Many of these are based on static features (such as signatures) with the assumption that most malware is static, as it doesn't mutate/change significantly at infection/replication time \cite{Sharma_2014}.
...
The last section of \textit{YARA rules}, which is the only one really required, contains the rule conditions that determine when the rule gets triggered. These conditions are Boolean expressions similar to those used in programming languages \cite{ArntzEYR}. Through the use of all the usual logical and relational operators, conditions can be made arbitrarly complex in order to accommodate for the author specific needs \cite{SimonWIY}.	
...
\begin{equation} \label{eq:featureNormalizationFunction}
	...
	\end{cases} \\= \begin{cases}
		\log{(1 + \textbf{x})} \frac{(1 + \frac{\textbf{x}}{|\textbf{x}|})}{2} - \log{(1-\textbf{x})} \frac{(1 - \frac{\textbf{x}}{|\textbf{x}|})}{2} &\text{if $\textbf{x} \ne 0$}\\
		x &\text{if $\textbf{x} = 0$}
	\end{cases}
\end{equation}
...
In order to evaluate the learned representation of PE files (embedding) of the proposed model on the sample family prediction task (fig. \ref{fig:workflow_fresh_dataset}), a further dataset, also referred to as \textit{\textbf{fresh\_dataset}} throughout this document (and in the code), was created. It consisted of a number of sample files' feature vectors along with the corresponding family labels and sha256 hashes.

More specifically, a number '\textit{\textbf{m}}' of PE malware families were selected from the list of the most prominent ones present in Italy at the time of writing (as reported by CERT-AGID summary \cite{CERTAGID}) of which Malware Bazaar provided at least '\textit{\textbf{x}}' sample files. Then, '\textit{\textbf{x}}' sample files per family were downloaded by Malware Bazaar \cite{MalwareBazaar} along with their metadata information (label, sha256 hash etc.) and the corresponding numerical feature vectors were extracted using the EMBER feature extraction and numerical feature generation codes \cite{AndersonEMBER} (and Sorel-20m normalization). Malware Bazaar is a malware sample database maintained by malware analysts which provides examples of malware executables and high-quality manually crafted classifications/descriptions for different malware families and it is therefore a good source for creating the new (fresh) dataset.
...
\end{itemize}
More specifically, the function downloads '\textit{\textbf{x}}' samples per malware family from Malware Bazaar (if possible, otherwise it skips the malware family and considers the next in order of importance between the ones selected), extracts the features from each sample and creates the new dataset containing '\textit{\textbf{n}} $=$ \textit{\textbf{x}} $\times$ \textit{\textbf{m}}' samples which are then stored on disk as (\textit{numpy}) memory mapped arrays (as it was done with the pre-processed Sorel20M dataset). Therefore, between the $\ge$ \textit{\textbf{m}} malware families provided to the function, only the first \textit{\textbf{m}} for which \textit{\textbf{x}} PE samples can be retrieved from Malware Bazaar are considered.

For all the model evaluations on the \textit{\textbf{fresh dataset}} done for this project the \textit{\textbf{fresh dataset}} was composed of \textit{\textbf{x}} $= 500$ samples per family, considering the following \textit{\textbf{m}} $= 10$ families: \textit{agenttesla}, \textit{formbook}, \textit{masslogger}, \textit{loki}, \textit{avemariarat}, \textit{njrat}, \textit{remcosrat}, \textit{netwire}, \textit{azorult} and \textit{asyncrat}. Therefore the \textit{\textbf{fresh dataset}} consisted of 5000 PE samples.
...
\begin{algorithm}[h!]
	...
	\EndFor
	\State \textit{S}, \textit{X}, \textit{y} $\gets$ \textit{ds}.\textbf{get\_as\_tensors}()
	\State \textit{S\_train}, \textit{S\_valid\_test}, \textit{X\_train}, \textit{X\_valid\_test}, \textit{y\_train}, \textit{y\_valid\_test} $\gets$ \textbf{train\_test\_split}(
	\State \hskip1.7em \textit{S}, \textit{X}, \textit{y}, test\_size=\textit{splits}[1] + \textit{splits}[2], stratify=\textit{y})
	\State \textit{S\_valid}, \textit{S\_test}, \textit{X\_valid}, \textit{X\_test}, \textit{y\_valid}, \textit{y\_test} $\gets$ \textbf{train\_test\_split}(
	\State \hskip1.7em \textit{S\_valid\_test}, \textit{X\_valid\_test}, \textit{y\_valid\_test}, test\_size=\textit{splits}[2], stratify=\textit{y\_valid\_test})
...
In order to refine the model learnt representation of PE files, the fresh dataset was split into training, validation and test subsets. In particular, the 5000 samples were divided following a 8-1-1 ratio: the training subset was created selecting the first 400 samples, per family, and therefore consisted of 4000 samples. The validation and test subsets, instead, consisted of 500 samples each (50 samples per family). The resulting dataset splits were then used to train (and evaluate) both a malware family classifier built on top of the proposed model (and joint embedding model, for comparison) learnt representation and a separate comparable malware family classifier trained from scatch. In particular, the training and validation subsets were used for training the classifier and validating its accuracy, while the test subset served to evaluate the classifier accuracy (and other scores) results on unseen samples. The resulting binary cross-entropy statistics, which included \textit{accuracy}, \textit{recall}, \textit{precision}, \textit{f1-score} at different \textit{FPRs} (False Positive Rates) as well as the per-family ROC (Receiving Operating Characteristic) curves and AUC-ROC (Area Under Curve) scores were then computed and plotted. 
...
\begin{algorithm}[h!]
	...
	\begin{algorithmic}[1]
		\Function{Evaluate\_Network}{\textit{ds\_path}, \textit{checkpoint\_file}, \textit{net\_type}, \textit{gen\_type}, \textit{batch\_size}, \textit{evaluate\_malware}, \textit{evaluate\_count}, \textit{evaluate\_tags}, \textit{feature\_dimension}, ...}
		\State \textit{Net}, \textit{Dataset}, \textit{get\_generator}, \textit{device}, \textit{run\_params} $\gets$ \textbf{import\_modules}(\textit{net\_type}, \textit{gen\_type})
		\State
		\State \textit{model} $\gets$ \textit{Net}(\textit{evaluate\_malware}, \textit{evaluate\_count}, \textit{evaluate\_tags},
		\State \hskip6.4em \textbf{len}(\textit{Dataset}.\textit{tags}), \textit{feature\_dimension}, \textit{run\_params}['layer\_sizes'],
		\State \hskip6.4em \textit{run\_params}['dropout\_p'], \textit{run\_params}['activation\_function'],
		\State \hskip6.4em \textit{run\_params}['normalization\_function'])
		\State
...
If the \textbf{Proposed Model} performs well enough on the malware family \textit{prediction} and \textit{ranking} tasks, it could be possible to use its learnt embedding space to define implicit signatures - the corresponding representation in this space, a.k.a. the embedding - of malware samples belonging to a specific malware family. These signatures could then be used to craft Yara Rules - with the use of a specially crafted custom Yara module that dynamically extracts the embedding representation for new PE files and compares them to the known signatures - to detect similar samples as belonging to the specific family represented by each rule. Yara Rules created in this way could potentially inherit the good properties of ML methods such as being able to detect not only known malware strains but also novel variants, being less susceptible to obfuscation attempts, etc. while being compatible with the highly expressive approach that is Yara.
...
\begin{algorithm}[h!]
	...
	\begin{algorithmic}[1]
		...
		\Function{Forward}{\textit{self}, \textit{data}}
			\State ...
			\State \textit{similarity\_scores} $\gets$ \textbf{torch.div}(\textbf{torch.add}(
			\State \hskip1.9em \textbf{Cosine\_similarity}(\textit{pe\_embedding}, \textit{tags\_embedding}, dim=2), 1.0), 2.0)
			\State \textit{rv}['similarity'] $\gets$ \textit{similarity\_scores}
			\State \textit{rv}['probability'] $\gets$ \textit{similarity\_scores}
			\State \textbf{return} \textit{rv}
		\EndFunction
...
The proposed model was trained multiple times for 10 \textit{epochs} on the first $6M$ training and $1.153.846$ validation samples of the pre-processed Sorel20M dataset (loaded with generator \textit{\textbf{alt3}}) with both the optional output heads (\textit{Malicious/Benign} label prediction and \textit{Vendor Count} prediction) on, changing the network hyper-parameters like the number of layers, layers dimensions (number of hidden units per layer), normalization funciton, activation function and dropout value for the base topology. Other hyper-parameters that were iteratively changed were the used optimizer (\textbf{Adam} or \textbf{SGD}), learning rate (LR), momentum, weight decay, loss weights and the similarity score function used (chosen between the \textit{dot product}, the \textit{cosine similarity} and one of the \textit{inverted Euclidean Distances}: \textit{exp}, \textit{inv} or \textit{inv\_pow}).
...
\begin{algorithm}[h!]
	...
	\begin{algorithmic}[1]
		\Function{Evaluate\_Fresh}{\textit{fresh\_ds\_path}, \textit{checkpoint\_path}, \textit{net\_type}, \textit{min\_n\_anchor\_samples}, \textit{max\_n\_anchor\_samples}, \textit{n\_query\_samples}, \textit{n\_evaluations}, \textit{batch\_size}}
		\State \textbf{evaluate\_fresh\_scores}(\textit{fresh\_ds\_path}, \textit{checkpoint\_path}, \textit{net\_type},
		...
		\State \textbf{evaluate\_fresh\_rankings}(\textit{fresh\_ds\_path}, \textit{checkpoint\_path}, \textit{net\_type}, \textit{n\_query\_samples},
		\State \hskip11.9em \textit{n\_evaluations}, \textit{batch\_size})
		\EndFunction
	\end{algorithmic}
\end{algorithm}

As previously mentioned, both the \textbf{\textit{Proposed Model}} and the \textbf{\textit{Joint Embedding}} model implementations are also evaluated on the \textit{f-way} malware family classification and family ranking tasks. In particular the function presented in algorithm \ref{alg:evaluateFreshFunction} (\textbf{Evaluate fresh function}) is used to produce both the metnioned evaluation results. More specifically, it calls the following two functions in succession: \textbf{\textit{evaluate\_fresh\_scores}} and \textbf{\textit{evaluate\_fresh\_rankings}}.
...
\begin{algorithm}[h!]
	...
	\begin{algorithmic}[1]
			\Function{Evaluate\_Fresh\_Scores}{\textit{ds\_path}, \textit{checkpoint\_path}, \textit{net\_type}, \textit{n\_queries}, \textit{min\_anchors}, \textit{max\_anchors}, \textit{n\_evaluations}, \textit{batch\_size}}
			...
			\ForAll{\textit{k}, \textit{n\_anchors} $\in$ \textbf{enumerate}(\textbf{range}(\textit{min\_anchors}, \textit{max\_anchors} $+ 1$))}
				...
				\ForAll{\textit{j} $\in$ \textbf{range}(\textit{n\_evaluations})}
					\State \textit{anchors} $\gets$ \textbf{get\_samples}(\textit{model}, \textit{generator}, \textit{n\_families}, \textit{n\_anchors})
					\State \textit{queries} $\gets$ \textbf{get\_samples}(\textit{model}, \textit{generator}, \textit{n\_families}, \textit{n\_queries}, other=\textit{anchors})
...
\begin{algorithm}[h!]
	...
	\begin{algorithmic}[1]
		\Function{Evaluate\_Fresh\_Rankings}{\textit{ds\_path}, \textit{checkpoint\_path}, \textit{net\_type}, \textit{n\_query\_samples}, \textit{n\_evaluations}, \textit{batch\_size}}
		\State \textit{Net}, \textit{run\_params} $\gets$ \textbf{import\_modules}(\textit{net\_type})
		\State
		\State \textit{model} $\gets$ \textit{Net}(use\_malware=False, use\_counts=False, n\_tags=\textbf{len}(\textit{Dataset}.\textit{tags}),
		\State \hskip6.3em feature\_dimension=2381, embedding\_dimension=32,
		...
		\ForAll{\textit{j} $\in$ \textbf{range}(\textit{n\_evaluations})}
		\State \textit{queries} $\gets$ \textbf{get\_samples}(\textit{model}, \textit{generator}, \textit{n\_families}, \textit{n\_query\_samples})
		...
		\EndFor
		\EndFor
		\State \textit{ranking\_scores}, \textit{global\_ranks\_to\_save}, \textit{rank\_per\_query} $\gets$ \textbf{compute\_ranking\_scores}(
...
\begin{algorithm}[h!]
	...
	\begin{algorithmic}[1]
		\Function{Compute\_Ranking\_Scores}{\textit{ranking\_scores}, \textit{global\_ranks\_to\_save}, \textit{rank\_per\_query}}
...
\begin{algorithm}[h!]
	...
	\begin{algorithmic}[1]
		\Function{Get\_Samples}{\textit{model}, \textit{generator}, \textit{n\_families}, \textit{n\_samples\_to\_get}, \textit{other}}
...
Given the \textbf{Proposed Model} relatively poor results in the Malware Family Classification task (which it was never trained for), the next step was the development of a \textbf{Malware Family Classifier} specifically trained (and tested) on the fresh dataset (which contains information about the malware family each sample belongs to) for such task. However, this dataset, which contains $5.000$ samples divided in $4.000$ training, $500$ validation and $500$ test samples, is arguably too small to produce a general enough model able to reliably predict the family label for each malware sample without overfitting on the training subsplit. This problem was therefore tackled with the aid of \textbf{transfer learning}.
...
\begin{algorithm}[h!]
	...
	\begin{algorithmic}[1]
		...
		\State \textit{fam\_class\_layers}.\textbf{append}(\textbf{nn.sigmoid}())
...
\begin{algorithm}[h!]
	...
	\begin{algorithmic}[1]
		...
		\Function{Forward}{\textit{self}, \textit{data}}
			\State \textit{base\_out} $\gets$ \textit{self}.\textit{model\_base}(\textit{data})
			\State \textit{pe\_embedding} $\gets$ \textit{self}.\textit{pe\_embedding}(\textit{base\_out})
			\State \textbf{return} \textit{self}.\textit{family\_classifier}(\textit{pe\_embedding})
		\EndFunction
		\algstore{bkbreak}
	\end{algorithmic}
\end{algorithm}
...
Algorithms \ref{alg:trainFamilyClassifierFunction} (\textbf{Family Classifier Train function}) and \ref{alg:evaluateFamilyClassifierFunction} (\textbf{Family Classifier Evaluate function}) show how the \textbf{\textit{Family Classifier}} model implementation gets trained and evaluated using samples from the Fresh Dataset. 
...
\begin{algorithm}[h!]
	\caption{Train Family Classifier function}\label{alg:trainFamilyClassifierFunction}
	\begin{algorithmic}[1]
			\Function{Train\_Network}{\textit{fresh\_ds\_path}, \textit{checkpoint\_path}, \textit{batch\_size}, \textit{epochs}, \textit{train\_split\_proportion}, \textit{valid\_split\_proportion}, \textit{test\_split\_proportion}, \textit{workers}, ...}
				\State \textit{dataset\_split\_proportions} $\gets$ \{'train': \textit{train\_split\_proportion},
				\State \hskip12.8em 'valid': \textit{valid\_split\_proportion},
				\State \hskip12.8em 'test': \textit{test\_split\_proportion}\}
				\State \textit{train\_generator}, \textit{valid\_generator}, \_ $\gets$ \textbf{get\_generator}(\textit{fresh\_ds\_path}, splits=[8, 1, 1],
				\State \hskip1.9em batch\_size=\textit{batch\_size}, return\_shas=True, num\_workers=\textit{workers}, ..)
				...
				\State \hskip1.9em feature\_dimension=2381, embedding\_dimension=32, layer\_sizes=\textit{run\_params}['layer\_sizes'], 
				...
				\State \hskip1.9em \textit{run\_params}['weight\_decay'], \textit{run\_params}['momentum'])
				\ForAll{\textit{epoch} $\in$ \textbf{range}(\textit{epochs})}
					\State \textit{model}.\textbf{train}()
					\ForAll{\textit{shas}, \textit{features}, \textit{labels} $\in$ \textit{train\_generator}}
						\State \textit{opt}.\textbf{zero\_grad}
						\State \textit{out} $\gets$ \textit{model}(\textit{features})
						\State \textit{loss} $\gets$ \textit{model}.\textbf{compute\_loss}(\textit{out}, \textit{labels})
						\State \textit{preds} $\gets$ [\textbf{argmax}(\textit{line} $\forall$ \textit{line} $\in$ \textit{out})]
					...
					\State
					\State \textit{model}.\textbf{eval}()
					\ForAll{\textit{shas}, \textit{features}, \textit{labels} $\in$ \textit{valid\_generator}}
					\State \textbf{torch.no\_grad}()
					\State \textit{out} $\gets$ \textit{model}(\textit{features})
					\State \textit{loss} $\gets$ \textit{model}.\textbf{compute\_loss}(\textit{out}, \textit{labels})
					\State \textit{preds} $\gets$ [\textbf{max}(\textit{line} $\forall$ \textit{line} $\in$ \textit{out})]
					\State accuracy $\gets$ \textbf{sum}(1 $\forall$ \textit{i}, \textit{pred} $\in$ \textbf{enumerate}(\textit{preds}) \textbf{if} \textit{pred} = \textit{labels}[\textit{i}]) / \textbf{len}(\textit{labels})
					\State \textbf{log}(\textit{loss})
					\State \textbf{log}(\textit{accuracy})
				\EndFor
...
\begin{algorithm}[h!]
	...
	\begin{algorithmic}[1]
			\Function{Evaluate\_Network}{\textit{fresh\_ds\_path}, \textit{checkpoint\_path}, \textit{batch\_size}, \textit{train\_split\_proportion}, \textit{valid\_split\_proportion}, \textit{test\_split\_proportion}, \textit{workers}, ...}
			\State \textit{dataset\_split\_proportions} $\gets$ \{'train': \textit{train\_split\_proportion},
			\State \hskip12.8em 'valid': \textit{valid\_split\_proportion},
			\State \hskip12.8em 'test': \textit{test\_split\_proportion}\}
			\State \_, \_, \textit{test\_generator} $\gets$ \textbf{get\_generator}(\textit{fresh\_ds\_path}, splits=[8, 1, 1], batch\_size=\textit{batch\_size},
			\State \hskip1.9em return\_shas=True, num\_workers=\textit{workers}, ..)
			\State \textit{model} $\gets$ \textit{Family\_Net}(families=[\textbf{label\_to\_sig}(\textit{lab}) $\forall$ \textit{lab} $\in$ \textbf{range}(\textit{n\_families})],
			\State \hskip1.9em feature\_dimension=2381, embedding\_dimension=32, layer\_sizes=\textit{run\_params}['layer\_sizes'],
			\State \hskip1.9em fam\_class\_layer\_sizes=\textit{run\_params}['fam\_class\_layer\_sizes'],
			\State \hskip1.9em dropout\_p=\textit{run\_params}['dropout\_p'],
...
The Family classifier evaluation \textbf{\textit{micro}} (global average), \textbf{\textit{macro}} (unweighted per-family average) and \textbf{\textit{weighted}} (per-family average, weighted by support) scores, such as '\textit{Jaccard similarity score}', '\textit{recall}', '\textit{precision}' and '\textit{f1-score}', are computed and saved to file, together with the resulting AUC-ROC scores and confusion matrix, by a distinct set of functions - which simply apply the respective mathematical formulas - not presented in this document.
