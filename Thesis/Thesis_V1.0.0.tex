...
This section shows the different models' performance evaluation results at the \textbf{Malware Family Prediction and Ranking} tasks. More specifically, table \ref{tab:MrrAndMapResults} shows the mean and standard deviation of the \textbf{MRR} (Mean Reciprocal Rank) and \textbf{MAP} (Mean Average Precision) scores corresponding to the produced rankings of the different model implementations. Figures \ref{fig:alohaMBAccuracyTrend}, \ref{fig:alohaAccuracyTrend}, \ref{fig:jointEmbeddingAccuracyTrend} and \ref{fig:proposedModelAccuracyTrend}, on the other hand, present the models' (mean and std) accuracy trends (varying the number of used anchor samples during the evaluation) and (figures \ref{fig:alohaMBConfMatrixMaxAcc}, \ref{fig:alohaConfMatrixMaxAcc}, \ref{fig:jointEmbeddingConfMatrixMaxAcc} and \ref{fig:proposedModelConfMatrixMaxAcc}) the confusion matrixes corresponding to the best predictions when using the number \textbf{k} of anchor samples which produced the overall best accuracy, resulting from the evaluation on the Malware Family Prediction task.

In particular, the \textit{Joint Embedding} and \textit{Proposed Model} implementations' learned joint embedding representations of PE files (of size 32) were compared also with the ALOHA and ALOHA (M/B only) model implementations' implicit latent representation (of size 512) after the based shared topology. Comparing the results of the different models (including both ALOHA model implementations) on the family prediction and ranking tasks, in fact, is possible given that also the implicit latent representations of the ALOHA-based implementations can be used to compute samples distances.

\MrrAndMapResultsTable

\alohaMBAccuracyTrend

\alohaAccuracyTrend

\jointEmbeddingAccuracyTrend

\proposedModelAccuracyTrend

\alohaMBConfMatrixMaxAcc

\alohaConfMatrixMaxAcc

\jointEmbeddingConfMatrixMaxAcc

\proposedModelConfMatrixMaxAcc
...
The current implementations of both models, however, have some limitations; namely the relatively poor results in the family classification task and the lack of interpretability of the resulting implicit signatures. By overcoming these shortcomings, future works may develop tools which would be very useful to the IT-Security field in the current scenario.


and could even enable the generation of explicit (and thus more interpretable) signatures derived from the learned implicit ones.
