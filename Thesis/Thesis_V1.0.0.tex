...
Anderson et al. \cite{AndersonEMBER} released in 2018 the first version of the Endgame Malware BEnchmark for Research (EMBER) dataset, extracted from a large corpus of Windows Portable Executable (PE) malicious and benign files. In particular, the samples were released together with the sha256 hash of the original file, and a label to denote whether the file is malicious or benign.

The authors of \cite{AndersonEMBER} also released open source code for extracting features from additional binaries so that additional sample features can be appended to the dataset.
