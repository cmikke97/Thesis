...
This section shows the different models' performance evaluation results at the \textbf{Malware Family Prediction and Ranking} tasks. More specifically, table \ref{tab:MrrAndMapResults} shows the mean and standard deviation of the \textbf{MRR} (Mean Reciprocal Rank) and \textbf{MAP} (Mean Average Precision) scores corresponding to the produced rankings of the different model implementations. Figures \ref{fig:AccuracyTrends}, \ref{fig:AucRocMicroTrends} and \ref{fig:AucRocMacroTrends}, on the other hand, present the models' (mean and standard deviation) accuracy and (\textit{micro} and \textit{macro} averaged) AUC ROC trends (varying the number of used anchor samples during the evaluation), respectively. Finally, figures \ref{fig:alohaMBConfMatrixMaxAcc}, \ref{fig:alohaConfMatrixMaxAcc}, \ref{fig:jointEmbeddingConfMatrixMaxAcc} and \ref{fig:proposedModelConfMatrixMaxAcc} show the confusion matrixes corresponding to the best predictions when using the number \textbf{k} of anchor samples which produced the overall best accuracy, resulting from the evaluation on the Malware Family Prediction task.
