		...
		\algrestore{bkbreak}
			\Function{Next}{\textit{self}}
				...
				\If{\textit{self.num\_workers} $= 1$}
					\State \textit{batch} $\gets$ \textbf{get\_batch}(\textit{tensors}, \textit{batch\_size}, \textit{i}, \textit{indices})
				...
				\Else
					...
					\While{\textit{self.i} $<$ \textit{self.dataset\_len} \textbf{and} \textbf{len}(\textit{self.async\_results}) $<$ \textit{self.num\_workers}}
						\State \textit{arguments} $\gets$ (\textit{tensors}, \textit{batch\_size}, \textit{i}, \textit{indices})
	...
	\begin{algorithmic}[1]
		\algrestore{bkbreak}
			\Function{Next}{\textit{self}}
				...
				\textcolor{codegreen}{\Comment{Until there is data in the dataset and there is space in the \textit{async\_results} list}}
					\While{\textit{self.i} $<$ \textit{self.dataset\_len} \textbf{and} \textbf{len}(\textit{self.async\_results}) $<$ \textit{self.num\_workers}}
						\State \textit{arguments} $\gets$ (\textit{tensors}, \textit{batch\_size}, \textit{i})
						\textcolor{codegreen}{\Comment{Set \textit{get\_batch} arguments}}
						\State \textit{async\_task} $\gets$ \textit{self.pool}.\textbf{apply\_async}(\textbf{get\_batch}, \textit{arguments})
						\textcolor{codegreen}{\Comment{Asynchronously call \textbf{get\_batch} function}}
						\State \textit{self.async\_results}.\textbf{append}(\textit{async\_task})
...
\begin{algorithm}
	...
	\begin{algorithmic}[1] % uncomment [1] for line numbers
		\Function{get\_batch}{\textit{chunk\_agg}, \textit{batch\_size}, \textit{i}, ...}
			\State $batch \gets [~]$
			\textcolor{codegreen}{\Comment{Get the data in order}}
			\ForAll{$t \in chunk\_agg$}
				\textcolor{codegreen}{\Comment{For all tensors in \textit{chunk\_agg}}}
				\State $batch\_data \gets t[i:(i + batch\_size)]$
				\textcolor{codegreen}{\Comment{Select data from the current tensor \textit{t}}}
				\State $batch.\textbf{append}(batch\_data)$
				\textcolor{codegreen}{\Comment{Append selected \textit{batch\_data} to \textit{batch} array}}
			\EndFor
			\State
			\State \textbf{return} $batch$
		\EndFunction
	\end{algorithmic}
\end{algorithm}
...
\begin{algorithm}
	...
	\begin{algorithmic}[1] % uncomment [1] for line numbers
		\Function{get\_chunks}{\textit{tensors}, \textit{chunk\_indices}, \textit{chunk\_size}, \textit{last\_chunk\_size}, \textit{n\_chunks}, \textit{shuffle}}
			\If{$n\_chunks - 1$ \textbf{in} $chunk\_indices$}
				\State $chunk\_agg\_size \gets (\textbf{len}(chunk\_indices) - 1) \cdot chunk\_size + last\_chunk\_size$
			\Else
				\State $chunk\_agg\_size \gets \textbf{len}(chunk\_indices) \cdot chunk\_size$
			\EndIf
			\State
			\State $chunk\_agg \gets []$
			\If {len(\textit{tensors}) = 3}
				\textcolor{codegreen}{\Comment{if \textit{tensors} contains also \textit{S} (shas) data}}
				\State $chunk\_agg.\textbf{append}(\textbf{emptyNumpyArray})$
				\ForAll{$t \in tensors[-2:]$}
					\State $chunk\_agg.\textbf{append}(\textbf{emptyTensor})$
				\EndFor
			\Else
				\ForAll{$t \in tensors$}
					\State $chunk\_agg.\textbf{append}(\textbf{emptyTensor})$
			...
			\EndIf
			\State
			\State $c\_start \gets 0$
			\ForAll{$idx \in [0, ..., \textbf{len}(chunk\_indices)]$}
				\State $t\_start \gets chunk\_indices[idx] \cdot chunk\_size$
				\If{$chunk\_indices[idx] \neq n\_chunks - 1$}
					\State $c\_end \gets c\_start + chunk\_size$
					\State $t\_end \gets t\_start + chunk\_size$
				\Else
					\State $c\_end \gets c\_start + last\_chunk\_size$
					\State $t\_end \gets t\_start + last\_chunk\_size$
				\EndIf
				
				\For{$i, t \in \textbf{enumerate}(tensors)$}
					\State $chunk\_agg[i][c\_start:c\_end] \gets t[t\_start:t\_end]$
				\EndFor
				\State $c\_start = c\_end$
			\EndFor
			\State
			\If{$shuffle = true$}
				\State $r = \textbf{randperm}(chunk\_agg\_size)$
				\For{$i, t \in \textbf{enumerate}(chunk\_agg)$}
					\State $chunk\_agg[i] = t[r]$
				\EndFor
			\EndIf
			\State
			\State \textbf{return} $chunk\_agg$, $chunk\_agg\_size$
		\EndFunction
	\end{algorithmic}
\end{algorithm}
...
\begin{algorithm}
	...
	\begin{algorithmic}[1] % uncomment [1] for line numbers
		\Class{FastTensorDataloader}
		\Function{Init}{\textit{self}, \textit{tensors}, \textit{batch\_size}, \textit{shuffle}, \textit{num\_workers}, ...}
		\State $self.tensors \gets tensors$
		\State $self.batch\_size \gets batch\_size$
		\State $self.shuffle \gets shuffle$
		\State $self.num\_workers \gets num\_workers$
		\State $self.dataset\_len \gets tensors[0].shape[0]$
		\textcolor{codegreen}{\Comment{Get dataset total length}}
		\If{$num\_workers > 1$}
		\State $self.async\_results \gets [~]$
		\textcolor{codegreen}{\Comment{Initialize async results list}}
		\State $self.pool \gets \textbf{ThreadPool}()$
		\textcolor{codegreen}{\Comment{Initialize Thread pool}}
		\EndIf
		\State $self.n\_batches \gets \lceil self.dataset\_len / self.batch\_size \rceil$
		\textcolor{codegreen}{\Comment{Compute number of batches}}
		\EndFunction
		\algstore{bkbreak}
	\end{algorithmic}
...
\begin{algorithm}[h]
	...
	\begin{algorithmic}[1]
		...
		\Function{Iter}{\textit{self}}
		\If{\textit{self.shuffle} = \textit{true}}
		\State $r \gets \textbf{randperm}(self.dataset\_len)$
		\textcolor{codegreen}{\Comment{Randomly create indices}}
		\For{$i, t \in \textbf{enumerate}(self.tensors)$}
		\State $self.tensors[i] \gets t[r]$
		\textcolor{codegreen}{\Comment{Permute (in-place) the data in tensors}}
		\EndFor
		\EndIf
		\State $self.i \gets 0$
...
\begin{algorithm}[h]
	...
	\begin{algorithmic}[1]
		\algrestore{bkbreak}
		\Function{Next}{\textit{self}}
		\If{$self.i \ge self.dataset\_len$}
		\textcolor{codegreen}{\Comment{If we are at the end of the dataset}}
		\If{($self.num\_workers = 1$ \textbf{or} $\textbf{len}(self.async\_results) = 0$)}
		\textcolor{codegreen}{\Comment{If just 1 worker was used or there are no async results to wait for in case of multiple workers}}
		\State \textbf{raise} $StopIteration$
		\textcolor{codegreen}{\Comment{Stop iteration}}
		\EndIf
		\EndIf
		
		\If{$self.num\_workers = 1$}
		\State $batch \gets \textbf{get\_batch}(tensors, batch\_size, i, indices)$
		\textcolor{codegreen}{\Comment{Get batch of data}}
		\State $self.i \gets self.i + self.batch\_size$
		\textcolor{codegreen}{\Comment{Update current index}}
		\State \textbf{return} \textit{batch}
		\Else
		\textcolor{codegreen}{\Comment{Until there is data in the dataset and there is space in the \textit{async\_results} list}}
		\While{$self.i < self.dataset\_len$ \textbf{and} $\textbf{len}(self.async\_results) < self.num\_workers$}
		\State $arguments \gets (tensors, batch\_size, i)$
		\textcolor{codegreen}{\Comment{Set \textit{get\_batch} arguments}}
		\State $async\_task \gets self.pool.\textbf{apply\_async}(\textbf{get\_batch}, arguments)$
		\textcolor{codegreen}{\Comment{Asynchronously call \textbf{get\_batch} function}}
		\State $self.async\_results.\textbf{append}(async\_task)$
		\textcolor{codegreen}{\Comment{Append async task to \textit{async\_results}}}
		\State $self.i \gets self.i + self.batch\_size$
		\textcolor{codegreen}{\Comment{Update current index}}
		\EndWhile
		\State $current\_result \gets self.async\_results.\textbf{pop}(0)$
		\textcolor{codegreen}{\Comment{Pop first element of \textit{async\_results}}}
		\State \textbf{return} $current\_result.\textbf{get}()$
		\textcolor{codegreen}{\Comment{Wait for task completion and return its value}}
		\EndIf
		\EndFunction
		\EndClass
