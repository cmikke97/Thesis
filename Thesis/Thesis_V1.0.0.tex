...
However, as shown in figure \ref{fig:ContrastiveLearningOnlineModel}, it is much more convenient to consider the \textit{siamese} model as a single model which takes as input, at different times, the elements of a triplet - an anchor, a positive and a negative - and outputs the corresponding PE \textbf{Embedding} (of size 32). The \textbf{Triplet loss} is then computed on those 3 embeddings and it is used to optimize the (single) model parameters. In practice, for every batch of data from the \textit{Fresh Dataset} each sample is passed through the model to obtain the corresponding PE \textbf{Embedding}, then all the possible combinations of 3 embeddings that produce \textbf{valid} triplets (or only the hardest ones, depending on the selected triplet mining strategy) are generated and used as input to the \textbf{Triplet loss} function.

Overall, the model is therefore simply composed of a \textbf{Shared Model Base} topology and a \textbf{PE embedding} layer situated on top of it, which are identical to the ones used in the previously defined \textbf{MTJE} model. This is done in order to be able to apply \textit{Transfer Learning} by importing the model parameters from a previous \textbf{MTJE} model training run on the Sorel-20m dataset. These parameters can then be 'fine-tuned' (further optimized) by minimizing the \textbf{Triplet loss} function using samples from the training subset of the \textit{Fresh Dataset}, thus generating a better implicit representation of PE files (PE Embedding) belonging to the same domain as the ones in the \textit{Fresh Dataset}.
...
The input data (PE files' numerical feature vectors) is forwarded through the model as shown in alg. \ref{alg:contrastiveLearningModelNetForward} (\textbf{\textit{Contrastive Learning Model} forward function}). In particular, it first passes through the model base topology (\textit{model\_base}) to produce an intermediate representation \textit{base\_out} which is then forwarded to the \textit{pe\_embedding} subnet which outputs the corresponding PE Embedding (of size 32). At the beginning of the Contrastive Model training procedure, the mapping function between the samples' input features and the produced PE embeddings corresponds, if transfer Learning was applied, to the one learned by the \textbf{MTJE} model using the Sorel-20m dataset.
...
\subsection{Experiments}
The \textit{Contrastive (Siamese)} model defined in the previous section was trained (and validated) for $25$ \textit{epochs} on the training and validation subsets of the \textit{Fresh Dataset} using different combinations of learning rates (\textit{LR}), momentum, weight decay, \textit{Online Triplet Mining} strategy (between \textbf{Batch Hard} and \textbf{Batch All}) and margin while keeping the same model shared base topology inherited by the previously described \textbf{MTJE} model implementation. Moreover, both the \textit{Adam} and \textit{SGD} optimizers were used in different runs. The model which performed best during validation was then evaluated using the test subset of the \textit{Fresh Dataset} first on the family prediction/classification - using the \textbf{Distance Weighted k-NN} approach with different values of $k$ - producing the corresponding accuracy and multi-class classification '\textit{micro}' (global average), '\textit{macro}' (unweighted per-class average) and '\textit{weighted}' (per-class average, weighted by support) scores such as the \textit{Jaccard Similarity}, \textit{recall}, \textit{precision} and '\textit{f1 score}' along with the corresponding \textit{confusion matrixes}. The same model was then also evaluated, again using the test subset of the \textit{Fresh Dataset}, on the family ranking task producing the corresponding ranking \textit{MRR} and \textit{MAP} scores, along with some example rankings which produced the best and worst \textit{AP} (Average Precision) and \textit{RR} (Reciprocal Rank).

The final \textit{Contrastive (Siamese)} model was trained with a learning rate $LR = 0.01$, \textbf{Adam} optimizer, $weight decay = 0.1$, $batch size = 250$, no \textit{momentum}, using the \textbf{Batch All} \textit{Online Triplet Mining} strategy with $margin = 0.3$, $rank size = 100$, $knn k min = 1$, $knn k max = 11$ and using the squared euclidean norm as distance function.
