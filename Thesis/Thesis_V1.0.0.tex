...
\begin{algorithm}[h]
	...
	\begin{algorithmic}[1]
		\algrestore{bkbreak}
			...
			\State \textit{loss\_dict} $\gets$ \{'total': 0.\}
			\State
			\State \textit{similarity\_loss} $\gets$ \textbf{Binary\_cross\_entropy\_with\_logits}(\textit{predictions}['similarity'],
...
\begin{algorithm}[h]
	\caption{Joint Embedding Net class, Compute Loss (with \textit{Cosine Similarity} or \textit{Inverted Pairwise Euclidean Distance})}\label{alg:jointEmbeddingNetComputeLossCosineOrDistance}
	\begin{algorithmic}[1]
		\algrestore{bkbreak}
			\Function{Compute\_loss}{\textit{predictions}, \textit{labels}, \textit{loss\_wts}}
			\State ...
			\State \textit{similarity\_loss} $\gets$ \textbf{Binary\_cross\_entropy}(\textit{predictions}['similarity'],
			\State \hskip7.9em \textit{labels}['tags'],
			\State \hskip7.9em reduction='none').\textbf{sum}(dim=1).\textbf{mean}(dim=0)
			\State
			\State \textit{loss\_dict}['jointEmbedding'] $\gets$ \textit{similarity\_loss}
...
A further alternative to the \textbf{Model forward function} is presented in alg. \ref{alg:distanceToSimilarityFunction} (\textbf{\textit{Joint Embedding} model forward function with \textit{Inverted Pairwise Euclidean Distance}})
...
\begin{algorithm}[h]
	...
	\begin{algorithmic}[1]
		...
		\State
		\ForAll{\textit{epoch} $\in$ \textbf{range}(1, \textit{epochs}$+1$)}
...
\begin{algorithm}[h]
	\caption{Proposed Model Net class, Get\_Embedding}\label{alg:ProposedModelNetGetEmbedding}
	\begin{algorithmic}[1]
		\algrestore{bkbreak}
		\Function{Get\_Embedding}{\textit{self}, \textit{data}}
			\State \textit{rv} $\gets$ \{\}
...
\begin{algorithm}[h]
	\caption{Proposed Model Net class, Compute Loss (with \textit{Cosine Similarity} or \textit{Inverted Pairwise Euclidean Distance})}\label{alg:proposedModelNetComputeLossCosineOrDistance}
	\begin{algorithmic}[1]
		\algrestore{bkbreak}
			\Function{Compute\_loss}{\textit{predictions}, \textit{labels}, \textit{loss\_wts}}
			\State ...
