...
The PE format structure consists of a number of standard headers followed by one or more sections (\ref{}) and it encapsulates the information necessary for the Windows OS loader to manage the wrapped executable code: for example it tells the dynamic linker how to map the file into memory. One of the headers included in this structure is the \textit{Common Object File Format (COFF)} file header which contains important information such as the type of machine for which the file is intended, the nature of the file (DLL, EXE, OBJ), the number of sections, the number of symbols, etc. Another header, called '\textit{optional header}', identifies the linker version, the size of the code, the size of initialized and uninitialized data, the address of the entry point, etc. Moreover, \textit{Data directories} within the optional header provide pointers to the sections that follow it, which include tables for exports, imports, resources, exceptions, debug information, certificate information, and relocation tables. The PE file format therefore provides a useful summary of the contents of an executable. Finally, the section table outlines the name, offset and size of each section in the PE file.

PE sections contain code and initialized data that the Windows loader maps into executable or readable/write-able memory pages, respectively, as well as imports, exports and resources defined by the file. Each section contains a header that specifies its size and address. An import address table, for example, instructs the loader on which functions to statically import. A resources section may contain resources such as required for user interfaces: cursors, fonts, bitmaps, icons, menus, etc. A basic PE file would normally contain a \textit{.text} code section (which holds program code) and one or more data sections (\textit{.data}, \textit{.text} or \textit{.bss}) holding global variables. Relocation tables are typically stored in a \textit{.reloc} section, used by the Windows loader to reassign a base address from the executable's preferred base. A \textit{.tls} section contains special thread local storage (TLS) structures for storing thread-specific local variables. Section names are arbitrary from the perspective of the Windows loader, but specific names have been adopted by precedent and are overwhelmingly common. Packers may create new sections, for example, the UPX packer creates \textit{UPX1} to house packed data and an empty section \textit{UPX0} that reserves an address range for runtime unpacking \cite{AndersonEMBER}.
