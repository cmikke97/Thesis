...
The previously mentioned \textbf{GetAnchorPositiveTripletMask} function, which returns a 2D mask where '$mask[a, p]$' is \textit{True} if and only if $a$ and $p$ are distinct and have the same label, is detailed in alg. \ref{alg:getAnchorPositiveTripletMaskFunction}. This function first produces a 2D mask selecting the non-identical samples (inverting an identity matrix) '\textit{indices\_not\_equal}', then it produces a 2D mask selecting the samples with equal label '\textit{labels\_equal}' and finally applies the logical \textit{and} function to obtain the final 2D mask '\textit{mask}'.

The \textbf{GetAnchorNegativeTripletMask} function, on the other hand, returns a 2D mask where '$mask[a, n]$' is \textit{True} if and only if $a$ and $n$ have distinct labels, as shown in alg. \ref{alg:getAnchorNegativeTripletMaskFunction}. In particular, it first produces a 2D mask selecting the samples with equal label '\textit{labels\_equal}' and then applies the logical \textit{not} function to obtain the final 2D mask '\textit{mask}'.

The computation of the \textit{Triplet loss} using the \textbf{Batch All} \textit{Online Triplet Mining Strategy} is described by algorithm \ref{alg:batchAllTripletLossFunction} (\textbf{BatchAllTripletLoss function}). More specifically, the \textbf{BatchAllTripletLoss} function begins in a similar way as the \textbf{BatchHardTripletLoss} function by computing a 2D matrix called '\textit{pairwise dist}' containing the euclidean distances between all possible pairs of sample embeddings, using function 'PairwiseDistances'. It then creates a 3D tensor '\textit{triplet\_loss}' of shape $(B, B, B)$ where the element at index $(i, j, k)$ contains the loss for triplet $(i, j, k)$. This 3D tensor thus contains the losses of all possible triplets. Next, the function generates a 3D mask '\textit{mask}' of valid triplets with function '\textit{GetTripletMask}' which is used to set to $0$ the loss for all invalid triplets (where $label(a) \ne label(p)$ or $label(n) = label(a)$ or $a = p$) in the 3D matrix '\textit{triplet\_loss}'. Finally, the function removes the negative losses (i.e. the easy triplets), calculates the fraction of positive triplets (where triplet loss $> 0$) with respect to the valid ones and computes the final \textbf{Triplet loss} by averaging over the positive triplets.

The \textbf{GetTripletMask} function, as described by algorithm \ref{alg:getTripletMaskFunction}, returns a 3D mask where $mask[a, p, n]$ is \textit{True} if and only if the triplet $(a, p, n)$ is \textit{valid} where a triplet $(i, j, k)$ is considered valid if $i$, $j$ and $k$ are distinct and $labels[i] = labels[j]$ and $labels[i] \ne labels[k]$. More specifically, the function first creates 3 3D masks '\textit{i\_not\_equal\_j}', '\textit{i\_not\_equal\_k}' and '\textit{j\_not\_equal\_k}' by extending a 2D mask called '\textit{indices\_not\_equal}' obtained by selecting the non-identical samples (inverting an identity matrix). These 3 masks are then combined using the logical \textit{and} function to create the 3D mask '\textit{distinct\_indices}' which is used to select the triplets where samples $i$, $j$ and $k$ are distinct. The function then creates 2 more 3D masks '\textit{i\_equal\_j}' and '\textit{i\_equal\_k}' by extending a 2D mask called '\textit{label\_equal}' obtained by selecting the samples with the same label. These 2 masks are then combined using the logical \textit{and} function after having inverted the '\textit{i\_equal\_k}' mask to produce the 3D mask '\textit{valid\_labels}' which is used to select the triplets where $labels[i] = labels[j]$ and $labels[i] \ne labels[k]$. Finally, the function combines the '\textit{distinct\_indices}' and 'valid\_labels' using the logical \textit{and} function to obtain the final 3D mask '\textit{mask}'.
...
The \textit{Contrastive Model} evaluation function, presented in algorithm \ref{alg:evaluateContrastiveLearningModelFunctionPt1}-\ref{alg:evaluateContrastiveLearningModelFunctionPt2}-\ref{alg:evaluateContrastiveLearningModelFunctionPt3} (\textbf{Contrastive Model Evaluation function}), begins (as shown in alg. \ref{alg:evaluateContrastiveLearningModelFunctionPt1}) by opening and loading the \textit{Fresh Dataset} splitting it in training, validation and test subsets following the selected proportions, similarly to what was done in the training function. The actual \textit{Contrastive Model} then gets instantiated by providing the necessary arguments to its \textit{init} member function and the checkpoint state parameters from a previous training run are loaded. Next, the model is set into \textit{evaluation} mode.
...
Then, as shown in alg. \ref{alg:evaluateContrastiveLearningModelFunctionPt2}, the function considers the samples from the test subset of the \textit{Fresh Dataset} as anchors for the computation of the corresponding ranking and family predictions. In practice, for all the mini-batches of data from the (\textit{Fresh Dataset}) test generator, the samples' features are fed into the model to obtain the corresponding \textit{PE Embedding} representations '\textit{anchor\_pe\_embeddings}'. Next, the function cycles through all the mini-batches of data from the (\textit{Fresh Dataset}) train generator in order to obtain the rankings for the current test mini-batch considering the samples from the training subset of the \textit{Fresh Dataset} as query samples. More specifically, the function obtains the \textit{PE Embedding} representation of the training samples '\textit{query\_pe\_embeddings}' by forwarding the corresponding features through the (same) model and then computes the \textit{Euclidean} distances between the embeddings of the \textit{anchor} and \textit{query} samples. These distances are used to update a set of 3 tensors of fixed size (equal to the selected \textit{rank\_size}) called '\textit{top\_shas}', '\textit{top\_labels}' and '\textit{top\_distances}' which contain, respectively, the shas, labels and distances (to the current anchor) of the top '\textit{rank\_size}' most similar samples, per anchor, ordered by similarity.
...
At the end of the inner cycle, as shown in algorithm \ref{alg:evaluateContrastiveLearningModelFunctionPt3}, these 3 tensors (\textit{top\_shas}, \textit{top\_labels} and \textit{top\_distances}) are used to generate the actual rankings for the anchor samples of the current test mini-batch. Thereafter, the function applies the \textbf{Distance Weighted k-NN} approach using the generated rankings to obtain the actual family predictions for the test samples of the current mini-batch. In particular, for each odd value '\textit{k}' from '\textit{knn\_k\_min}' to '\textit{knn\_k\_max}', the function gets the first \textit{k} labels from the current \textit{top\_labels} tensor, computes each nearest neighbour weight inverting its distance squared to the anchor and assigns to the current anchor the family with the highest cumulative score. Finally, the function computes the ranking scores (\textbf{MAP and MRR}) and saves to file both the produced rankings and family predictions.
	...
	\begin{algorithmic}[1]
		\algrestore{bkbreak}
				...
				\For{\textit{k}, \textit{s} $\in$ \textbf{enumerate}(\textit{shas})}
					...
					\State \hskip1.9em 'rank\_labels': [\textit{lab} $\forall$ \textit{lab} $\in$ \textit{top\_labels}[\textit{k}]],
...
The \textit{Contrastive (Siamese) Model} evaluation accuracy and \textbf{\textit{micro}} (global average), \textbf{\textit{macro}} (unweighted per-family average) and \textbf{\textit{weighted}} (per-family average, weighted by support) scores, such as '\textit{Jaccard similarity score}', '\textit{recall}', '\textit{precision}' and '\textit{f1-score}', are computed for each value of $k$ and saved to file, together with the resulting confusion matrixes, by a distinct set of functions - which simply apply the respective mathematical formulas - not presented in this document. Moreover, after $n\_runs$ another set of function plots the trends of the previously computed multi-class classification scores as the number of nearest neighbours $k$ changes from '\textit{knn\_k\_min}' to '\textit{knn\_k\_max}'.
