...
Here is an example:
\begin{lstlisting}[caption={Jump Instructions with the Same Target}, label=JumpSameTarget, language={[x86masm]Assembler}, commentstyle=\color{ForestGreen}, keepspaces=true, keywordstyle=\color{blue}]
74 03		jz 	loc
75 01		jnz	loc
...
Example:
\begin{lstlisting}[caption={Jump Instructions with a Constant Condition example}, label=JumpConstant, language={[x86masm]Assembler}, commentstyle=\color{ForestGreen}, keepspaces=true, keywordstyle=\color{blue}]
33 C0		xor	eax, eax
74 01		jz 	loc
...
Example:
\begin{lstlisting}[caption={Impossible Disassembly example}, label=ImpossibleDisassembly, language={[x86masm]Assembler}, commentstyle=\color{ForestGreen}, keepspaces=true, keywordstyle=\color{blue}]
EB 
	JMP -1
FF
	INC EAX
...
\begin{itemize}
	...
	\begin{lstlisting}[caption={OutputDebugString debugger detection}, label=OutputDebugString, language=C, commentstyle=\color{ForestGreen}, keepspaces=true, keywordstyle=\color{blue}]
...
\begin{itemize}
	...
	\begin{lstlisting}[caption={BeingDebugged manual check}, label=BeingDebuggedManual, language={[x86masm]Assembler}, commentstyle=\color{ForestGreen}, keepspaces=true, keywordstyle=\color{blue}]
...
However, malware authors usually combine those 2 types of obfuscation techniques in complex and difficult ways to strengthen the resulting obfuscation.

\subsubsection{Data-Based Obfuscation}
Data-based obfuscation techniques focus on modifying data values and non-control computations.

\paragraph{Constant Unfolding}
\textit{Constant folding} is a technique commonly used by compilers to optimize a program's code. It does so by replacing expressions with results known at compile time with the results themselves.
...
\begin{lstlisting}[caption={Before constant folding}, label=ConstantFoldingBefore, language=C, commentstyle=\color{ForestGreen}, keepspaces=true, keywordstyle=\color{blue}]
x = 4 * 5;
\end{lstlisting}

\begin{lstlisting}[caption={After constant folding}, label=ConstantFoldingAfter, language=C, commentstyle=\color{ForestGreen}, keepspaces=true, keywordstyle=\color{blue}]
x = 20;
\end{lstlisting}
...
\begin{lstlisting}[caption={Before constant unfolding}, label=ConstantUnfoldingBefore, language={[x86masm]Assembler}, commentstyle=\color{ForestGreen}, keepspaces=true, keywordstyle=\color{blue}]
push	0h
\end{lstlisting}

\begin{lstlisting}[caption={After constant unfolding}, label=ConstantUnfoldingAfter, language={[x86masm]Assembler}, commentstyle=\color{ForestGreen}, keepspaces=true, keywordstyle=\color{blue}]
push	0F9CBE47Ah
add	dword ptr [esp],	6341B86h
\end{lstlisting}

\paragraph{Data-Encoding Schemes}
The previously described technique is, however, easily defeated by simply applying the standard compiler's constant folding optimization. This is possible because both the data encoding and decoding functions ($f(x)=x-6341B86h$ and $f_{-1}(x)=x+6341B86h$ respectively) were present in the code one after the other. The use of \textit{fully Homomorphic} mappings (operation-preserving mappings) allow us to perform some operations on the encoded data before decoding it back, thus overcoming the previous technique's flaw. \textit{Fully Homomorphic} mappings are however still not widely used because still too inefficient.

\paragraph{Dead Code Insertion}
\textit{Dead code elimination} is another common compiler optimization technique. Its objective is to remove program statements/expressions that nave no real effects on the program operation and final results.
...
\begin{lstlisting}[caption={Before dead code elimination}, label=DeadCodeEliminationBefore, language=C, commentstyle=\color{ForestGreen}, keepspaces=true, keywordstyle=\color{blue}]
int f(){
	int x, y;
...
\begin{lstlisting}[caption={After dead code elimination}, label=DeadCodeEliminationAfter, language=C, commentstyle=\color{ForestGreen}, keepspaces=true, keywordstyle=\color{blue}]
int f(){
	return 3;
}
\end{lstlisting}
...
However, when used alone, this techniques produces an obfuscated program that can be efficiently de-obfuscated by using the compiler's dead code elimination optimization, which is effectively the inverse operation.

\paragraph{Arithmetic Substitution via Identities}
This technique aims at replacing certain operators with combinations of other operators with equal net result. Exploiting the equivalence between different combinations of different operators the code can be changed arbitrarily without changing the effective program operation and final result. Here are some examples of operators equivalences:

\begin{lstlisting}[caption={Operators equivalences}, label=OperatorEquivalences, language=C, commentstyle=\color{ForestGreen}, keepspaces=true, keywordstyle=\color{blue}]
-x == ~x + 1
...
\textit{Pattern-based} obfuscation, another commonly used technique, consists in constructing \textit{patterns} (transformations) that map single or multiple adjacent instructions into a more complex, computationally equivalent, sequence of instructions.
...
\begin{lstlisting}[caption={Original sequence}, label=PatternObfuscationOriginalSequence, language={[x86masm]Assembler}, commentstyle=\color{ForestGreen}, keepspaces=true, keywordstyle=\color{blue}]
push	reg32
\end{lstlisting}

\begin{lstlisting}[caption={Obfuscation using pattern 1}, label=PatternObfuscationPattern1, language={[x86masm]Assembler}, commentstyle=\color{ForestGreen}, keepspaces=true, keywordstyle=\color{blue}]
push	imm32
mov	dword ptr [esp],	reg32
\end{lstlisting}

\begin{lstlisting}[caption={Obfuscation using pattern 2}, label=PatternObfuscationPattern2, language={[x86masm]Assembler}, commentstyle=\color{ForestGreen}, keepspaces=true, keywordstyle=\color{blue}]
lea	esp,	[esp-4]
mov	dword ptr [esp],	reg32
\end{lstlisting}

\begin{lstlisting}[caption={Obfuscation using pattern 3}, label=PatternObfuscationPattern3, language={[x86masm]Assembler}, commentstyle=\color{ForestGreen}, keepspaces=true, keywordstyle=\color{blue}]
sub	esp,	4
mov	dword ptr [esp],	reg32
\end{lstlisting}
...
\begin{lstlisting}[caption={Original sequence \#2}, label=PatternObfuscationOriginalSequence2, language={[x86masm]Assembler}, commentstyle=\color{ForestGreen}, keepspaces=true, keywordstyle=\color{blue}]
sub	esp,	4
\end{lstlisting}

\begin{lstlisting}[caption={Obfuscation of sequence \#2}, label=PatternObfuscation2Pattern, language={[x86masm]Assembler}, commentstyle=\color{ForestGreen}, keepspaces=true, keywordstyle=\color{blue}]
push	reg32
mov	reg32,	esp
xchg	[esp],	reg32
pop	esp
...
\subsection{Polymorphic and Metamorphic Malware}
